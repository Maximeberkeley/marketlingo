...
- Give each lesson a good wavy look, and make sure people can scroll all the way to the last module and the last lesson. Take huge inspo from duolingo, but keep app identity. Make sure as you change sections, colors change, title change, your screen indicates some change. The app should feel alive. 
...
- Make back navigation return naturally to the Course map. it should take you to al full page, rediseged and simplified to match app new standrads. 
...
- no flotting islands that have just square rounded shapes and look like a beta app, everything must feel thought and polished, and serve a visual purpuse. EVERYTHING, from the countering, to the text shape, to every single component on the screen. It must serve a purpose. 
...
- Check the mobile build and runtime logs before declaring the redesign ready for the next archive. only modify the /mobile, no web app.