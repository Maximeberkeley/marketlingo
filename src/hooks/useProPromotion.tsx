...
export function useProPromotion() {
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(promoState));
  }, [promoState]);
  ...
  useEffect(() => {
    ...
    // Only increment if this is a new session (different from last activity)
  ...
  const triggerAfterLesson = useCallback((lessonCount: number) => {
    ...
    // Show after every 4th or 5th lesson
  ...
  }, [promoState.sessionCount, checkRandomTrigger, shouldShowPromo]);
