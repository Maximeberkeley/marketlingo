...
const QUEST_POOL: Omit<DailyQuest, "current" | "isCompleted">[] = [
  ...
  { id: "speed_learner", title: "Speed Learner", description: "Complete lesson in under 5 min", emoji: "⏱️", target: 1, xpBonus: 35, multiplier: 1.8, type: "lesson" },
  { id: "finish_2_games", title: "Gamer", description: "Complete 2 games", emoji: "🕹️", target: 2, xpBonus: 25, multiplier: 1.4, type: "game" },
];

function getDayQuests(dateStr: string) {
  let seed = 0;
...
export function useDailyQuests(dailyCompletion: DailyCompletionData | null, streak?: number) {
  ...
  const quests = useMemo<DailyQuest[]>(() => {
    ...
    return templates.map((t) => {
      let current = 0;
      switch (t.type) {
        case "lesson": current = lesson ? 1 : 0; break;
        case "drill": current = Math.min(drills, t.target); break;
        case "game": current = Math.min(games, t.target); break;
        case "combo":
          if (t.id === "lesson_plus_drill") current = (lesson ? 1 : 0) + Math.min(drills, 1);
          else if (t.id === "triple_threat") current = (lesson ? 1 : 0) + (games > 0 ? 1 : 0) + (drills > 0 ? 1 : 0);
          break;
        case "streak": current = Math.min(streak || 0, t.target); break;
      }
  ...
  return { quests, completedCount, totalBonusXP, allComplete };
}
