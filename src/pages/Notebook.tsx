...
import { Search, BookOpen, Loader2, Lightbulb, Calendar, Tag, ArrowLeft, Sparkles, PenLine, FileText } from "lucide-react";
...
export default function NotebookPage() {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      ...
      if (error) { console.error("Error fetching notes:", error); setNotes([]); }
      else setNotes(data || []);
      setLoading(false);
    };
  ...
  const handleAddNote = async () => {
    ...
    const { data, error } = await supabase
      .from("notes")
      .insert({ user_id: user.id, content: newNoteContent.trim(), linked_label: "Personal Note", market_id: selectedMarket })
      .select()
      .single();
    ...
    <AppLayout>
      <div className="screen-padding pt-safe pb-28">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 mb-4">
          ...
          {notes.length > 0 && (
            <button
              ...
              className="w-10 h-10 rounded-xl bg-accent flex items-center justify-center shadow-md active:scale-95 transition-transform"
            >
              <PenLine size={18} className="text-bg-0" />
            </button>
          )}
        </motion.div>
        ...
        {notes.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            <div className="relative overflow-hidden rounded-2xl">
              <img src={notebookHero} alt="Notebook" className="w-full h-40 object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-rose-900/90 via-rose-900/30 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-5">
                ...
                <button
                  onClick={() => setShowAddNote(true)}
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/20 backdrop-blur-sm border border-white/30 text-sm text-white font-medium active:scale-95 transition-transform"
                >
                  <PenLine size={14} />
                  Add your first note
                </button>
        ...
        {notes.length > 0 && notes.length < 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-4 p-3 rounded-xl bg-accent/5 border border-accent/20">
        ...
        {/* Filters */}
        {notes.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="flex gap-2 mb-5 overflow-x-auto scrollbar-hide pb-1">
            {filters.map((filter) => (
              <button
                key={filter.id || "all"}
        ...
        {Object.keys(groupedNotes).length > 0 ? (
          <div className="space-y-5">
            {Object.entries(groupedNotes).map(([date, dateNotes], groupIdx) => (
              <motion.div key={date} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + groupIdx * 0.05 }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
            ...
            <p className="text-body text-text-muted">Try a different search term or filter</p>
          </motion.div>
        ) : null}
      </div>
        ...
        {showAddNote && (
          ...
          >
            ...
            >
              ...
              <div className="px-5 pb-4 overflow-y-auto flex-1" style={{ overscrollBehavior: 'contain' }}>
                ...
                <div className="flex gap-3 mt-4">
                  <button onClick={() => setShowAddNote(false)} className="flex-1 py-3 rounded-xl bg-bg-2 border border-border text-caption text-text-secondary">
                    Cancel
                  </button>
                  <button onClick={handleAddNote} disabled={!newNoteContent.trim()} className="flex-1 py-3 rounded-xl bg-accent text-white text-caption font-medium disabled:opacity-50">
                    Save Note
                  </button>
                </div>
              </div>
