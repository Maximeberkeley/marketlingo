...
function getActivityIcon(friend) {
    if (friend.currentStreak >= 7)
        return 'activity';
    if (friend.totalXP > 1000)
        return 'award';
    return 'book-open';
}
export function FriendActivityFeed({ friends }) {
    ...
    return (<View style={styles.container}>
      ...
      {recentFriends.map((friend) => (<View key={friend.id} style={styles.row}>
