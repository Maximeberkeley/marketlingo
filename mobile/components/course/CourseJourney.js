...
function MovingLessonTitle({ title, long }) {
    ...
    return (<View style={styles.weekTitleWrap}>
      ...
      {!reduceMotion ? (<Animated.View pointerEvents="none" style={[styles.titleSweep, {
...
function SectionCluster({ section, lesson, title, weekTitle, marketName, activeSection, lessonCompletedToday, arenaCompletedToday, caseCompletedToday, intelDone, onModule, onLeo, onHeader, }) {
    const moduleComplete = (kind) => {
        if (!activeSection)
            return kind === 'lesson' && Boolean(lesson?.completed);
        if (kind === 'lesson')
            return lessonCompletedToday || Boolean(lesson?.completed);
        if (kind === 'arena')
            return arenaCompletedToday;
        if (kind === 'case')
            return caseCompletedToday;
        if (kind === 'intel')
            return intelDone;
        return false;
    };
    const coreCompleted = ['lesson', 'arena', 'case', 'intel']
    ...
    return (<View style={styles.sectionBlock}>
      ...
      <View style={[styles.cluster, !section.unlocked && styles.clusterLocked]}>
        <Svg width={268} height={268} style={styles.orbitSvg} pointerEvents="none">
          ...
          <Circle cx={134} cy={134} r={orbitRadius} fill="none" stroke={COLORS.accent} strokeWidth={4} strokeLinecap="round" strokeDasharray={`${orbitCircumference} ${orbitCircumference}`} strokeDashoffset={orbitOffset} rotation={-90} origin="134, 134"/>
        </Svg>
...
function CurriculumPreview({ visible, section, title, lessons, currentDay, onClose, onOpenLesson, }) {
    if (!section)
        return null;
      ...
      <View style={styles.previewScreen}>
        ...
        <ScrollView contentContainerStyle={styles.previewList} showsVerticalScrollIndicator={false}>
          ...
          {sectionLessons.map(item => {
            ...
            return (<TouchableOpacity key={item.day} style={[styles.lessonRow, locked && styles.lessonRowLocked]} disabled={locked} onPress={() => onOpenLesson(item)} accessibilityRole="button" accessibilityLabel={`Day ${item.day}, ${item.title}${locked ? ', locked' : item.completed ? ', complete' : ''}`} accessibilityState={{ disabled: locked }}>
                ...
                <View style={styles.lessonCopy}>
                  <Text style={[styles.lessonTitle, item.title.length > 35 && styles.lessonTitleLong, locked && styles.lockedText]} numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.82}>{item.title}</Text>
...
export function CourseJourney({ marketId, currentDay, learningGoal, completedStackIds, streak, totalXp, level, lessonCompletedToday, arenaCompletedToday = false, caseCompletedToday = false, intelReadToday, intelTarget, rescueAvailable = false, safeTop, onOpenLesson, onAskLeo, }) {
    ...
    useEffect(() => () => {
        if (industryPopoverTimer.current)
            clearTimeout(industryPopoverTimer.current);
    }, []);
    ...
    const handleModule = (section, lesson, kind) => {
        ...
        playSound('tap').catch(() => { });
        if (kind === 'lesson') {
            if (lesson?.stackId && lesson.day <= currentDay)
                onOpenLesson(lesson.stackId);
            else
                Alert.alert('Lesson unavailable', 'This lesson is not ready yet. Your progress is safe.');
        }
        else if (kind === 'arena') {
    ...
    return (<View style={styles.container}>
      <FlatList ref={listRef} data={sections} keyExtractor={item => String(item.index)} showsVerticalScrollIndicator={false} initialNumToRender={3} contentContainerStyle={styles.listContent} onScrollToIndexFailed={({ index }) => listRef.current?.scrollToOffset({ offset: Math.max(0, index * 540), animated: false })} ListHeaderComponent={(<View style={[styles.topHeader, { paddingTop: safeTop + 10 }]}>
            <View style={styles.industryBadgeWrap}>
              <TouchableOpacity style={styles.industryBadge} onPress={showIndustryPopover} activeOpacity={0.82} accessibilityRole="button" accessibilityLabel={`Current industry: ${marketName}`} accessibilityHint="Shows your active industry">
