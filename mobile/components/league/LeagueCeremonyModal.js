...
import { LEAGUE_TIERS, TIER_META } from '../../hooks/useLeague';
...
export function LeagueCeremonyModal({ visible, tier, result, finalRank, onClose, onViewLeague }) {
    const rise = useRef(new Animated.Value(0)).current;
    const glow = useRef(new Animated.Value(0)).current;
    const destination = result === 'promoted' ? nextTier(tier, 1) : result === 'demoted' ? nextTier(tier, -1) : tier;
    ...
    const headline = result === 'promoted' ? 'Promoted' : result === 'demoted' ? 'Relegated' : 'Position held';
    const subline = result === 'promoted'
        ...
        <View style={styles.card}>
          ...
          <Animated.View style={[
            ...
            <Animated.View style={[styles.badgeCore, { backgroundColor: meta.color + '18' }, { opacity: glow }]}>
              <Feather name={result === 'demoted' ? 'arrow-down-circle' : result === 'promoted' ? 'award' : 'shield'} size={56} color={meta.color}/>
            </Animated.View>
          ...
          <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: meta.color }]} onPress={() => {
            triggerHaptic('medium');
            onViewLeague();
        }} activeOpacity={0.9}>
