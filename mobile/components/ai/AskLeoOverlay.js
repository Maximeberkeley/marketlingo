...
import { View, Text, TouchableOpacity, TextInput, StyleSheet, Modal, Animated, ScrollView, KeyboardAvoidingView, Platform, } from 'react-native';
...
const MODES = [
    {
        ...
        instruction: 'Re-explain the current card at a 9th-grade level in two short sentences, using one everyday comparison.',
    },
    {
        id: 'example',
        ...
        instruction: 'Give one concrete example from this industry — a real company, deal or number — in two short sentences.',
    },
    {
        id: 'why',
        ...
        instruction: 'In two short sentences, say where this shows up in a real job, interview or investment call.',
    },
    {
        id: 'quiz',
        label: 'Quiz me',
        ...
        ask: 'Quiz me on this.',
        instruction: 'Ask exactly one short question about the current card, with three lettered options (A, B, C). Do not reveal the answer yet. When the learner replies, say if they are right in one sentence and why.',
    },
];
export function AskLeoOverlay({ visible, onClose, lessonContext, contextLabel, accentColor, messages: externalMessages, onMessagesChange, autoAsk, onSaveAnswer, }) {
    ...
    const ask = useCallback(async (text, instruction) => {
        ...
        if (!(await isFeatureEnabled('ai_leo'))) {
            setMessages([
                ...
                {
                    role: 'assistant',
                    content: 'Leo is taking a short break. Your lessons, drills and reviews all still work.',
                },
            ]);
            return;
        }
        ...
        try {
            ...
            const payload = instruction
                ? [...next.slice(0, -1), { role: 'user', content: `${question}\n\n${instruction}` }]
                : next;
            ...
            const answer = data.message || "Hmm, that one lost me. Ask me again?";
    ...
    const leoMood = useMemo(() => {
        if (isLoading)
            return 'thinking';
        if (typed !== null || isPlayingAudio)
            return 'waving';
        return 'idle';
    }, [isLoading, typed, isPlayingAudio]);
    const handleSave = useCallback((idx, text) => {
        onSaveAnswer?.(text);
        setSaved(s => [...s, idx]);
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => { });
    }, [onSaveAnswer]);
    return (<Modal visible={visible} animationType="fade" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 8 : 0}>
        ...
        <Animated.View style={[
          ...
          {!!contextLabel && (<View style={[styles.contextStrip, { borderLeftColor: accent }]}>
          ...
          <ScrollView ref={scrollRef} style={styles.messagesScroll} contentContainerStyle={styles.messagesContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            {messages.length === 0 && !isLoading && (<View style={styles.emptyState}>
            ...
            return (<View key={idx} style={[styles.row, msg.role === 'user' ? styles.rowUser : styles.rowLeo]}>
                  <View style={[
                    styles.bubble,
                    msg.role === 'user'
                  ...
                  {msg.role === 'assistant' && !isTyping && (<View style={styles.answerActions}>
                      <TouchableOpacity style={styles.answerBtn} onPress={() => playTTS(msg.content)}>
                        <Feather name={isPlayingAudio ? 'pause' : 'volume-2'} size={14} color={tokens.color.textSecondary}/>
                        <Text style={styles.answerBtnText}>
                          {isPlayingAudio ? 'Stop' : 'Listen'}
                        </Text>
                      </TouchableOpacity>
                      {!!onSaveAnswer && (<TouchableOpacity style={styles.answerBtn} onPress={() => handleSave(idx, msg.content)} disabled={saved.includes(idx)}>
                          <Feather name={saved.includes(idx) ? 'check' : 'bookmark'} size={14} color={saved.includes(idx) ? tokens.color.correctDark : tokens.color.textSecondary}/>
                          <Text style={[
            ...
            {isLoading && (<View style={[styles.row, styles.rowLeo]}>
          ...
          {modesExpanded ? (<Animated.View style={{ opacity: modesAnim }}>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.modeRowBig} keyboardShouldPersistTaps="handled">
                {MODES.map(mode => (<TouchableOpacity key={mode.id} style={[styles.modeCard, { borderColor: mode.color + '66', backgroundColor: mode.color + '14' }]} onPress={() => ask(mode.ask, mode.instruction)} disabled={isLoading} activeOpacity={0.85}>
            ...
            </Animated.View>) : (<View style={styles.modeRowSmallWrap}>
              <TouchableOpacity style={styles.modeExpandBtn} onPress={() => {
                setModesExpanded(true);
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.modeRow} keyboardShouldPersistTaps="handled">
                {MODES.map(mode => (<TouchableOpacity key={mode.id} style={[styles.modeChip, { borderColor: mode.color + '55', backgroundColor: mode.color + '12' }]} onPress={() => ask(mode.ask, mode.instruction)} disabled={isLoading}>
          ...
          <View style={styles.inputRow}>
            <TextInput style={styles.textInput} value={input} onChangeText={setInput} placeholder={isRecording ? 'Listening…' : 'Ask Leo anything…'} placeholderTextColor={tokens.color.textMuted} multiline maxLength={500} returnKeyType="send" onSubmitEditing={() => ask(input)}/>
            {input.trim().length === 0 ? (<TouchableOpacity style={[styles.micBtn, isRecording && { backgroundColor: tokens.color.incorrect }]} onPressIn={startRecording} onPressOut={stopRecording} disabled={isLoading}>
                <Feather name="mic" size={20} color={isRecording ? '#fff' : accent}/>
              </TouchableOpacity>) : (<TouchableOpacity style={[styles.sendBtn, { backgroundColor: accent }, isLoading && styles.disabled]} onPress={() => ask(input)} disabled={isLoading}>
