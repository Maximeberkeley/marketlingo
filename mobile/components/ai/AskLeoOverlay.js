...
const LEO_VOICE_ID = "onwK4e9ZLuTAKqWW03F9";
  ...
  const ask = useCallback(
    async (text, instruction) => {
      ...
      try {
        ...
        const payload = instruction ? [...next.slice(0, -1), { role: "user", content: `${question}
...
${instruction}` }] : next;
  ...
  const leoMood = useMemo(() => {
    ...
    if (typed !== null || isPlayingAudio) return "waving";
    return "idle";
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerText}>
              ...
              <Text style={styles.headerSub}>
                {isLoading ? "Thinking it through\u2026" : isPlayingAudio ? "Talking" : "Your study partner for this card"}
              </Text>
            </View>
            <TouchableOpacity
    onPress={toggleMute}
  ...
  >
    ...
    >
      ...
      style={[
        styles.bubble,
        msg.role === "user" ? [styles.userBubble, { backgroundColor: accent }] : styles.leoBubble
      ]}
    >
    ...
    keyboardShouldPersistTaps="handled"
  >
                {MODES.map((mode) => <TouchableOpacity
    key={mode.id}
  ...
  />
            {input.trim().length === 0 ? <TouchableOpacity
