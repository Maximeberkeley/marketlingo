...
export function LeoCoachChips({ context }) {
    ...
    const run = useCallback(async (action) => {
        ...
        try {
            ...
            if (res.status === 429)
                throw new Error('Leo is busy right now. Try again in a moment.');
            if (res.status === 402)
                throw new Error('AI coaching is unavailable right now.');
            if (!res.ok)
                throw new Error('Leo could not respond. Try again.');
    ...
    return (<View style={styles.wrap}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
        {ACTIONS.map((a) => (<TouchableOpacity key={a.id} style={[styles.chip, loadingAction === a.id && styles.chipActive]} onPress={() => run(a)} disabled={loadingAction !== null} activeOpacity={0.85}>
            {loadingAction === a.id ? (<ActivityIndicator size="small" color={COLORS.accent}/>) : (<Feather name={a.icon} size={13} color={COLORS.accent}/>)}
      ...
      {!!response && (<View style={styles.answer}>
