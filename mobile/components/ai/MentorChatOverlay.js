...
export function MentorChatOverlay({ visible, mentor, onClose, context, marketId }) {
    ...
    useEffect(() => {
        if (visible && mentor) {
            const initial = [{ id: '0', role: 'mentor', content: mentor.greeting }];
...
${context ? `Current context: ${context}` : ''}`;
            ...
            const errorMsg = err?.message?.includes('429') || err?.status === 429
    ...
    return (<Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 10 : 0}>
        ...
        {/* Messages */}
        <ScrollView ref={scrollViewRef} style={styles.messages} contentContainerStyle={{ paddingVertical: 16, gap: 10 }} showsVerticalScrollIndicator={false}>
          {messages.map((msg) => (<View key={msg.id} style={[styles.bubble, msg.role === 'user' ? styles.userBubble : styles.mentorBubble]}>
              {msg.role === 'mentor' && (<View style={styles.bubbleMentorAvatar}>
          ...
          {isLoading && (<View style={[styles.bubble, styles.mentorBubble]}>
        ...
        {messages.length <= 3 && (<ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.suggestionsRow} contentContainerStyle={{ gap: 8, paddingHorizontal: 16, paddingVertical: 8 }}>
            {getContextualPrompts(context).map((prompt) => (<TouchableOpacity key={prompt} style={styles.suggestionChip} onPress={() => setInput(prompt)}>
        ...
        <View style={styles.inputRow}>
          <TextInput style={styles.input} placeholder={`Ask ${mentor.name.split(' ')[0]}...`} placeholderTextColor={COLORS.textMuted} value={input} onChangeText={setInput} multiline returnKeyType="send" onSubmitEditing={handleSend} editable={!isLoading}/>
          <TouchableOpacity style={[styles.sendBtn, (!input.trim() || isLoading) && { opacity: 0.4 }]} onPress={handleSend} disabled={!input.trim() || isLoading}>
            {isLoading ? (<ActivityIndicator size="small" color="#FFFFFF"/>) : (<Text style={styles.sendBtnText}>→</Text>)}
          </TouchableOpacity>
        </View>
