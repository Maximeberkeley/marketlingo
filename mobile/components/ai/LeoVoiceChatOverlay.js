...
function LeoVoiceChatOverlay({
  ...
  useEffect(() => {
    if (visible) {
      ...
      loadLeoMuted().then((muted) => {
        ...
        setTimeout(() => {
          if (cancelled) return;
          const greeting = "Hey! \u{1F98A} Ask me anything about your industry!";
  ...
  const sendToLeo = useCallback(async (userText) => {
    ...
    try {
      ...
      const data = await response.json();
      const reply = data?.message || "Hmm, let me think about that... \u{1F914}";
      setMessages((prev) => [...prev, { role: "assistant", content: reply }]);
      setNarrationText(reply);
      setIsGenerating(false);
      await speakResponse(reply);
    } catch {
      const fallback = "Oops, I had a hiccup! Try again. \u{1F98A}";
  ...
  >
              {isGenerating || isTranscribing ? <View style={st.statusRow}>
                  ...
                  {narrationText || "Hey! \u{1F98A} Ask me anything about your industry!"}
                </Text>}
          ...
          <View style={[st.bottomBar, { paddingBottom: insets.bottom + 16 }]}>
            {showTextInput ? <View style={st.keyboardComposer}>
                <TouchableOpacity style={st.keyboardDismiss} onPress={() => {
    Keyboard.dismiss();
    setShowTextInput(false);
