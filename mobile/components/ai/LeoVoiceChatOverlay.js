...
import { View, Text, StyleSheet, Modal, Image, TouchableOpacity, Animated, Dimensions, ActivityIndicator, Platform, StatusBar, TextInput, KeyboardAvoidingView, Keyboard, ScrollView, } from 'react-native';
...
async function transcribeVoice(uri) {
    try {
...
export function LeoVoiceChatOverlay({ visible, onClose, marketId = 'general', lessonContext, }) {
    ...
    const sendToLeo = useCallback(async (userText) => {
    ...
    if (!visible)
        return null;
    return (<Modal visible={visible} animationType="none" transparent statusBarTranslucent>
        ...
        <Animated.View style={[st.container, { opacity: fadeAnim }]}>
          ...
          <View style={[st.topBar, { paddingTop: insets.top + 12 }]}>
            ...
            <View style={st.topBarRight}>
              <TouchableOpacity style={[st.topBtn, showTextInput && st.topBtnActive]} onPress={() => setShowTextInput(!showTextInput)} accessibilityRole="button" accessibilityLabel="Toggle text mode">
          ...
          <View style={[st.dialogStage, { bottom: insets.bottom + 154 }]}>
            ...
            <TouchableOpacity style={st.subtitleBox} onPress={() => setSubtitlesExpanded(!subtitlesExpanded)} activeOpacity={0.9} accessibilityRole="button" accessibilityLabel="Leo's latest response">
              {isGenerating || isTranscribing ? (<View style={st.statusRow}>
                  <ActivityIndicator size="small" color="#FB7A22"/>
                  <Text style={st.subtitleText}>{isTranscribing ? 'Transcribing…' : 'Leo is thinking…'}</Text>
                </View>) : subtitlesExpanded && messages.length > 0 ? (<ScrollView style={st.conversationScroll} contentContainerStyle={st.conversationContent} showsVerticalScrollIndicator>
                  {messages.map((message, index) => (<View key={`${message.role}-${index}`} style={[st.conversationBubble, message.role === 'user' ? st.userConversationBubble : st.leoConversationBubble]}>
                      <Text style={st.conversationRole}>{message.role === 'user' ? 'YOU' : 'LEO'}</Text>
                      <Text style={st.conversationText}>{message.content}</Text>
                    </View>))}
                </ScrollView>) : (<Text style={st.subtitleText} numberOfLines={4}>
                  {narrationText || "Hey! 🦊 Ask me anything about your industry!"}
                </Text>)}
              {!subtitlesExpanded && narrationText.length > 150 && (<Text style={st.expandHint}>Tap to read more</Text>)}
            </TouchableOpacity>

            <View style={st.statusContainer}>
              {isGenerating && (<Text style={st.statusText}>Preparing an answer</Text>)}
              {isRecording && (<View style={st.statusRow}>
                  <Animated.View style={[st.recordDot, { opacity: glowAnim }]}/>
                  <Text style={st.statusText}>Listening… tap again to send</Text>
                </View>)}
              {isSpeaking && !isGenerating && (<Text style={st.statusText}>Leo is speaking…</Text>)}
            </View>
          </View>

          <View style={[st.bottomBar, { paddingBottom: insets.bottom + 16 }]}>
            {showTextInput ? (<View style={st.keyboardComposer}>
                ...
                <View style={st.textInputRow}>
                  <TextInput style={st.textInput} value={textInput} onChangeText={setTextInput} placeholder="Type a message..." placeholderTextColor="rgba(255,255,255,0.4)" multiline autoFocus blurOnSubmit maxLength={500} returnKeyType="send" onSubmitEditing={handleSendText}/>
                <TouchableOpacity style={[st.sendBtn, (!textInput.trim() || isGenerating) && { opacity: 0.4 }]} onPress={handleSendText} disabled={!textInput.trim() || isGenerating}>
                  <Feather name="arrow-up" size={18} color="#fff"/>
                </TouchableOpacity>
                </View>
              </View>) : (<View style={st.callToolbar}>
                <TouchableOpacity style={st.sideAction} onPress={handleMute} accessibilityRole="button" accessibilityLabel={isMuted ? 'Unmute Leo' : 'Mute Leo'}>
                ...
                <TouchableOpacity style={st.primaryAction} onPress={handleLeoTap} activeOpacity={0.8} disabled={isGenerating || isTranscribing} accessibilityRole="button" accessibilityLabel={isRecording ? 'Stop and send recording' : 'Talk to Leo'}>
                  <Animated.View style={[
                st.bigMicBtn,
                ...
                <TouchableOpacity style={st.sideAction} onPress={() => setShowTextInput(true)} accessibilityRole="button" accessibilityLabel="Open keyboard">
