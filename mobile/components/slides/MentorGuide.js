...
const getMentorMessage = (slideIndex, totalSlides, isIntro, mentorId, marketId) => {
...
export function MentorGuide({ context, slideIndex, totalSlides, isIntro = false, marketId }) {
    const mentor = getMentorForContext(context, marketId);
    const message = useMemo(() => getMentorMessage(slideIndex, totalSlides, isIntro, mentor.id, marketId), [slideIndex, totalSlides, isIntro, mentor.id, marketId]);
    const mentorImage = MENTOR_IMAGES[mentor.id];
    const firstName = mentor.name.split(' ')[0];
    return (<View style={styles.container}>
      {/* Mentor Avatar */}
      <View style={styles.avatarWrap}>
        {mentorImage ? (<Image source={mentorImage} style={styles.avatar}/>) : (<View style={styles.avatarFallback}>
