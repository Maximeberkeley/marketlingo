...
export function QuizCard({ quiz, onAnswer, accentColor }) {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.options}>
        {quiz.options.map((option, i) => {
            ...
            return (<TouchableOpacity key={i} onPress={() => handleSelect(i)} disabled={revealed} style={[styles.option, { backgroundColor: bg, borderColor: border }]} activeOpacity={0.75}>
              ...
              {revealed && isSelected && (<Feather name={isCorrectOption ? 'check-circle' : 'x-circle'} size={18} color={isCorrectOption ? '#22C55E' : '#EF4444'}/>)}
            </TouchableOpacity>);
        })}
      </View>

      {/* Result feedback */}
      {revealed && (<View style={[styles.feedback, { borderColor: isCorrect ? '#22C55E40' : '#EF444440' }]}>
          <Text style={styles.feedbackMsg}>
            {isCorrect ? rand(CORRECT_MSGS) : rand(WRONG_MSGS)}
          </Text>
          {quiz.explanation && (<Text style={styles.feedbackExplain}>{quiz.explanation}</Text>)}
        </View>)}

      {!revealed && (<Text style={styles.tapHint}>Select the best answer</Text>)}
    </View>);
}
/** Generate a simple quiz from slide content */
export function generateQuizFromSlide(slideTitle, slideBody, _slideIndex) {
    // Only generate for slides with enough content
    if (!slideBody || slideBody.length < 100)
        return null;
    ...
    for (let i = shuffled.length - 1; i > 0; i--) {
        ...
        if (shuffled[i] === correctOption)
            correctIndex = i;
        if (shuffled[j] === correctOption)
            correctIndex = j;
    }
    return {
        question,
...
export function shouldShowQuiz(cardIndex, totalCards) {
    if (totalCards < 6)
        return false;
    if (cardIndex < 3 || cardIndex >= totalCards - 2)
        return false;
