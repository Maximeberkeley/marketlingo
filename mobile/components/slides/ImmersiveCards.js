...
import { fluidFont, fluidLineHeight } from '../../lib/fluidType';
...
export function ObjectiveCard({ goals, accentColor, marketId }) {
    ...
    return (<Animated.View style={[objStyles.container, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
      {/* Illustration watermark */}
      {illustration && (<Image source={illustration} style={objStyles.watermark}/>)}
      ...
      <View style={objStyles.goalsList}>
        {goals.map((goal, i) => (<Animated.View key={i} style={[
                objStyles.goalRow,
                {
...
export function RecapCard({ dayNumber, previousTopic, currentTopic, accentColor, marketId }) {
    const fadeIn = useRef(new Animated.Value(0)).current;
    ...
    return (<Animated.View style={[recapStyles.container, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
      ...
      <View style={recapStyles.timeline}>
        {/* Previous day */}
        {previousTopic && (<View style={recapStyles.timelineItem}>
...
export function ReflectionCard({ keyTakeaway, nextPreview, accentColor, dayNumber }) {
    const fadeIn = useRef(new Animated.Value(0)).current;
    const slideUp = useRef(new Animated.Value(40)).current;
    ...
    return (<Animated.View style={[reflStyles.container, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
      ...
      {/* What's next */}
      {nextPreview && (<View style={reflStyles.nextSection}>
      ...
      {dayNumber && (<View style={reflStyles.progressHint}>
