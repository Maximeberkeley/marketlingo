...
/* ── Component ─────────────────────────────────────────────────── */
export function WordMatchGame({ pairs, onComplete, accentColor }) {
    const [selectedTerm, setSelectedTerm] = useState(null);
    const [matched, setMatched] = useState(new Set());
    const [wrongPair, setWrongPair] = useState(null);
    const [firstTryScore, setFirstTryScore] = useState(0);
    const [attemptedTerms, setAttemptedTerms] = useState(new Set());
    // Animated values for matched cards
    const matchAnims = useRef(pairs.map(() => new Animated.Value(0))).current;
    const wrongAnim = useRef(new Animated.Value(0)).current;
    const completionAnim = useRef(new Animated.Value(0)).current;
    const shuffledDefs = useMemo(() => shuffleArray(pairs.map((p, i) => ({ ...p, origIndex: i }))), [pairs]);
    const allDone = matched.size === pairs.length;
    /* ── Handlers ──────────────────────────────────────────────── */
    const handleTermTap = useCallback((index) => {
        if (matched.has(index) || allDone)
            return;
    ...
    const animateMatch = useCallback((termIndex) => {
    ...
    const handleDefTap = useCallback((defIdx) => {
        if (selectedTerm === null || allDone)
            return;
        ...
        <View style={styles.column}>
          ...
          {pairs.map((pair, i) => {
            ...
            const scale = isMatched
                ...
                : 1;
            return (<Animated.View key={`t-${i}`} style={{ transform: [{ scale }] }}>
                <TouchableOpacity onPress={() => handleTermTap(i)} disabled={isMatched} activeOpacity={0.7} style={[
                    styles.card,
                  ...
                  {isMatched && (<View style={styles.checkBadge}>
        ...
        <View style={styles.column}>
          ...
          {shuffledDefs.map((def, i) => {
            ...
            const scale = isMatched
                ...
                : 1;
            return (<Animated.View key={`d-${i}`} style={{ transform: [{ scale }] }}>
                <TouchableOpacity onPress={() => handleDefTap(i)} disabled={isMatched || selectedTerm === null} activeOpacity={0.7} style={[
                    styles.card,
                    isMatched && styles.cardMatched,
...
export function extractTermPairs(slideTitle, slideBody) {
    const pairs = [];
    if (!slideBody || slideBody.length < 60)
        return pairs;
    const seen = new Set();
...
export function shouldShowWordMatch(slideIndex, totalSlides, extractedPairs) {
    if (extractedPairs.length < 3)
        return false;
    if (totalSlides < 6)
        return false;
    if (slideIndex < 2 || slideIndex >= totalSlides - 2)
        return false;
