...
export function LeoInterstitial({ type, progress, slideTitle, customMessage }) {
    ...
    return (<Animated.View style={[styles.container, { opacity: opacityAnim, transform: [{ scale: scaleAnim }] }]}>
      {/* Leo with glow */}
      <View style={styles.leoContainer}>
        <View style={[styles.glow, { backgroundColor: accent + '20' }]}/>
        <Animated.Image source={LEO_IMAGE} style={[styles.leoImage, { transform: [{ translateY: bounceAnim }] }]}/>
      </View>
...
export function shouldShowLeoCard(cardIndex, totalCards) {
    ...
    if ((cardIndex - 3) % 6 === 0 && cardIndex !== midpoint) {
        const types = ['encouragement', 'fun-fact', 'check-in'];
