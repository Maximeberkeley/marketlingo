...
import { FLUID } from "../../lib/fluidType";
...
function getTopicIllustration(text) {
...
function RichText({ text, style }) {
    ...
    while ((match = regex.exec(text)) !== null) {
        if (match.index > lastIndex) {
            parts.push(<Text key={key++} style={style}>
          {text.slice(lastIndex, match.index)}
        </Text>);
        }
        if (match[2]) {
            parts.push(<Text key={key++} style={[style, styles.boldHighlight]}>
          {match[2]}
        </Text>);
        }
        else if (match[3]) {
            parts.push(<Text key={key++} style={[style, { fontStyle: "italic" }]}>
          {match[3]}
        </Text>);
        }
...
function FormattedText({ text, style, accentColor = COLORS.accent, }) {
    if (!text)
        return null;
    ...
    return (<View>
      {paragraphs.map((para, idx) => (<View key={idx} style={idx > 0 ? { marginTop: 14 } : undefined}>
...
// ── Main ConceptCard ────────────────────────────────────────────────
export function ConceptCard({ type, title, content, bullets, sources, keyTerms, cardIndex, totalCards, accentColor = COLORS.accent, }) {
    ...
    if (type === "header") {
        ...
        return (<Animated.View style={[styles.headerCard, { opacity: fadeIn, transform: [{ translateY: slideUp }, { scale }] }]}>
    ...
    if (type === "key-terms" && keyTerms && keyTerms.length > 0) {
        return (<Animated.View style={[styles.keyTermsCard, { opacity: fadeIn, transform: [{ translateY: slideUp }, { scale }] }]}>
        ...
        <ScrollView style={{ maxHeight: MAX_CARD_CONTENT_HEIGHT }} showsVerticalScrollIndicator={false} nestedScrollEnabled contentContainerStyle={{ paddingBottom: 40 }}>
          {keyTerms.map((item, idx) => (<View key={idx} style={styles.termMiniCard}>
    ...
    // ── Key stat card ───────────────────────────────────
    if (type === "key-stat") {
        return (<Animated.View style={[styles.keyStatCard, { opacity: fadeIn, transform: [{ translateY: slideUp }, { scale }] }]}>
    ...
    if (type === "example") {
        return (<Animated.View style={[styles.exampleCard, { opacity: fadeIn, transform: [{ translateY: slideUp }, { scale }] }]}>
    ...
    if (type === "bullet-group") {
        return (<Animated.View style={[styles.card, { opacity: fadeIn, transform: [{ translateY: slideUp }, { scale }] }]}>
        ...
        <ScrollView style={{ maxHeight: MAX_CARD_CONTENT_HEIGHT }} showsVerticalScrollIndicator={false} nestedScrollEnabled>
          <View style={styles.bulletList}>
            {(bullets || []).map((bullet, idx) => (<BulletItem key={idx} text={bullet} index={idx} accentColor={accentColor}/>))}
          </View>
        </ScrollView>
      </Animated.View>);
    }
    // ── Sources card ────────────────────────────────────
    if (type === "sources") {
        return (<Animated.View style={[styles.sourcesCard, { opacity: fadeIn, transform: [{ translateY: slideUp }, { scale }] }]}>
        <View style={styles.sourcesHeader}>
          <Feather name="link" size={14} color={COLORS.textMuted}/>
          <Text style={styles.sourcesLabel}>Sources</Text>
        </View>
        <View style={styles.sourcesRow}>
          {(sources || []).map((source, idx) => (<TouchableOpacity key={idx} style={styles.sourceChip} onPress={() => Linking.openURL(source.url)}>
    ...
    return (<Animated.View style={[styles.card, { opacity: fadeIn, transform: [{ translateY: slideUp }, { scale }] }]}>
      ...
      <ScrollView style={expanded ? { maxHeight: MAX_CARD_CONTENT_HEIGHT } : undefined} showsVerticalScrollIndicator={false} nestedScrollEnabled>
        <FormattedText text={displayContent} style={styles.conceptText} accentColor={accentColor}/>
        {isLongContent && !expanded && (<TouchableOpacity style={styles.readMoreBtn} onPress={() => setExpanded(true)}>
            <Text style={[styles.readMoreText, { color: accentColor }]}>See more</Text>
            <Feather name="chevron-down" size={14} color={accentColor}/>
          </TouchableOpacity>)}
        {isLongContent && expanded && (<TouchableOpacity style={styles.readMoreBtn} onPress={() => setExpanded(false)}>
...
function BulletItem({ text, index, accentColor }) {
    const fadeIn = useRef(new Animated.Value(0)).current;
    ...
    return (<Animated.View style={[styles.bulletRow, { opacity: fadeIn, transform: [{ translateX: slideX }] }]}>
...
function classifyContent(text) {
    ...
    if (statPatterns.test(text) && text.length < 250)
        return "stat";
...
function breakIntoStorySequence(text, maxChars) {
...
export function parseSlideIntoCards(slideTitle, body, sources, _slideIndex, marketId) {
    const cards = [];
    ...
    const splitAtSentence = (text, maxLen) => {
        ...
        if (lastGoodBreak === -1) {
            ...
            if (lastSpace > maxLen * 0.3)
                lastGoodBreak = lastSpace + 1;
            else
                lastGoodBreak = maxLen;
        }
    ...
    const autoSplitLongContent = (text, titlePrefix) => {
        ...
        const final = chunks.length > 5
            ? (() => { const m = []; const p = Math.ceil(chunks.length / 5); for (let i = 0; i < chunks.length; i += p)
                m.push(chunks.slice(i, i + p).join(" ")); return m; })()
            : chunks;
            ...
            for (const chunk of storyChunks) {
                ...
                while (remaining.length > 0) {
                    ...
                    for (const split of splits) {
                        ...
                        const isCallout = (lower.includes("key takeaway") ||
