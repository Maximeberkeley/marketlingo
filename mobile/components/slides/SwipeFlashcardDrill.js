...
export function generateFlashcardsFromSlides(slides) {
    const cards = [];
...
function createFalseStatement(sentence) {
    ...
    if (/\b(is|are|was|were|has|have|had|does|do|did|can|will|would|should)\b/i.test(sentence)) {
        return sentence.replace(/\b(is|are|was|were|has|have|had|does|do|did|can|will|would|should)\b/i, '$1 not');
    }
    return null;
}
export function SwipeFlashcardDrill({ cards, onComplete, accentColor = COLORS.accent }) {
    ...
    if (isComplete) {
        ...
        return (<View style={styles.completeContainer}>
