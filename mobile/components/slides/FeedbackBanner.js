...
export function FeedbackBanner({ visible, isCorrect, message, explanation, xpEarned, comboMultiplier, onContinue, }) {
    ...
    return (<Animated.View style={[
      ...
      <View style={styles.content}>
        ...
        <View style={styles.messageRow}>
          <Feather name={isCorrect ? 'check-circle' : 'x-circle'} size={24} color={colors.text}/>
          <Text style={[styles.message, { color: colors.text }]}>{message}</Text>
        </View>

        {/* Explanation */}
        {explanation && (<Text style={[styles.explanation, { color: colors.text }]}>{explanation}</Text>)}

        {/* XP earned row */}
        {isCorrect && xpEarned && xpEarned > 0 && (<View style={styles.xpRow}>
      ...
      {/* Continue CTA */}
      <TouchableOpacity style={[styles.continueBtn, { backgroundColor: colors.btnBg }]} onPress={onContinue} activeOpacity={0.85}>
