...
import { View, Text, TouchableOpacity, ScrollView, StyleSheet, Dimensions, Animated, Modal, Linking, Image, } from 'react-native';
...
// ─────────────────────────────────────────────
function MentorCard({ position, marketId, slideIndex, totalSlides, onChat, }) {
    ...
    return (<Animated.View style={[mcStyles.card, { borderColor: accent + '40', backgroundColor: accent + '10', opacity, transform: [{ translateY }] }]}>
      ...
      <View style={mcStyles.textBlock}>
        ...
        {position === 'last' && (<TouchableOpacity onPress={onChat} style={[mcStyles.chatBtn, { backgroundColor: accent + '25', borderColor: accent + '60' }]}>
...
function TipBubble({ tip, marketId, onDismiss }) {
    const opacity = useRef(new Animated.Value(0)).current;
    ...
    return (<Animated.View style={[tipStyles.bubble, { opacity, transform: [{ translateY }] }]}>
...
function renderBody(body) {
    ...
    return paragraphs.map((paragraph, idx) => {
        ...
        if (isBullet) {
            return (<View key={idx} style={bodyStyles.bulletRow}>
...
// Completion modal
// ─────────────────────────────────────────────
function CompletionModal({ isReview, hasMetMinimumTime, remainingMinutes, remainingSecondsDisplay, marketId, onComplete, onClose, timeSpentSeconds, }) {
    ...
    return (<Animated.View style={[cStyles.overlay]}>
      <Animated.View style={[cStyles.card, { transform: [{ scale }], opacity }]}>
        {/* Mentor avatar — pops up from top like web */}
        <Animated.Image source={avatarSrc} style={[cStyles.mentorAvatar, { transform: [{ translateY: avatarY }] }]}/>
        ...
        <View style={cStyles.btnGroup}>
          {!isReview && !hasMetMinimumTime ? (<TouchableOpacity style={cStyles.outlineBtn} onPress={onClose}>
...
// Main SlideReader
// ─────────────────────────────────────────────
export function SlideReader({ stackTitle, stackType, slides, onClose, onComplete, onSaveInsight, onAddNote, isReview = false, marketId, isProUser = true, onAskMentor, mentorName, }) {
    const [currentIndex, setCurrentIndex] = useState(-1);
    const [showCompletion, setShowCompletion] = useState(false);
    const [activeTip, setActiveTip] = useState(null);
    const [shownTipIndices, setShownTipIndices] = useState(new Set());
    ...
    const mentorPosition = currentIndex === 0 ? 'first'
    ...
    return (<Modal visible animationType="slide" presentationStyle="fullScreen">
      <View style={styles.container}>
        ...
        <View style={styles.topBar}>
          ...
          {!isReview && stackType === 'LESSON' && !hasMetMinimumTime && !isIntroSlide && (<View style={styles.timerPill}>
              <Text style={styles.timerText}>⏱ {remainingMinutes}:{remainingSecondsDisplay.toString().padStart(2, '0')}</Text>
            </View>)}
          {!isIntroSlide && (isReview || stackType !== 'LESSON' || hasMetMinimumTime) && (<Text style={styles.slideCounter}>{currentIndex + 1}/{slides.length}</Text>)}
        </View>
        ...
        <View style={styles.progressRow}>
          ...
          {slides.map((_, idx) => (<View key={idx} style={[
        ...
        <SlideIntroCard stackTitle={stackTitle} stackType={stackType} totalSlides={slides.length} marketId={marketId}/>) : (<Animated.View style={{ transform: [{ translateX: slideAnim }] }}>
              {/* Mentor card at key positions */}
              {mentorPosition && (<MentorCard position={mentorPosition} marketId={marketId} slideIndex={currentIndex} totalSlides={totalSlides} onChat={() => { }}/>)}

              {/* Proactive tip bubble */}
              {activeTip && (<TipBubble tip={activeTip} marketId={marketId} onDismiss={() => setActiveTip(null)}/>)}
              ...
              <View style={styles.slideCard}>
                ...
                {/* Sources */}
                {currentSlide?.sources && currentSlide.sources.length > 0 && (<View style={styles.sourcesContainer}>
                    <Text style={styles.sourcesLabel}>Sources</Text>
                    <View style={styles.sourceChips}>
                      {currentSlide.sources.map((source, idx) => (<TouchableOpacity key={idx} onPress={() => Linking.openURL(source.url)} style={styles.sourceChip}>
        ...
        {/* ── NAVIGATION ARROWS ── */}
        {!isIntroSlide && (<View style={styles.navRow}>
        ...
        <View style={styles.bottomBar}>
          {/* Progress dots */}
          {!isIntroSlide && (<View style={styles.progressDots}>
          ...
          {isIntroSlide ? (<TouchableOpacity style={styles.ctaBtn} onPress={goToNext}>
              <Text style={styles.ctaBtnText}>Begin →</Text>
            </TouchableOpacity>) : isLastSlide ? (<View style={{ gap: 8 }}>
              <View style={styles.actionRow}>
                ...
                {onAskMentor && (<TouchableOpacity style={styles.mentorCTA} onPress={onAskMentor}>
        ...
        <Modal visible={showCompletion} transparent animationType="fade">
          <CompletionModal isReview={isReview} hasMetMinimumTime={hasMetMinimumTime} remainingMinutes={remainingMinutes} remainingSecondsDisplay={remainingSecondsDisplay} marketId={marketId} onComplete={() => { setShowCompletion(false); onComplete(isReview, timeSpentSeconds); }} onClose={() => setShowCompletion(false)} timeSpentSeconds={timeSpentSeconds}/>
        </Modal>
      </View>
    </Modal>);
}
