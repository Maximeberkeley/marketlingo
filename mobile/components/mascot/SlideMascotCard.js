...
const getLeo = () => ({ id: 'leo', name: 'Leo', isLeo: true });
const getMentorCharacter = (marketId) => {
...
const getMessages = (position, charName, slideIndex, totalSlides) => {
...
export function SlideMascotCard({ position, slideIndex, totalSlides, marketId }) {
    ...
    const messages = useMemo(() => getMessages(position, character.name, slideIndex, totalSlides), [position, character.name, slideIndex, totalSlides]);
    useEffect(() => {
        Animated.parallel([
    ...
    return (<Animated.View style={[
      ...
      <Animated.Image source={avatarSource} style={[
            styles.avatar,
...
export function getSlidePosition(slideIndex, totalSlides) {
    if (slideIndex === 0)
        return 'first';
