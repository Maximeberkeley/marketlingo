...
// LeoQuiz Component
// ─────────────────────────────────────────────
export function LeoQuiz({ question, options, onComplete, onDismiss }) {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.options}>
        {options.map((option, index) => {
            ...
            return (<TouchableOpacity key={index} onPress={() => handleSelect(index)} disabled={showResult} style={[styles.option, { backgroundColor: bg, borderColor: border }]} activeOpacity={0.75}>
              {showResult && isSelected && (<Text style={styles.resultIcon}>{isCorrect ? '✅' : '❌'}</Text>)}
      ...
      {/* Skip */}
      {onDismiss && !showResult && (<TouchableOpacity onPress={onDismiss} style={styles.skipBtn}>
...
export function useLeoQuiz() {
    ...
    const triggerQuiz = (question, options) => {
