...
export const getRandomCharacter = (marketId?: string): MascotCharacter => {
  ...
  const weighted = [...characters, characters[0], characters[0]]; // Leo weighted higher
...
const getDefaultMessage = (
  ...
  switch (type) {
    case 'intro':
      return isLeo ? "Let's dive in together! 🚀" : "I'll guide you through this one. Ready?";
    case 'midpoint': {
      const progress =
    ...
    case 'complete':
      return isLeo ? 'You crushed it! 🎉' : 'Amazing work! You should be proud.';
    case 'encourage':
      return isLeo ? "You've got this! Don't give up!" : "Keep pushing—you're learning!";
    case 'tip':
      return isLeo ? 'Pro tip incoming! 💡' : "Here's something to remember...";
    default:
      return 'Hey there! 👋';
  }
};
...
const BG_TINTS: Record<MascotBreakType, string> = {
  ...
  midpoint: 'rgba(245,158,11,0.12)',
  complete: 'rgba(34,197,94,0.12)',
  encourage: 'rgba(59,130,246,0.12)',
  tip: 'rgba(168,85,247,0.12)',
};

// ─────────────────────────────────────────────
// MascotBreak Component
// ─────────────────────────────────────────────
...
export function MascotBreak({
  ...
  return (
    <Animated.View
      style={[
        styles.container,
        { backgroundColor: BG_TINTS[type], opacity, transform: [{ scale }] },
      ]}
    >
      {/* Character */}
      <TouchableOpacity activeOpacity={onDismiss ? 0.8 : 1} onPress={onDismiss ? () => { setDismissed(true); onDismiss(); } : undefined}>
        <Animated.View style={{ transform: [{ translateY: bobY }] }}>
          <Image
            source={avatarSource}
            style={[styles.avatar, isLeo ? styles.leoAvatar : styles.mentorAvatar]}
          />
        </Animated.View>
...
// ─────────────────────────────────────────────
// Utility: when to show a mascot break
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// InlineMascot - small corner version
// ─────────────────────────────────────────────
