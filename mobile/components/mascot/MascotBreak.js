...
export const getRandomCharacter = (marketId) => {
    ...
    if (marketId === 'neuroscience') {
        ...
        if (sophia)
            weighted.push(sophia, sophia);
    }
    return weighted[Math.floor(Math.random() * weighted.length)];
};
const getDefaultMessage = (type, charName, slideIndex, totalSlides) => {
...
export function MascotBreak({ type, message, characterId, marketId, slideIndex, totalSlides, onDismiss, autoDismissMs }) {
    ...
    return (<Animated.View style={[styles.container, { backgroundColor: BG_TINTS[type], opacity, transform: [{ scale }] }]}>
...
export function shouldShowMascotBreak(slideIndex, totalSlides, isIntro) {
    if (isIntro)
        return 'intro';
    const midpoint = Math.floor(totalSlides / 2);
    if (slideIndex === midpoint && totalSlides > 3)
        return 'midpoint';
    if (slideIndex === totalSlides - 1)
        return 'complete';
    return null;
}
export function InlineMascot({ characterId, marketId, message, position = 'left', size = 'sm' }) {
    const character = useMemo(() => {
        if (characterId) {
            const found = getAllCharacters().find((c) => c.id === characterId);
            if (found)
    ...
    return (<View style={[styles.inlineRow, position === 'right' && styles.inlineRowReverse]}>
