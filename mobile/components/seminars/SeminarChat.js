...
import { useSeminarChat } from '../../hooks/useSeminars';
export function SeminarChat({ seminarId }) {
    ...
    const threaded = useMemo(() => {
        ...
        const sorted = [...tops].sort((a, b) => {
            if (sort === 'top')
                return (b.like_count - a.like_count) || (new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    ...
    const renderBubble = (item, isReply = false) => {
        const isOwn = item.user_id === currentUserId;
        return (<View key={item.id} style={[styles.messageBubble, item.is_pinned && styles.pinnedBubble, isReply && styles.replyBubble]}>
        {item.is_pinned && (<View style={styles.pinnedBadge}>
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity onPress={() => handleLike(item)} style={styles.actionBtn} hitSlop={8}>
            <Feather name="heart" size={14} color={item.liked_by_me ? '#EF4444' : COLORS.textMuted} {...(item.liked_by_me && { style: { opacity: 1 } })}/>
            <Text style={[styles.actionText, item.liked_by_me && { color: '#EF4444', fontWeight: '700' }]}>
              {item.like_count > 0 ? item.like_count : 'Like'}
            </Text>
          </TouchableOpacity>

          {!isReply && (<TouchableOpacity onPress={() => handleReply(item)} style={styles.actionBtn} hitSlop={8}>
              <Feather name="corner-up-left" size={13} color={COLORS.textMuted}/>
              <Text style={styles.actionText}>Reply</Text>
            </TouchableOpacity>)}

          {isOwn && (<TouchableOpacity onPress={() => handleDelete(item)} style={styles.actionBtn} hitSlop={8}>
    ...
    const renderThread = ({ item }) => {
        ...
        {item.replies.length > 0 && (<TouchableOpacity onPress={() => toggleThread(item.top.id)} style={styles.threadToggle}>
        ...
        {expanded && (<View style={styles.repliesContainer}>
            {item.replies.map(r => renderBubble(r, true))}
          </View>)}
      </View>);
    };
    return (<KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={120}>
      <View style={styles.chatHeader}>
        ...
        <TouchableOpacity onPress={() => setSort(s => s === 'newest' ? 'top' : 'newest')} style={styles.sortBtn} hitSlop={8}>
          <Feather name={sort === 'top' ? 'trending-up' : 'clock'} size={12} color="#4338CA"/>
          <Text style={styles.sortText}>{sort === 'top' ? 'Top' : 'Newest'}</Text>
        </TouchableOpacity>
      </View>

      <FlatList ref={flatListRef} data={threaded} renderItem={renderThread} keyExtractor={item => item.top.id} style={styles.messageList} contentContainerStyle={styles.messageListContent} ListEmptyComponent={<View style={styles.emptyState}>
            <Feather name="message-square" size={32} color={COLORS.textMuted}/>
            <Text style={styles.emptyText}>No messages yet. Start the conversation!</Text>
          </View>}/>

      {replyTo && (<View style={styles.replyingBanner}>
      ...
      <View style={styles.inputRow}>
        <TextInput ref={inputRef} style={styles.input} value={text} onChangeText={setText} placeholder={replyTo ? `Reply to ${replyTo.username}...` : 'Type a message...'} placeholderTextColor={COLORS.textMuted} multiline maxLength={500} returnKeyType="send" onSubmitEditing={handleSend} blurOnSubmit/>
...
const styles = StyleSheet.create({
    ...
    replyBubble: { backgroundColor: COLORS.bg2 },
    ...
    inputRow: { flexDirection: 'row', alignItems: 'flex-end', gap: 8, padding: 12, borderTopWidth: 1, borderTopColor: COLORS.border, backgroundColor: COLORS.bg2 },
