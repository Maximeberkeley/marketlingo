...
const STATUS_CONFIG = {
    upcoming: { label: 'Upcoming', colors: ['#4338CA', '#6366F1'], icon: 'clock' },
    live: { label: 'LIVE', colors: ['#DC2626', '#EF4444'], icon: 'radio' },
    completed: { label: 'Replay', colors: ['#059669', '#10B981'], icon: 'play-circle' },
};
export function SeminarCard({ seminar, onPress, prepProgress }) {
    ...
    return (<TouchableOpacity onPress={onPress} activeOpacity={0.85} style={styles.card}>
      <LinearGradient colors={['#1E1B4B', '#312E81', '#4338CA']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.gradient}>
        ...
        <View style={styles.header}>
          ...
          {seminar.status !== 'completed' && (<Text style={styles.countdown}>{countdown}</Text>)}
        </View>

        {/* Content */}
        <Text style={styles.title} numberOfLines={2}>{seminar.title}</Text>
        
        {seminar.speaker_name && (<View style={styles.speakerRow}>
            <View style={styles.speakerAvatar}>
              <Feather name="user" size={12} color="#A5B4FC"/>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.speakerName}>{seminar.speaker_name}</Text>
              {seminar.speaker_title && (<Text style={styles.speakerTitle} numberOfLines={1}>{seminar.speaker_title}</Text>)}
            </View>
          </View>)}

        {/* Prep progress */}
        {prepProgress !== undefined && prepProgress > 0 && (<View style={styles.prepRow}>
        ...
        <View style={styles.footer}>
          ...
          {seminar.is_pro_only && (<View style={styles.proBadge}>
