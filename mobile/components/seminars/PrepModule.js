...
export function PrepModule({ module, index, isCompleted, onComplete }) {
    ...
    return (<View style={[styles.container, isCompleted && styles.completedContainer]}>
      <TouchableOpacity onPress={toggleExpanded} style={styles.header} activeOpacity={0.7}>
        <View style={[styles.numberBadge, isCompleted && styles.completedBadge]}>
          {isCompleted ? (<Feather name="check" size={14} color="#FFF"/>) : (<Text style={styles.numberText}>{index + 1}</Text>)}
        </View>
        <View style={{ flex: 1 }}>
      ...
      {expanded && (<View style={styles.body}>
          {module.module_type === 'concept_quiz' && (<ConceptQuizContent module={module} isCompleted={isCompleted} onComplete={handleComplete}/>)}
          {module.module_type === 'flashcards' && (<FlashcardsContent module={module} isCompleted={isCompleted} onComplete={handleComplete}/>)}
          {module.module_type === 'scenario' && (<ScenarioContent module={module} isCompleted={isCompleted} onComplete={handleComplete}/>)}
          {module.module_type === 'reflection' && (<ReflectionContent module={module} isCompleted={isCompleted} onComplete={handleComplete}/>)}
        </View>)}
    </View>);
}
// ---------- CONCEPT QUIZ ----------
function ConceptQuizContent({ module, isCompleted, onComplete }) {
    ...
    return (<>
      {module.content && (<ScrollView style={styles.contentScroll} nestedScrollEnabled>
          <Text style={styles.content}>{module.content.replace(/##\s/g, '').replace(/###\s/g, '').replace(/\*\*/g, '')}</Text>
        </ScrollView>)}

      {module.key_takeaways.length > 0 && (<View style={styles.takeawaysBox}>
          <Text style={styles.takeawaysTitle}>Key Takeaways</Text>
          {module.key_takeaways.map((t, i) => (<View key={i} style={styles.takeawayItem}>
              <View style={styles.bulletDot}/>
              <Text style={styles.takeawayText}>{t}</Text>
            </View>))}
        </View>)}

      {module.quiz_question && options.length > 0 && (<View style={styles.quizSection}>
          ...
          {options.map((opt, idx) => {
                ...
                return (<TouchableOpacity key={idx} onPress={() => handleAnswer(idx)} disabled={answered} style={[styles.option, showCorrect && styles.correctOption, showWrong && styles.wrongOption]} activeOpacity={0.7}>
          ...
          {answered && (<View style={[styles.feedbackBanner, isCorrect ? styles.correctBanner : styles.wrongBanner]}>
...
// ---------- FLASHCARDS ----------
function FlashcardsContent({ module, isCompleted, onComplete }) {
    ...
    return (<View style={{ gap: 12 }}>
      ...
      <TouchableOpacity onPress={() => { Haptics.selectionAsync().catch(() => { }); setFlipped(f => !f); }} activeOpacity={0.85}>
        <LinearGradient colors={flipped ? ['#0E7490', '#06B6D4'] : ['#312E81', '#4338CA']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.flashcard}>
...
// ---------- SCENARIO ----------
function ScenarioContent({ module, isCompleted, onComplete }) {
    const [selectedAnswer, setSelectedAnswer] = useState(null);
    const [answered, setAnswered] = useState(isCompleted);
    const options = module.quiz_options || [];
    const handleAnswer = (idx) => {
    ...
    const isCorrect = selectedAnswer === module.correct_index;
    return (<>
      {module.scenario_brief && (<View style={styles.scenarioBox}>
      ...
      {module.quiz_question && (<Text style={styles.quizQuestion}>{module.quiz_question}</Text>)}
      {options.map((opt, idx) => {
            const isSelected = selectedAnswer === idx;
            const showCorrect = answered && idx === module.correct_index;
            const showWrong = answered && isSelected && !isCorrect;
      ...
      {answered && module.content && (<View style={styles.explanationBox}>
...
// ---------- REFLECTION ----------
function ReflectionContent({ module, isCompleted, onComplete }) {
    const [text, setText] = useState('');
    const minChars = 40;
    const ready = text.trim().length >= minChars;
    return (<View style={{ gap: 12 }}>
      {module.reflection_prompt && (<View style={styles.reflectionBox}>
          <Feather name="edit-3" size={14} color="#B45309"/>
          <Text style={styles.reflectionPrompt}>{module.reflection_prompt}</Text>
        </View>)}
      <TextInput style={styles.reflectionInput} value={text} onChangeText={setText} placeholder={`Write at least ${minChars} characters...`} placeholderTextColor={COLORS.textMuted} multiline maxLength={1500} editable={!isCompleted}/>
      <View style={styles.reflectionFooter}>
        <Text style={styles.charCount}>{text.trim().length}/{minChars}+</Text>
        <TouchableOpacity onPress={onComplete} disabled={!ready || isCompleted} style={[styles.primaryBtn, (!ready || isCompleted) && styles.primaryBtnDisabled]} activeOpacity={0.85}>
