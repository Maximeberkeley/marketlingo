...
export function SectionIntroSlide({ type, dayNumber, monthTheme }) {
    const info = sectionInfo[type];
    return (<View style={[styles.container, { backgroundColor: info.bgColor }]}>
      {/* Day/Month badges */}
      {(dayNumber || monthTheme) && (<View style={styles.badgesRow}>
          {dayNumber && (<View style={styles.dayBadge}>
              <Text style={styles.dayBadgeText}>Day {dayNumber}</Text>
            </View>)}
          {monthTheme && (<View style={[styles.themeBadge, { backgroundColor: 'rgba(139,92,246,0.2)' }]}>
...
export function createIntroSlide(stackType, stackTitle) {
    let sectionType = 'micro_lesson';
