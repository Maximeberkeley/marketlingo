...
const BITE_ICONS = ['book-open', 'eye', 'award'];
const BITE_COLORS = ['#3B82F6', '#8B5CF6', '#22C55E'];
export function QuickBiteSelector({ totalSlides, completedBites, onSelectBite, onFullLesson, lessonTitle, isLessonComplete, }) {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.bitesRow}>
        {bites.map((bite) => {
            ...
            return (<TouchableOpacity key={bite.index} style={[styles.biteCard, bite.isComplete && styles.biteCardComplete]} onPress={() => onSelectBite(bite.index)} activeOpacity={0.7}>
              <View style={[styles.biteIconWrap, { backgroundColor: bite.isComplete ? COLORS.successSoft : color + '14' }]}>
                <Feather name={bite.isComplete ? 'check' : icon} size={18} color={bite.isComplete ? COLORS.success : color}/>
              </View>
      ...
      <TouchableOpacity style={[styles.fullLessonBtn, isLessonComplete && styles.fullLessonBtnDone]} onPress={onFullLesson} activeOpacity={0.8}>
