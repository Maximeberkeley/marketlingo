...
export function StreakAtRisk({ streak, hoursLeft, onStartLesson, onDismiss }) {
    ...
    const bgColor = urgencyLevel === 'critical' ? COLORS.errorSoft
        : urgencyLevel === 'warning' ? COLORS.orangeSoft
            : COLORS.goldSoft;
    const borderColor = urgencyLevel === 'critical' ? 'rgba(239, 68, 68, 0.25)'
        : urgencyLevel === 'warning' ? 'rgba(251, 146, 60, 0.25)'
            : 'rgba(251, 191, 36, 0.2)';
    const textColor = urgencyLevel === 'critical' ? COLORS.error : urgencyLevel === 'warning' ? COLORS.orange : COLORS.warning;
    ...
    return (<Animated.View style={[
      ...
      <TouchableOpacity style={[
        ...
        ]} onPress={onStartLesson} activeOpacity={0.8}>
...
export function getStreakRiskHours(streakExpiresAt, currentStreak, lessonCompletedToday) {
    if (!streakExpiresAt || currentStreak === 0 || lessonCompletedToday)
        return null;
