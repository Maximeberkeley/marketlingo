...
export function SundayRecapCard({ recap, accent, tierLabel, rank, onDismiss, onOpenLeague }) {
    const max = Math.max(1, ...recap.perDay.map((d) => d.xp));
    const delta = recap.xpThisWeek - recap.xpLastWeek;
    const deltaText = recap.xpLastWeek === 0
    ...
    return (<View style={styles.card}>
      ...
      <View style={styles.chart}>
        {recap.perDay.map((d) => (<View key={d.label} style={styles.barCol}>
      ...
      {recap.bestDay && (<Text style={styles.best}>
