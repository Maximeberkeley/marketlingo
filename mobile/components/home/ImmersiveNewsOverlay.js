...
import { View, Text, StyleSheet, Modal, Image, TouchableOpacity, Animated, PanResponder, Dimensions, ActivityIndicator, Linking, Platform, StatusBar, } from 'react-native';
...
// ── Helpers ──
async function generateNarration(article, marketId, goal) {
...
// ── Component ──
export function ImmersiveNewsOverlay({ visible, articles, initialIndex, onClose, onOpenChat, onQuiz, onArticleView, marketId, learningGoal = 'curiosity', }) {
    ...
    useEffect(() => {
        if (visible && article && onArticleView)
...
User's goal: ${learningGoal}`;
    ...
    return (<Modal visible={visible} animationType="none" transparent statusBarTranslucent>
      ...
      <Animated.View style={[st.container, { opacity: fadeAnim, transform: [{ translateY }] }]} {...panResponder.panHandlers}>
        {/* Background image */}
        <Animated.View style={[st.bgContainer, { transform: [{ translateX }] }]}>
          {article.imageUrl ? (<Image source={{ uri: article.imageUrl }} style={st.bgImage} blurRadius={1}/>) : (<View style={[st.bgImage, { backgroundColor: '#1A1035' }]}/>)}
        ...
        <View style={st.topBar}>
          ...
          <View style={st.topBarRight}>
            {onQuiz ? (<TouchableOpacity style={st.quizButton} onPress={() => onQuiz(article)} accessibilityRole="button" accessibilityLabel="Take quiz about this story">
                <Feather name="help-circle" size={17} color={COLORS.textOnAccent}/>
                <Text style={st.quizButtonText}>Quiz</Text>
              </TouchableOpacity>) : null}
            {/* Discuss with AI */}
            <TouchableOpacity style={st.topBtn} onPress={() => { onClose(); setTimeout(() => onOpenChat(article), 300); }}>
        ...
        <View style={st.centerStage}>
          <Animated.View style={[st.sophiaRing, {
            ...
            <TouchableOpacity onPress={handleSophiaTap} activeOpacity={0.8}>
              ...
              {isRecording && (<View style={st.micBadge}>
          ...
          <View style={st.statusContainer}>
            {isGenerating && (<View style={st.statusRow}>
                <ActivityIndicator size="small" color={COLORS.accent}/>
                <Text style={st.statusText}>Sophia is thinking...</Text>
              </View>)}
            {isTranscribing && (<View style={st.statusRow}>
                <ActivityIndicator size="small" color={COLORS.accent}/>
            ...
            {isSpeaking && !isGenerating && (<Text style={st.statusText}>🎙 Sophia is speaking...</Text>)}
            {isDoneSpeaking && !isGenerating && !isRecording && !isTranscribing && (<Text style={st.statusText}>Tap Sophia to ask a question</Text>)}
          </View>

          {/* Narration text (subtitle-style, tap to expand) */}
          {narrationText && !isGenerating && (<TouchableOpacity style={st.subtitleBox} onPress={() => setSubtitlesExpanded(!subtitlesExpanded)} activeOpacity={0.8}>
              <Text style={st.subtitleText} numberOfLines={subtitlesExpanded ? undefined : 2}>
                {narrationText}
              </Text>
              {!subtitlesExpanded && narrationText.length > 80 && (<Text style={st.expandHint}>Tap to read more</Text>)}
            </TouchableOpacity>)}
        </View>
        ...
        <View style={st.bottomBar}>
          <TouchableOpacity style={st.sourceBtn} onPress={() => Linking.openURL(article.sourceUrl).catch(() => { })}>
            <Feather name="external-link" size={16} color="#fff"/>
            <Text style={st.sourceBtnText}>Read Full Article</Text>
          </TouchableOpacity>

          {/* Swipe hints */}
          {isDoneSpeaking && hasNext && (<Text style={st.swipeHint}>Swipe left for next article →</Text>)}
        </View>

        {/* Navigation dots */}
        <View style={st.dotsRow}>
          {articles.map((_, i) => (<View key={i} style={[st.dot, i === currentIndex && st.dotActive]}/>))}
        </View>
