...
function QuestRow({ quest, index, onStartLesson, }) {
    ...
    return (<Animated.View style={[{ transform: [{ translateX: slideAnim }], opacity: opacityAnim }]}>
      <TouchableOpacity style={styles.questRow} onPress={handlePress} activeOpacity={0.7} disabled={quest.isCompleted}>
        {/* Icon circle */}
        <View style={[styles.questIcon, quest.isCompleted && styles.questIconDone]}>
          {quest.isCompleted ? (<Animated.View style={{ transform: [{ scale: checkScale }] }}>
              <Feather name="check" size={14} color="#fff"/>
            </Animated.View>) : (<Feather name={iconName} size={14} color={COLORS.accent}/>)}
        </View>
...
export function DailyQuests({ quests, completedCount, totalBonusXP, allComplete, onStartLesson }) {
    return (<View style={styles.container}>
      ...
      {allComplete && (<View style={styles.allCompleteBanner}>
          <Feather name="award" size={16} color={COLORS.success}/>
          <Text style={styles.allCompleteText}>All done! +{totalBonusXP} XP</Text>
        </View>)}

      {/* Quest rows */}
      {quests.map((quest, idx) => (<QuestRow key={quest.id} quest={quest} index={idx} onStartLesson={onStartLesson}/>))}
    </View>);
}
