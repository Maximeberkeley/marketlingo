...
export function QuickActionsGrid({ actions }) {
    ...
    useEffect(() => {
        const animations = anims.map((anim, i) => Animated.sequence([
            Animated.delay(i * 80),
            Animated.parallel([
    ...
    return (<View style={styles.grid}>
      {actions.map((action, i) => (<Animated.View key={action.label} style={[styles.itemWrap, { opacity: anims[i].opacity, transform: [{ scale: anims[i].scale }] }]}>
          <TouchableOpacity style={styles.item} onPress={action.onPress} activeOpacity={0.7}>
            {action.icon ? (<View style={styles.iconCircle}>
                <Image source={action.icon} style={styles.iconImage}/>
              </View>) : (<View style={styles.iconCircle}/>)}
