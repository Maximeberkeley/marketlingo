...
const compact = (text, index) => {
    const clean = text.replace(/^[\s•\-–—]+/, '').replace(/\s+/g, ' ').trim();
    if (/responsib|accountab|owns?\b/i.test(clean))
        return 'Trace who owns the outcome';
    if (/distinguish|compare|difference|versus|\bvs\.?\b/i.test(clean))
        return 'Separate the key players';
    if (/decision|purchas|buyer|contact/i.test(clean))
        return 'Find the real decision-maker';
    if (/risk|failure|threat/i.test(clean))
        return 'Spot the hidden risk';
    if (/metric|number|margin|cost|revenue/i.test(clean))
        return 'Read the decisive number';
    if (/mechanism|process|works?|flow/i.test(clean))
        return 'Map how the system works';
...
export function LessonGoalsScreen({ title, slides, objectives, marketId, day = 1, isBite, onStart, onBack }) {
