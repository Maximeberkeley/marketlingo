...
function normalizeQuizQuestions(payload) {
  const questionList = Array.isArray(payload) ? payload : payload && typeof payload === "object" && Array.isArray(payload.questions) ? payload.questions : null;
  if (!questionList) return null;
  const normalized = questionList.map((item) => {
    if (!item || typeof item !== "object") return null;
    const candidate = item;
    const question = typeof candidate.question === "string" ? candidate.question.trim() : "";
    const options = Array.isArray(candidate.options) ? candidate.options.filter((option) => typeof option === "string" && option.trim().length > 0).slice(0, 4) : [];
    const explanation = typeof candidate.explanation === "string" && candidate.explanation.trim().length > 0 ? candidate.explanation.trim() : "Review the article again to reinforce the main takeaway.";
    const rawCorrectIndex = typeof candidate.correctIndex === "number" ? candidate.correctIndex : Number(candidate.correctIndex);
...
function extractQuizResponseText(data) {
  const rawContent = data?.message || data?.reply || data?.response || data?.content || data?.choices?.[0]?.message?.content || "";
...
function getImpactFromContent(title, summary) {
  ...
  if (highSignals.some((s2) => text.includes(s2))) return "high";
  if (mediumSignals.some((s2) => text.includes(s2))) return "medium";
  return "low";
}
...
No other text, just the JSON array.`
  ...
  >
            {loading ? <View style={qz.loadingBox}>
                ...
                <Text style={qz.completeEmoji}>{score === questions.length ? "\u{1F389}" : score > 0 ? "\u{1F44F}" : "\u{1F4D6}"}</Text>
    ...
    return <TouchableOpacity
      ...
      onPress={() => {
        ...
        setSelectedAnswer(i);
        if (i === q.correctIndex) setScore((s2) => s2 + 1);
    ...
    >
      ...
      showResult && isSelected && !isCorrect && { color: "#EF4444" }
                ...
                {selectedAnswer !== null && <TouchableOpacity
    style={qz.nextBtn}
    onPress={() => {
      setSelectedAnswer(null);
      setCurrentQ((c) => c + 1);
...
function ArticleDetailSheet({
  ...
  return <Modal visible={!!article} transparent animationType="none" onRequestClose={handleClose}>
      <View style={ds.backdrop}>
        ...
        <Animated.View style={[ds.sheet, { transform: [{ translateY }] }]}>
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : void 0} style={{ flex: 1 }}>
    ...
    if (article.sourceUrl) Linking.openURL(article.sourceUrl).catch(() => {
    });
  }} style={ds.sourceLink}>
  ...
  ].map(({ mode, icon, label }) => <TouchableOpacity
    key={mode}
    onPress={() => {
      resetAi();
      handleAiAction(mode);
...
function DailyNews({ marketId, learningGoal, autoOpen = false }) {
  ...
  const mapDbItem = (item) => {
    ...
    const publishedAt = publishedRaw && !isNaN(publishedRaw.getTime()) ? publishedRaw.toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "Recent";
    return {
  ...
  const fetchNews = async (forceRefresh = false) => {
    ...
    try {
      if (!forceRefresh) {
        const { data, error: dbError } = await supabase.from("news_items").select("*").eq("market_id", marketId).order("published_at", { ascending: false }).limit(15);
        if (!dbError && data && data.length > 0) {
          ...
          if (hasAnyMissingImages) {
            supabase.functions.invoke("fetch-market-news", { body: { marketId } }).then(async () => {
              const { data: freshData } = await supabase.from("news_items").select("*").eq("market_id", marketId).order("published_at", { ascending: false }).limit(15);
      ...
      if (!fnError && liveData?.success && liveData.data?.length > 0) {
        setNews(liveData.data.map((item) => ({
          ...
          summary: item?.summary || void 0,
          imageUrl: item?.imageUrl || item?.image || void 0,
          impact: item?.impact || getImpactFromContent(item?.title || "", item?.summary)
        })));
        setLastFetched(/* @__PURE__ */ new Date());
      } else {
        const { data: fallback } = await supabase.from("news_items").select("*").eq("market_id", marketId).order("published_at", { ascending: false }).limit(15);
...
Help them understand the key implications.`,
  ...
  const selectStory = (item) => openStory(news, news.findIndex((n) => n.id === item.id));
  return <View style={s.container}>
    ...
    /* Featured horizontal carousel */
          ...
          {featured.length > 0 && <FeaturedCarousel items={featured} onSelect={selectStory} />}
    ...
    /* Vertical feed */
          ...
          <View style={s.feedList}>
            {feed.slice(0, 2).map((item, index) => <NewsFeedCard
    key={item.id}
    item={item}
    index={index}
    onSelect={selectStory}
  />)}
            {midStoryIds.size >= 2 ? <View style={s.midFeedCarousel}>
                <FeaturedCarousel items={midStories} onSelect={selectStory} />
              </View> : null}
            {remainingFeed.map((item, index) => <NewsFeedCard
    key={item.id}
    item={item}
    index={index + 2}
    onSelect={selectStory}
...
Help them understand the key implications.`;
  ...
  />
    ...
    /* AI Chat overlay */
      ...
      {chatNewsItem && <MentorChatOverlay
...
const s = StyleSheet.create({
  ...
  feedCard: {
    ...
    gap: 14,
    borderWidth: 1,
    borderColor: COLORS.intelSeparator,
    ...SHADOWS.sm
  },
