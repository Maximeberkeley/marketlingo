...
import { View, Text, StyleSheet, TouchableOpacity, Animated, Easing, Linking, Image, ScrollView, Dimensions, Modal, TextInput, KeyboardAvoidingView, Platform, ActivityIndicator, } from 'react-native';
...
function buildFallbackQuiz(article) {
    return [
        {
...
function normalizeQuizQuestions(payload) {
    ...
    const normalized = questionList
        ...
        const options = Array.isArray(candidate.options)
            ? candidate.options.filter((option) => typeof option === 'string' && option.trim().length > 0).slice(0, 4)
            : [];
...
function extractQuizResponseText(data) {
...
// ── Impact dot helper ──
function getImpactFromContent(title, summary) {
    ...
    if (highSignals.some(s => text.includes(s)))
        return 'high';
    if (mediumSignals.some(s => text.includes(s)))
        return 'medium';
    return 'low';
}
// ── Featured Carousel ──
function FeaturedCarousel({ items, onSelect }) {
    ...
    if (items.length === 0)
        return null;
    return (<View style={s.featuredCarousel}>
      <Animated.ScrollView horizontal showsHorizontalScrollIndicator={false} snapToInterval={FEATURED_CARD_WIDTH + 12} decelerationRate="fast" contentContainerStyle={s.featuredTrack} onScroll={Animated.event([{ nativeEvent: { contentOffset: { x: scrollX } } }], { useNativeDriver: false })} scrollEventThrottle={16}>
        {items.map((item, i) => {
            ...
            return (<Animated.View key={item.id} style={{ transform: [{ scale }], opacity, width: FEATURED_CARD_WIDTH }}>
              <TouchableOpacity activeOpacity={0.85} onPress={() => onSelect(item)} style={s.featuredCard}>
                {item?.imageUrl ? (<Image source={{ uri: item.imageUrl }} style={s.featuredImage} resizeMode="cover"/>) : (<View style={[s.featuredImage, { backgroundColor: GRADIENT_SETS[i % GRADIENT_SETS.length][0] }]}/>)}
                ...
                <View style={s.featuredContent}>
                  ...
                  {item?.summary ? (<Text style={s.featuredSummary} numberOfLines={1}>{item.summary}</Text>) : null}
...
// ── News Feed Card ──
function NewsFeedCard({ item, index, onSelect, }) {
    ...
    return (<Animated.View style={{ transform: [{ translateY: slideAnim }], opacity: opacityAnim }}>
      <TouchableOpacity activeOpacity={0.8} onPress={() => onSelect(item)} style={s.feedCard}>
        ...
        <View style={s.feedThumb}>
          {item.imageUrl ? (<Image source={{ uri: item.imageUrl }} style={s.feedThumbImage} resizeMode="cover"/>) : (<View style={[s.feedThumbImage, { alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.accentSoft }]}>
...
// ── Quiz Modal ──
function QuizModal({ article, marketId, onClose, }) {
...
No other text, just the JSON array.`,
    ...
    return (<Modal visible={!!article} transparent animationType="slide" onRequestClose={onClose}>
      <View style={qz.backdrop}>
        <View style={qz.sheet}>
          ...
          <ScrollView style={qz.bodyScroll} contentContainerStyle={qz.bodyScrollContent} showsVerticalScrollIndicator={false} bounces={false}>
            {loading ? (<View style={qz.loadingBox}>
                <ActivityIndicator size="small" color={COLORS.accent}/>
                <Text style={qz.loadingText}>Generating quiz from this article...</Text>
              </View>) : isComplete ? (<View style={qz.completeBox}>
              ...
              </View>) : q ? (<View style={qz.questionBox}>
                ...
                return (<TouchableOpacity key={i} style={[
                    ...
                    }} activeOpacity={0.7} disabled={selectedAnswer !== null}>
                      <Text style={[
                        qz.optionText,
                ...
                {selectedAnswer !== null && (<View style={qz.explainBox}>
                    <Feather name="info" size={13} color={COLORS.accent}/>
                    <Text style={qz.explainText}>{q.explanation}</Text>
                  </View>)}

                {selectedAnswer !== null && (<TouchableOpacity style={qz.nextBtn} onPress={() => {
                    setSelectedAnswer(null);
                    setCurrentQ(c => c + 1);
                }}>
...
function ArticleDetailSheet({ article, onClose, marketId, }) {
    ...
    return (<Modal visible={!!article} transparent animationType="none" onRequestClose={handleClose}>
        ...
        <Animated.View style={[ds.sheet, { transform: [{ translateY }] }]}>
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
            ...
            <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} bounces={true}>
              {article.imageUrl ? (<View style={ds.heroContainer}>
              ...
              <View style={ds.contentPad}>
                <View style={ds.metaRow}>
                  ...
                  {isHighImpact && (<View style={ds.highImpactPill}>
                ...
                <TouchableOpacity onPress={() => { if (article.sourceUrl)
        Linking.openURL(article.sourceUrl).catch(() => { }); }} style={ds.sourceLink}>
                ...
                <View style={ds.aiSection}>
                  ...
                  <View style={ds.aiButtonsRow}>
                    {([
            { mode: 'discuss', icon: 'message-circle', label: 'Discuss' },
            { mode: 'summarize', icon: 'file-text', label: 'Summarize' },
            { mode: 'why', icon: 'zap', label: 'Why it matters' },
        ]).map(({ mode, icon, label }) => (<TouchableOpacity key={mode} onPress={() => { resetAi(); handleAiAction(mode); }} style={[ds.aiBtn, aiMode === mode && ds.aiBtnActive]} activeOpacity={0.7}>
                  ...
                  {(aiLoading || aiResponse) ? (<View style={ds.aiResponseBox}>
                      {aiLoading && !aiResponse ? (<View style={ds.aiLoadingRow}>
                          <ActivityIndicator size="small" color={COLORS.accent}/>
                          <Text style={ds.aiLoadingText}>Analyzing...</Text>
                        </View>) : aiMode === 'discuss' ? (<View>
                          {chatMessages.map((msg, i) => (<Text key={i} style={[ds.chatMsg, msg.role === 'user' ? ds.chatMsgUser : ds.chatMsgAi]}>{msg.content}</Text>))}
                          {aiLoading && (<View style={ds.aiLoadingRow}>
                              <ActivityIndicator size="small" color={COLORS.accent}/>
                              <Text style={ds.aiLoadingText}>Thinking...</Text>
                            </View>)}
                          <View style={ds.chatInputRow}>
                            <TextInput value={chatInput} onChangeText={setChatInput} placeholder="Ask a follow-up..." placeholderTextColor={COLORS.textMuted} style={ds.chatInput} onSubmitEditing={handleChatSend} returnKeyType="send"/>
                        ...
                        </View>) : (<Text style={ds.aiResponseText}>{aiResponse}</Text>)}
                    </View>) : null}
                </View>
              </View>
            </ScrollView>
...
// ── Main Component ──
export function DailyNews({ marketId, learningGoal, autoOpen = false }) {
    ...
    const [chatNewsItem, setChatNewsItem] = useState(null);
    ...
    const openStory = useCallback((items, index) => {
        const safe = index >= 0 ? index : 0;
        setImmersiveIndex(safe);
        const item = items[safe];
        if (item)
            intel.recordRead(item.id);
    }, [intel]);
    const kaiMentor = getMentorForContext('news') || {
        ...
        try {
            if (!forceRefresh) {
                ...
                if (!dbError && data && data.length > 0) {
                    ...
                    const hasAnyMissingImages = data.some((item) => !item.image_url);
            ...
            if (!fnError && liveData?.success && liveData.data?.length > 0) {
                setNews(liveData.data.map((item) => ({
                    id: item?.id ?? `news-${Math.random().toString(36).slice(2)}`,
    ...
    useEffect(() => {
        if (!autoOpen || autoOpened.current || isLoading || news.length === 0)
            return;
    ...
    return (<View style={s.container}>
      {/* Loading */}
      {isLoading && (<View style={s.loadingContainer}>
      ...
      {/* Error */}
      {!isLoading && error && (<View style={s.emptyCard}>
      ...
      {/* Content */}
      {!isLoading && !error && news.length > 0 && (<View>
          {/* Featured horizontal carousel */}
          {featured.length > 0 && (<FeaturedCarousel items={featured} onSelect={(item) => {
                    openStory(news, news.findIndex(n => n.id === item.id));
                }}/>)}

          {/* Feed section label */}
          {feed.length > 0 && featured.length > 0 && (<View style={s.feedSectionHeader}>
          ...
          <View style={s.feedList}>
            {feed.map((item, index) => (<NewsFeedCard key={item.id} item={item} index={index} onSelect={(item) => {
                    openStory(news, news.findIndex(n => n.id === item.id));
                }}/>))}
          </View>
        </View>)}
      ...
      {/* Empty */}
      {!isLoading && !error && news.length === 0 && (<View style={s.emptyCard}>
      ...
      {lastFetched && !isLoading && news.length > 0 && (<Text style={s.lastUpdated}>Updated {lastFetched.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>)}

      {/* Immersive News Overlay */}
      <ImmersiveNewsOverlay visible={immersiveIndex >= 0} articles={news} initialIndex={immersiveIndex >= 0 ? immersiveIndex : 0} onClose={() => setImmersiveIndex(-1)} onOpenChat={(article) => {
            const ctx = `The user wants to discuss this ${marketId} industry news article:\n\nTitle: "${article.title}"\nSource: ${article.sourceName}\nSummary: ${article.summary ?? 'N/A'}\n\nHelp them understand the key implications.`;
            setChatContext(ctx);
            setChatNewsItem(article);
        }} onArticleView={(article) => intel.recordRead(article.id)} onQuiz={(article) => {
            setImmersiveIndex(-1);
            setQuizArticle(article);
        }} marketId={marketId} learningGoal={learningGoal}/>

      {/* Quiz Modal */}
      <QuizModal article={quizArticle} marketId={marketId} onClose={() => setQuizArticle(null)}/>

      {/* AI Chat overlay */}
      {chatNewsItem && (<MentorChatOverlay visible={!!chatNewsItem} mentor={kaiMentor} onClose={() => setChatNewsItem(null)} context={chatContext} marketId={marketId}/>)}
    </View>);
}
// ── Styles ──
const INTEL = {
