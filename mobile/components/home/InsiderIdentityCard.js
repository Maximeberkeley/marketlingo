...
function identityLabel(streak) {
    if (streak >= 90)
        return 'Insider';
    if (streak >= 30)
        return 'Regular in the room';
    if (streak >= 14)
        return 'Getting fluent';
    if (streak >= 7)
        return 'Speaking the language';
    if (streak >= 3)
        return 'Showing up';
    return 'Just committed';
}
export function InsiderIdentityCard({ marketName, streak, longestStreak = 0, lessonCompletedToday, focus, deliverableTitle, deliverableCompletion, reviewDueCount, tomorrowTitle, rescueAvailable, accent = COLORS.accent, onStartLesson, onOpenDeliverable, onOpenReview, onRescue, }) {
    ...
    const mood = useMemo(() => {
        if (lessonCompletedToday)
            return streak >= 7 ? 'trophy' : 'success';
        if (minutes <= 90)
            return 'urgent';
        if (minutes <= 240)
            return 'sassy';
    ...
    const line = useMemo(() => {
        ...
        if (minutes <= 90)
            return `${hours}h ${mins}m left. You built ${streak} days — do not hand them back tonight.`;
        if (minutes <= 240)
            return `Still unfinished. Your ${streak}-day run on ${topic} is watching you.`;
    ...
    return (<View style={[styles.card, { borderColor: accent + '2E' }]}>
      <View style={styles.top}>
        ...
        <View style={styles.flex}>
          ...
          {longestStreak > streak ? (<Text style={styles.best}>Your best run: {longestStreak} days</Text>) : null}
        </View>
      </View>
      ...
      {!lessonCompletedToday && (<>
          <TouchableOpacity style={[styles.cta, { backgroundColor: accent }]} onPress={() => { triggerHaptic('medium'); onStartLesson(); }} activeOpacity={0.9}>
            <Feather name="play" size={15} color="#FFFFFF"/>
            <Text style={styles.ctaText}>
              Keep it alive · {hours > 0 ? `${hours}h ${mins}m` : `${mins}m`} left today
            </Text>
          </TouchableOpacity>
          {rescueAvailable && onRescue ? (<TouchableOpacity style={styles.rescue} onPress={onRescue} activeOpacity={0.8}>
      ...
      <View style={styles.loops}>
        ...
        {reviewDueCount > 0 && (<TouchableOpacity style={styles.loop} onPress={onOpenReview} activeOpacity={0.8}>
        ...
        {tomorrowTitle ? (<View style={styles.loop}>
