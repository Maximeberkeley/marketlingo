...
import { View, Text, StyleSheet, Modal, TouchableOpacity, ScrollView, Animated, PanResponder, Dimensions, Image, } from 'react-native';
import { Feather } from '@expo/vector-icons';
import { COLORS } from '../../lib/constants';
import { getSegmentStyle } from '../../lib/segmentColors';
const { width: SCREEN_WIDTH } = Dimensions.get('window');
export function CompanyDetailModal({ company, onClose, isOnWatchlist, onToggleWatchlist }) {
    ...
    const panResponder = useRef(PanResponder.create({
        ...
        onPanResponderRelease: (_, gestureState) => {
            if (gestureState.dx < -50)
                setCurrentSlide(s => Math.min(s + 1, (company?.slides.length ?? 0)));
    ...
    return (<Modal visible={!!company} animationType="slide" transparent onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.sheet}>
          ...
          <View style={styles.header}>
            <View style={styles.logoContainer}>
              {company.logoUrl ? (<Image source={{ uri: company.logoUrl }} style={styles.logoImage} resizeMode="contain"/>) : (<Text style={styles.logoInitial}>{company.name.charAt(0)}</Text>)}
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.companyName}>{company.name}</Text>
              <View style={{ flexDirection: 'row', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                {company.ticker && (<View style={styles.tickerBadge}>
            ...
            {/* Watchlist toggle */}
            {onToggleWatchlist && (<TouchableOpacity style={[styles.watchlistBtn, isOnWatchlist && styles.watchlistBtnActive]} onPress={() => onToggleWatchlist(company)}>
                <Feather name={isOnWatchlist ? 'check' : 'plus'} size={14} color={isOnWatchlist ? '#22C55E' : COLORS.accent}/>
          ...
          <View style={styles.progressRow}>
            {Array.from({ length: totalSlides }).map((_, i) => (<TouchableOpacity key={i} onPress={() => goToSlide(i)} style={[styles.progressDot, i === currentSlide && styles.progressDotActive]}/>))}
          </View>
        ...
        <View style={{ gap: 14 }}>
                ...
                { label: 'Employees', value: company.employees, icon: 'E' },
            ].map((stat) => (<View key={stat.label} style={styles.statCard}>
                  ...
                  {/* Key Products */}
                  {company.keyProducts.length > 0 && (<View>
                      <Text style={styles.sectionLabel}>KEY PRODUCTS</Text>
                      <View style={styles.productsRow}>
                        {company.keyProducts.map((p) => (<View key={p} style={styles.productChip}>
                  ...
                  {onToggleWatchlist && !isOnWatchlist && (<TouchableOpacity style={styles.watchlistCTA} onPress={() => onToggleWatchlist(company)}>
        ...
        <View style={styles.insightSlide}>
                ...
                : slideData?.type === 'investment' ? 'INVESTMENT THESIS'
                  ...
                  {slideData?.highlight && (<View style={[styles.highlightBadge, { borderColor: slideTypeColor + '50', backgroundColor: slideTypeColor + '20' }]}>
          ...
          <View style={styles.navRow}>
            <TouchableOpacity onPress={() => goToSlide(currentSlide - 1)} style={[styles.navBtn, currentSlide === 0 && styles.navBtnDisabled]} disabled={currentSlide === 0}>
              <Text style={styles.navBtnText}>←</Text>
            </TouchableOpacity>
            <Text style={styles.slideCounter}>{currentSlide + 1} / {totalSlides}</Text>
            <TouchableOpacity onPress={() => goToSlide(currentSlide + 1)} style={[styles.navBtn, currentSlide === totalSlides - 1 && styles.navBtnDisabled]} disabled={currentSlide === totalSlides - 1}>
              <Text style={styles.navBtnText}>→</Text>
            </TouchableOpacity>
          </View>
        </View>
