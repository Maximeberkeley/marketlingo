...
export function StreakCriticalTimer({ streak, expiresAt, onStartLesson, isProUser, canUseLeoLogs, onUseLeoLogs, }) {
    ...
    const handleLeoLogs = async () => {
        ...
        setUsingLogs(false);
        if (ok) {
            Alert.alert('🦁 Leo added logs to your fire!', `Your ${streak}-day streak is safe for 24 more hours. Leo's got your back!`);
        }
        else {
    ...
    if (expired) {
        return (<View style={[styles.container, styles.expiredContainer]}>
    ...
    return (<Animated.View style={[styles.container, { opacity: glowAnim.interpolate({ inputRange: [0.3, 1], outputRange: [0.92, 1] }) }]}>
      {/* Timer display */}
      ...
      {/* CTAs */}
      <TouchableOpacity style={[styles.ctaBtn, isUltraCritical && { backgroundColor: COLORS.error }]} onPress={() => { triggerHaptic('medium'); onStartLesson(); }} activeOpacity={0.8}>
      ...
      {/* Pro: Leo Logs */}
      {isProUser && canUseLeoLogs && (<TouchableOpacity style={styles.leoLogsBtn} onPress={handleLeoLogs} activeOpacity={0.8} disabled={usingLogs}>
