...
export function SeasonSection({ seasonNumber, title, subtitle, weeks, onWeekClick, onLessonClick, defaultExpanded = false, }) {
    ...
    return (<View style={styles.container}>
      ...
      {/* Progress bar */}
      {progress > 0 && (<View style={styles.progressBarWrap}>
          <View style={[styles.progressBarFill, { width: `${progress}%` }]}/>
        </View>)}

      {/* Weeks (collapsed/expanded) */}
      {isExpanded && (<View style={styles.weeksContainer}>
          {weeks.map((week, index) => (<View key={week.weekNumber} style={styles.weekWrapper}>
              {/* Node + title row */}
              <View style={styles.weekRow}>
                <RoadmapNode weekNumber={week.weekNumber} status={week.status} onClick={() => handleWeekClick(week.weekNumber, week.status)}/>

                <TouchableOpacity style={[
                    styles.weekLabel,
                    week.status === 'locked' && styles.weekLabelLocked,
                    week.status === 'current' && styles.weekLabelCurrent,
                ]} onPress={() => handleWeekClick(week.weekNumber, week.status)} disabled={week.status === 'locked'} activeOpacity={0.7}>
                  <Text style={[
                    styles.weekTitle,
                ...
                {week.status === 'current' && (<View style={styles.currentChip}>
                    <Text style={styles.currentChipText}>NOW</Text>
                  </View>)}
              </View>

              {/* Expanded lessons - Duolingo style */}
              {expandedWeek === week.weekNumber && week.lessons && week.lessons.length > 0 && (<View style={styles.lessonsContainer}>
                  {week.lessons.map((lesson, lessonIdx) => (<TouchableOpacity key={lesson.day} style={[
                            styles.lessonRow,
                            lesson.completed && styles.lessonRowCompleted,
                            lesson.current && styles.lessonRowCurrent,
                        ]} onPress={() => onLessonClick?.(lesson.day)} disabled={!lesson.completed && !lesson.current} activeOpacity={0.75}>
                      <View style={[
                            styles.lessonIcon,
                      ...
                      {(lesson.completed || lesson.current) && (<Text style={{ fontSize: 12, color: COLORS.textMuted }}>›</Text>)}
                    </TouchableOpacity>))}
                </View>)}

              {/* Connector line */}
              {index < weeks.length - 1 && (<View style={styles.connectorWrap}>
                  <View style={[
                        styles.connector,
