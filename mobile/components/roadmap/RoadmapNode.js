...
export function RoadmapNode({ weekNumber, status, onClick }) {
    ...
    return (<TouchableOpacity onPress={isInteractive ? onClick : undefined} disabled={!isInteractive} style={styles.container} activeOpacity={isInteractive ? 0.7 : 1}>
      {status === 'current' && (<Animated.View style={[
                styles.pulseRing,
                {
      ...
      <View style={nodeStyle}>
        {status === 'locked' && (<View style={styles.lockIconWrap}>
            <View style={styles.lockBar}/>
            <View style={styles.lockBody}/>
          </View>)}
        {status === 'completed' && <Text style={styles.checkIcon}>✓</Text>}
        {(status === 'current' || status === 'available') && (<Text style={styles.weekNumber}>{weekNumber}</Text>)}
      </View>
    </TouchableOpacity>);
}
const NODE_SIZE = 48;
