...
export function SnapshotCompare({ visible, onClose, snapshots }) {
    ...
    const deltaColor = (v) => (v > 0 ? '#22C55E' : v < 0 ? '#EF4444' : COLORS.textMuted);
        ...
        <View style={styles.container}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
            ...
            <View style={styles.selectorRow}>
              <View style={styles.selectorCol}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  {snapshots.map((s, i) => (<TouchableOpacity key={s.id} style={[styles.selectorChip, i === leftIdx && styles.selectorChipActive]} onPress={() => { setLeftIdx(i); if (i >= rightIdx)
            setRightIdx(Math.min(i + 1, snapshots.length - 1)); }}>
              ...
              <View style={styles.selectorCol}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  {snapshots.map((s, i) => (<TouchableOpacity key={s.id} style={[styles.selectorChip, i === rightIdx && { ...styles.selectorChipActive, borderColor: '#3B82F6' }]} onPress={() => { setRightIdx(i); if (i <= leftIdx)
            setLeftIdx(Math.max(i - 1, 0)); }}>
            ...
            <View style={styles.compareCard}>
              ...
              {['core', 'growth', 'speculative'].map(cat => {
            const lVal = lBd[cat] || 0;
            const rVal = rBd[cat] || 0;
            const d = rVal - lVal;
            return (<View key={cat} style={styles.compareRow}>
            ...
            {/* Changes */}
            {(addedCompanies.length > 0 || removedCompanies.length > 0) && (<View style={styles.changesCard}>
                <Text style={styles.compareLabel}>POSITION CHANGES</Text>
                {addedCompanies.map(p => (<View key={p.company?.id} style={styles.changeRow}>
                ...
                {removedCompanies.map(p => (<View key={p.company?.id} style={styles.changeRow}>
                ...
                {addedCompanies.length === 0 && removedCompanies.length === 0 && (<Text style={styles.noChanges}>Same companies, different allocations</Text>)}
              </View>)}

            {/* Strategy notes comparison */}
            {(left.strategy_notes || right.strategy_notes) && (<View style={styles.notesCompare}>
                <Text style={styles.compareLabel}>STRATEGY NOTES</Text>
                {left.strategy_notes && (<View style={styles.noteBox}>
                    <Text style={styles.noteBoxLabel}>{left.snapshot_name}</Text>
                    <Text style={styles.noteBoxText}>{left.strategy_notes}</Text>
                  </View>)}
                {right.strategy_notes && (<View style={[styles.noteBox, { borderColor: 'rgba(59,130,246,0.3)' }]}>
