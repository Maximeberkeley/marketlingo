...
export function ThesisReviewOverlay({ visible, onClose, dueTheses, onReview, onUpdateThesis }) {
    const [currentIdx, setCurrentIdx] = useState(0);
    const [editMode, setEditMode] = useState(false);
    const [editedThesis, setEditedThesis] = useState('');
    if (dueTheses.length === 0)
        return null;
    const current = dueTheses[Math.min(currentIdx, dueTheses.length - 1)];
    if (!current)
        return null;
    const handleReview = async (valid) => {
      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {editMode ? (<View style={styles.editSection}>
                <Text style={styles.editLabel}>Update your thesis</Text>
                <TextInput style={styles.editInput} value={editedThesis} onChangeText={setEditedThesis} multiline placeholder="Write your updated investment thesis..." placeholderTextColor={COLORS.textMuted} textAlignVertical="top"/>
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  <TouchableOpacity style={styles.cancelBtn} onPress={() => setEditMode(false)}>
                    <Text style={styles.cancelBtnText}>Cancel</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.updateBtn, !editedThesis.trim() && { opacity: 0.5 }]} disabled={!editedThesis.trim()} onPress={handleUpdate}>
                ...
                <TouchableOpacity style={styles.editBtn} onPress={() => { setEditedThesis(current.thesis); setEditMode(true); }}>
            ...
            {/* Progress dots */}
            {dueTheses.length > 1 && (<View style={styles.dotsRow}>
                {dueTheses.map((_, i) => (<View key={i} style={[styles.dot, i === currentIdx && styles.dotActive, i < currentIdx && styles.dotDone]}/>))}
              </View>)}
          </ScrollView>
        </View>
      </View>
