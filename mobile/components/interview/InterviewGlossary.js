...
export function InterviewGlossary({ marketName, marketId }) {
    const [search, setSearch] = useState('');
    const { saveToNotebook, saving } = useInterviewNotebook(marketId || 'default');
    const filtered = useMemo(() => {
        if (!search.trim())
            return CONSULTING_GLOSSARY;
        const q = search.toLowerCase();
        return CONSULTING_GLOSSARY.filter(t => t.term.toLowerCase().includes(q) || t.definition.toLowerCase().includes(q) || t.category.toLowerCase().includes(q));
    }, [search]);
    ...
    return (<ScrollView contentContainerStyle={st.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={st.searchBar}>
        ...
        <TextInput style={st.searchInput} placeholder="Search terms..." placeholderTextColor={COLORS.textMuted} value={search} onChangeText={setSearch}/>
        {search.length > 0 && (<Text style={st.resultCount}>{filtered.length} results</Text>)}
      </View>

      {/* Terms by category */}
      {Object.entries(grouped).map(([category, terms]) => (<View key={category}>
          <Text style={st.categoryLabel}>{category}</Text>
          {terms.map(t => (<View key={t.term} style={st.termCard}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                <Text style={[st.termName, { flex: 1 }]}>{t.term}</Text>
                <TouchableOpacity onPress={() => saveToNotebook(`📖 ${t.term}\n\n${t.definition}\n\nCategory: ${t.category}`, 'interview-glossary')} disabled={saving} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      {filtered.length === 0 && (<View style={st.emptyState}>
