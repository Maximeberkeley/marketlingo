...
const BUSINESS_MATH_BY_INDUSTRY = {
    aerospace: [
...
function generateArithmeticProblem(difficulty) {
    const ops = ['+', '−', '×', '÷', '%'];
...
function NumericKeypad({ value, onChange, onSubmit, disabled }) {
    ...
    return (<View style={kp.container}>
      ...
      <View style={kp.grid}>
        {keys.map((key) => (<TouchableOpacity key={key} disabled={disabled} onPress={() => {
                triggerHaptic('light');
                if (key === '⌫')
      ...
      <TouchableOpacity style={[kp.submitKey, (!value || disabled) && { opacity: 0.4 }]} disabled={!value || disabled} onPress={onSubmit}>
...
// ─── Main Component ───
export function MentalMathTab({ marketName, marketId }) {
    ...
    const checkBusinessAnswer = (optIndex) => {
        if (!problem)
            return;
    ...
    return (<ScrollView contentContainerStyle={st.scrollContent} showsVerticalScrollIndicator={false}>
      {/* Header */}
      <View style={st.sectionHeader}>
        ...
        {streak >= 3 && (<Animated.View style={[st.streakBadge, { transform: [{ scale: pulseAnim }] }]}>
      ...
      <View style={st.modeToggle}>
        <TouchableOpacity onPress={() => { setMode('arithmetic'); triggerHaptic('light'); }} style={[st.modeBtn, mode === 'arithmetic' && st.modeBtnActive]}>
        ...
        <TouchableOpacity onPress={() => { setMode('business'); triggerHaptic('light'); }} style={[st.modeBtn, mode === 'business' && st.modeBtnActive]}>
      ...
      {mode === 'arithmetic' && (<View style={st.difficultyRow}>
          {[1, 2, 3].map(d => (<TouchableOpacity key={d} onPress={() => { setDifficulty(d); triggerHaptic('light'); nextArithmetic(); }} style={[st.diffBtn, difficulty === d && st.diffBtnActive]}>
      ...
      {/* Timer expired */}
      {!timerActive && timer === 0 && (<View style={st.timerEndCard}>
      ...
      {/* Problem Card */}
      {problem && (timer > 0 || !timerActive) && (<View style={st.problemCard}>
          ...
          {mode === 'arithmetic' ? (<>
              <NumericKeypad value={inputValue} onChange={setInputValue} onSubmit={checkArithmeticAnswer} disabled={result !== null}/>
              {result && (<View style={[st.resultBanner, result === 'correct' ? st.resultCorrect : st.resultWrong]}>
              ...
              {problem.options?.map((opt, i) => {
                    ...
                    return (<TouchableOpacity key={i} disabled={revealed} onPress={() => checkBusinessAnswer(i)} style={[
                            st.optionBtn,
              ...
              {result && problem.explanation && (<View style={st.explanationBox}>
                  <Feather name="info" size={14} color="#3B82F6"/>
                  <Text style={st.explanationText}>{problem.explanation}</Text>
                </View>)}
            </>)}

          {result && (<TouchableOpacity style={st.nextBtn} onPress={nextQuestion}>
