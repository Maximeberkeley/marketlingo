...
export function FrameworksTab({ marketName, marketId }) {
    const [expanded, setExpanded] = useState(null);
    const [readFrameworks, setReadFrameworks] = useState(new Set());
    ...
    return (<ScrollView contentContainerStyle={st.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      {FRAMEWORKS_LIBRARY.map((fw) => {
            ...
            return (<View key={fw.id} style={st.fwCard}>
            <TouchableOpacity onPress={() => toggleFramework(fw.id)} style={st.fwHeader}>
              ...
              {readFrameworks.has(fw.id) && (<View style={st.readBadge}>
                  <Feather name="check" size={10} color="#10B981"/>
                </View>)}
            </TouchableOpacity>

            {isExpanded && (<View style={st.fwBody}>
                <Text style={st.fwOverview}>{fw.overview}</Text>

                {/* Visual Grid */}
                {fw.id === 'bcg' ? (<View style={st.bcgGrid}>
                    {fw.components.map((c, i) => (<View key={i} style={[st.bcgCell, { borderColor: c.color }]}>
                  ...
                  </View>) : (<View style={st.componentsList}>
                    {fw.components.map((c, i) => (<View key={i} style={[st.componentItem, { borderLeftColor: c.color }]}>
                ...
                {/* Industry Example */}
                {industryExample && (<View style={st.industryBox}>
                ...
                {/* Save to Notebook */}
                <TouchableOpacity onPress={() => handleSaveFramework(fw)} disabled={saving} style={st.saveBtn}>
