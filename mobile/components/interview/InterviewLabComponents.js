...
// ─── Progress Tracker ───
export function StageTracker({ current, onTap }) {
    const stages = [
    ...
    return (<View style={styles.trackerRow}>
      {stages.map((s, i) => {
            const done = current > s.stage;
            const active = current === s.stage;
            return (<React.Fragment key={s.stage}>
...
// ─── Vibe Check Meter ───
export function VibeMeter({ text }) {
    ...
    return (<View style={styles.vibeMeter}>
...
// ─── LEO Celebration ───
export function LeoCelebration({ visible, score, onDismiss }) {
    ...
    if (!visible)
        return null;
    return (<Animated.View style={[styles.celebrationOverlay, { opacity: opacityAnim }]}>
...
export function ScoreBar({ label, value, color }) {
    return (<View style={styles.scoreBarWrap}>
...
// ─── Mental Math Drill ───
export function MathDrill({ question: q }) {
    const [selected, setSelected] = React.useState(null);
    const revealed = selected !== null;
    return (<View style={styles.card}>
      <Text style={[styles.cardBody, { fontWeight: '600', marginBottom: 10 }]}>{q.question}</Text>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
        {q.options.map((opt, i) => (<TouchableOpacity key={i} disabled={revealed} onPress={() => { setSelected(i); triggerHaptic(i === q.correctIndex ? 'success' : 'error'); }} style={[
                styles.mathOption,
