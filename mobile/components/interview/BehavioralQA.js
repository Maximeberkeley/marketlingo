...
export function BehavioralQA({ marketName, marketId }) {
    ...
    const saveNote = useCallback(async (questionId) => {
        if (!noteText.trim())
            return;
    ...
    const deleteNote = useCallback(async (questionId) => {
    ...
    return (<ScrollView contentContainerStyle={st.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={st.categoryScroll} contentContainerStyle={st.categoryContent}>
        {CATEGORIES.map(cat => (<TouchableOpacity key={cat} onPress={() => { setSelectedCategory(cat); triggerHaptic('light'); }} style={[st.categoryBtn, selectedCategory === cat && st.categoryBtnActive]}>
      ...
      {filteredQuestions.map((q) => {
            ...
            return (<View key={q.id} style={[st.qaCard, hasNote && st.qaCardAnswered]}>
            ...
            {isExpanded && (<View style={st.qaBody}>
                {/* Tip */}
                ...
                {/* Frameworks */}
                {q.frameworks && (<View style={st.fwRow}>
                    {q.frameworks.map(fw => (<View key={fw} style={st.fwTag}>
                ...
                {/* Add/Edit Note */}
                {isEditing ? (<View style={st.noteEditBox}>
                    <TextInput style={st.noteInput} multiline placeholder="Write your personalized answer..." placeholderTextColor={COLORS.textMuted} value={noteText} onChangeText={setNoteText} autoFocus/>
                  ...
                  </View>) : !hasNote ? (<TouchableOpacity onPress={() => { setEditingNote(q.id); setNoteText(''); triggerHaptic('light'); }} style={st.addNoteBtn}>
