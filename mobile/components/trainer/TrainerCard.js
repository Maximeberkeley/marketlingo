...
function getWhyExplanation(scenario, question, marketId) {
...
function LeoFeedbackReaction({ isCorrect }) {
    ...
    return (<Animated.View style={[
      ...
      <Animated.Image source={LEO_IMAGE} style={[leoStyles.avatar, { transform: [{ translateY: bounceAnim }] }]}/>
...
// ─────────────────────────────────────────────
export function TrainerCard({ scenario, onSaveToNotebook, onNext, onAskMentor, onAttemptComplete, marketId, }) {
    ...
    const getOptionStyle = (index) => {
        ...
        if (isCorrectOption)
            return styles.optionCorrectUnselected;
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.optionsList}>
        {scenario.options.map((opt, idx) => (<TouchableOpacity key={idx} style={[styles.option, getOptionStyle(idx)]} onPress={() => handleOptionSelect(idx)} disabled={showFeedback || isSubmitting} activeOpacity={0.75}>
            ...
            {isSubmitting && idx === selectedIndex && (<ActivityIndicator size="small" color={COLORS.accent}/>)}
          </TouchableOpacity>))}
      </View>

      {/* Feedback */}
      {showFeedback && (<View style={styles.feedbackSection}>
          {/* Leo Reaction */}
          ...
          {/* Pro Reasoning */}
          {(serverFeedback?.feedback_pro_reasoning || scenario.feedbackProReasoning) && (<View style={[styles.feedbackCard, styles.feedbackGreen]}>
          ...
          {/* Common Mistake */}
          {(serverFeedback?.feedback_common_mistake || scenario.feedbackCommonMistake) && (<View style={[styles.feedbackCard, styles.feedbackAmber]}>
          ...
          {/* Mental Model */}
          {(serverFeedback?.feedback_mental_model || scenario.feedbackMentalModel) && (<View style={[styles.feedbackCard, styles.feedbackBlue]}>
          ...
          {/* Follow-up */}
          {(serverFeedback?.follow_up_question || scenario.followUpQuestion) && (<View style={[styles.feedbackCard, styles.feedbackPurple]}>
          ...
          {/* Mentor CTA */}
          {onAskMentor && (<View style={styles.mentorCTACard}>
              ...
              <TouchableOpacity style={styles.mentorCTABtn} onPress={() => onAskMentor(serverFeedback?.follow_up_question || scenario.followUpQuestion || scenario.question)} activeOpacity={0.8}>
