...
export function LessonCard({ title, subtitle, headline, xp, duration, colorScheme = 'purple', imageSrc, isCompleted = false, onClick }) {
    const colors = colorMap[colorScheme] || colorMap.purple;
    return (<TouchableOpacity style={[styles.container, { borderColor: colors.border }, isCompleted && { opacity: 0.75 }]} onPress={onClick} activeOpacity={0.85}>
      {/* Hero Image Section */}
      {imageSrc && (<View style={styles.imageContainer}>
      ...
      <View style={[styles.content, imageSrc ? styles.contentWithImage : { backgroundColor: colors.bg }]}>
        ...
        {!imageSrc && (<View style={styles.header}>
        ...
        <View style={styles.bottomRow}>
          <View style={{ flex: 1, marginRight: 8 }}>
            {imageSrc && (<View style={[styles.tag, { backgroundColor: colors.tag, marginBottom: 4, alignSelf: 'flex-start' }]}>
                <Text style={[styles.tagText, { color: colors.accent }]}>{subtitle}</Text>
              </View>)}
