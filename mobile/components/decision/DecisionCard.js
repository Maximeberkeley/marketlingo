...
import { submitDecision, } from '../../lib/decisionEngine';
import { ConfidenceSelector } from './ConfidenceSelector';
import { LeoCoachChips } from '../ai/LeoCoachChips';
export function DecisionCard({ scenario, askConfidence = true, onResolved, lessonContext }) {
    const [selected, setSelected] = useState(null);
    const [confidence, setConfidence] = useState(askConfidence ? null : 'fairly_sure');
    ...
    return (<View style={styles.card}>
      ...
      {scenario.options.map((opt, i) => {
            ...
            return (<TouchableOpacity key={i} activeOpacity={0.85} disabled={!!result || submitting} onPress={() => {
      ...
      {askConfidence && !result && (<ConfidenceSelector value={confidence} onChange={setConfidence} disabled={selected === null}/>)}

      {!result && (<TouchableOpacity style={[styles.submit, !canSubmit && { opacity: 0.45 }]} disabled={!canSubmit} onPress={handleSubmit} activeOpacity={0.85}>
          {submitting ? (<ActivityIndicator color="#FFFFFF"/>) : (<Text style={styles.submitText}>Lock in decision</Text>)}
        </TouchableOpacity>)}

      {result && (<View style={styles.reveal}>
