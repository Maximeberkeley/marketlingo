...
import { fetchScenarios, gradeFromAnswer } from '../../lib/decisionEngine';
import { isFeatureEnabled } from '../../hooks/useFeatureFlags';
import { trackEvent } from '../../lib/analytics';
import { DecisionCard } from './DecisionCard';
export function LessonDecisionModal({ visible, marketId, stackId, dayNumber, lessonTitle, onDone, onGraded, }) {
    ...
    return (<Modal visible transparent animationType="slide" onRequestClose={onDone}>
      <View style={[styles.sheet, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 20 }]}>
        {loading || !scenario ? (<ActivityIndicator color={COLORS.accent} style={{ marginTop: 40 }}/>) : (<ScrollView contentContainerStyle={{ paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
            ...
            <View style={{ marginTop: 18 }}>
              <DecisionCard scenario={scenario} askConfidence lessonContext={lessonTitle} onResolved={(res) => {
                setResolved(true);
