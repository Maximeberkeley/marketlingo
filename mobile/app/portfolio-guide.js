...
const GUIDE_STEPS = [
    {
        ...
        icon: 'edit-3',
        color: '#8B5CF6',
        description: 'Every great portfolio starts with a thesis — a clear belief about where an industry is heading. Your thesis guides every investment decision.',
        tips: [
    ...
    },
    {
        ...
        icon: 'search',
        color: '#3B82F6',
        description: 'Deep research separates conviction from speculation. Study the companies that define your chosen market — their moats, financials, and competitive dynamics.',
        tips: [
    ...
    },
    {
        ...
        icon: 'bookmark',
        color: '#10B981',
        description: 'Your watchlist is your shortlist of companies that align with your thesis. Track 8-15 companies you believe in, then narrow down to your top picks.',
        tips: [
    ...
    },
    {
        ...
        icon: 'pie-chart',
        color: '#F59E0B',
        description: 'Position sizing is where most investors fail. Allocate based on conviction, risk tolerance, and correlation between holdings.',
        tips: [
    ...
    },
    {
        ...
        icon: 'refresh-cw',
        color: '#EF4444',
        description: 'A portfolio is never "done." Set a regular review cadence — quarterly at minimum — to assess performance, rebalance positions, and update your thesis.',
        tips: [
...
export default function PortfolioGuideScreen() {
    ...
    const [expandedStep, setExpandedStep] = useState('thesis');
    useEffect(() => {
    ...
    const getStepStatus = (stepId) => {
        ...
        if (stepId === 'research')
            return 'active';
        if (stepId === 'watchlist')
            return watchlistCount > 0 ? 'done' : 'active';
        if (stepId === 'allocation')
            return watchlistCount >= 3 ? 'active' : 'locked';
        if (stepId === 'review')
            return watchlistCount >= 5 ? 'active' : 'locked';
        return 'locked';
    };
    if (loading) {
        ...
        <View style={styles.stepsContainer}>
          {GUIDE_STEPS.map((step, index) => {
            ...
            return (<View key={step.id}>
                ...
                <View style={styles.timelineRow}>
                  ...
                  <View style={styles.timelineCol}>
                    ...
                    styles.timelineDot,
                    {
                      ...
                      {status === 'done' ? (<Text style={styles.dotCheck}>✓</Text>) : (<Text style={styles.dotNumber}>{step.number}</Text>)}
                    </View>
                    {!isLast && (<View style={[
                        styles.timelineLine,
                        {
                            backgroundColor: status === 'done' ? '#22C55E40' : COLORS.border,
                        },
                    ]}/>)}
                  </View>
                  ...
                  <TouchableOpacity style={[
                ...
                ]} onPress={() => setExpandedStep(isExpanded ? null : step.id)} disabled={status === 'locked'} activeOpacity={0.7}>
                    ...
                    {isExpanded && (<View style={styles.stepBody}>
                        <Text style={styles.stepDesc}>{step.description}</Text>

                        <View style={styles.tipsBox}>
                          <Text style={styles.tipsLabel}>Key Actions</Text>
                          {step.tips.map((tip, i) => (<View key={i} style={styles.tipRow}>
                              <View style={[
                            styles.tipBullet,
                        ...
                        {step.action && (<TouchableOpacity style={[
                            styles.actionBtn,
                            { backgroundColor: step.color },
