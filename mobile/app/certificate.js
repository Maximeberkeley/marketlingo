...
function determineSkillAreas(currentDay) {
    const skills = [];
    if (currentDay >= 30)
        skills.push('Industry Fundamentals', 'Supply Chain Dynamics');
    if (currentDay >= 60)
        skills.push('Commercial Aviation', 'Airline Economics');
    if (currentDay >= 90)
        skills.push('Defense Procurement', 'Government Contracting');
    if (currentDay >= 120)
        skills.push('Space Commerce', 'Satellite Systems');
    if (currentDay >= 150)
        skills.push('Emerging Technologies', 'Sustainable Aviation');
    if (currentDay >= 180)
        skills.push('Aerospace Business Strategy', 'Investment Analysis');
...
export default function CertificateScreen() {
    ...
    const handleShare = async () => {
        if (!data)
            return;
        const text = `I just completed the 180-day ${data.marketName} Industry Mastery Program!\n\n` +
    ...
    if (!data)
        return null;
    const STAT_ITEMS = [
        { featherIcon: 'bar-chart-2', value: data.totalXP.toLocaleString(), label: 'XP Earned' },
        { featherIcon: 'book-open', value: data.lessonsCompleted, label: 'Lessons' },
        { featherIcon: 'target', value: data.trainersCompleted, label: 'Scenarios' },
        { featherIcon: 'activity', value: data.longestStreak, label: 'Day Streak' },
    ];
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.certOuter}>
          ...
          <View style={styles.certInner}>
            ...
            <View style={styles.statsGrid}>
              {STAT_ITEMS.map((s, i) => (<View key={i} style={styles.statItem}>
            ...
            <View style={styles.skillsWrap}>
              {data.skillAreas.map((skill, i) => (<View key={i} style={styles.skillChip}>
        ...
        <View style={styles.actionsRow}>
          ...
          <TouchableOpacity style={styles.linkedinButton} onPress={async () => {
            const text = `I just completed the 180-day ${data.marketName} Industry Mastery Program on MarketLingo!\n\n` +
