...
import { ArenaScreen } from '../lesson-kit/arena/ArenaScreen';
...
function ArenaPrerequisite() {
    ...
    return (<View style={styles.prerequisite}>
      <TouchableOpacity activeOpacity={0.92} onPress={togglePose} accessibilityRole="button" accessibilityLabel={`Leo is ${isScholar ? 'ready to study' : 'thinking'}. Tap to change his pose.`}>
      ...
      <TouchableOpacity style={styles.courseButton} activeOpacity={0.88} onPress={() => router.replace('/(tabs)/home')} accessibilityRole="button">
...
export default function ArenaRoute() {
    ...
    const waves = useMemo(() => buildArena({
        marketId,
    ...
    return (<ArenaScreen key={runKey} waves={waves} marketId={marketId} marketName={getMarketName(marketId)} bestScore={rewards.arenaBestScore} onExit={() => router.back()} onFinish={handleFinish}/>);
}
const styles = StyleSheet.create({
    ...
    leoImage: { width: 192, height: 192, resizeMode: 'contain', backfaceVisibility: 'hidden', imageRendering: 'crisp-edges' },
