...
export default function SummariesScreen() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setSummaries((data || []).map((s) => ({ ...s, key_takeaways: Array.isArray(s.key_takeaways) ? s.key_takeaways : null })));
            setLoading(false);
        };
        fetchData();
    }, [user]);
    ...
    const readTime = (c) => Math.max(2, Math.ceil(c.length / 1000));
    // Detail view
    if (selectedSummary) {
        ...
        <ScrollView contentContainerStyle={[styles.detailContent, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
          ...
          {selectedSummary.key_takeaways && selectedSummary.key_takeaways.length > 0 && (<View style={styles.takeawaysCard}>
              <Text style={styles.takeawaysTitle}>Key Takeaways</Text>
              {selectedSummary.key_takeaways.map((t, i) => (<View key={i} style={styles.takeawayRow}>
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.tabs}>
        {['WEEKLY', 'MONTHLY'].map((tab) => (<TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]} onPress={() => setActiveTab(tab)}>
      ...
      {loading ? (<View style={styles.centered}><ActivityIndicator color={COLORS.accent} size="large"/></View>) : filtered.length === 0 ? (<View style={styles.emptyState}>
        ...
        </View>) : (<ScrollView contentContainerStyle={[styles.listContent, { paddingBottom: insets.bottom + 80 }]} showsVerticalScrollIndicator={false}>
          {filtered.map((summary) => {
                const tc = typeColors[summary.summary_type] || typeColors.DAILY;
                return (<TouchableOpacity key={summary.id} style={styles.summaryCard} onPress={() => setSelectedSummary(summary)} activeOpacity={0.8}>
                <View style={{ flex: 1 }}>
                  ...
                  <View style={styles.summaryMeta}>
                    ...
                    {summary.key_takeaways && summary.key_takeaways.length > 0 && (<><Text style={styles.summaryMetaDot}>·</Text><Text style={styles.summaryMetaText}>{summary.key_takeaways.length} takeaways</Text></>)}
                  </View>
                </View>
