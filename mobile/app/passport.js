...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, Share, ActivityIndicator, Image, } from 'react-native';
...
function getGradeFromCompletion(daysCompleted, totalDays) {
    if (daysCompleted === 0)
        return null;
...
export default function PassportScreen() {
    ...
    const fetchPassportData = async () => {
        ...
        const builtStamps = themes.map((theme, i) => {
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[styles.scroll, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.passportCard}>
          <View style={styles.passportHeader}>
            <Image source={require('../assets/illustrations/passport-hero.png')} style={{ width: 48, height: 48 }} resizeMode="contain"/>
            <Text style={styles.passportTitle}>INDUSTRY PASSPORT</Text>
            {marketId && (<View style={styles.marketRow}>
                <Text style={styles.marketLabel}>{getMarketName(marketId)}</Text>
              </View>)}
            {learningGoal && (<View style={styles.goalBadge}>
          ...
          <View style={styles.stampsGrid}>
            ...
            return (<View key={stamp.month} style={[
                    styles.stampCard,
                    stamp.completed
                  ...
                  <Feather name={stamp.completed ? (STAMP_FEATHER[index] || 'award') : stamp.grade !== null ? 'book-open' : 'bar-chart-2'} size={22} color={stamp.completed ? COLORS.accent : COLORS.textMuted} style={{ opacity: stamp.completed ? 1 : stamp.grade !== null ? 0.7 : 0.3 }}/>
                  <Text style={styles.stampTheme} numberOfLines={1}>{stamp.theme}</Text>
                  <Text style={styles.stampMonth}>Month {stamp.month}</Text>
                  {stamp.grade ? (<View style={[styles.gradeBadge, { backgroundColor: gc.bg, borderColor: gc.border }]}>
                      <Text style={[styles.gradeText, { color: gc.text }]}>{stamp.grade}</Text>
                    </View>) : (<View style={styles.lockedRow}>
                  ...
                  {stamp.completedDate && (<Text style={styles.completedDate}>{stamp.completedDate}</Text>)}
                  {stamp.completed && (<View style={styles.checkBadge}>
...
const styles = StyleSheet.create({
    ...
    stampCard: { width: '47%', padding: 16, borderRadius: 16, borderWidth: 2, alignItems: 'center', position: 'relative' },
