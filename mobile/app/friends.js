...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, TextInput, Alert, ActivityIndicator, Animated, Share, Image, } from 'react-native';
...
export default function FriendsScreen() {
    ...
    const [myStats, setMyStats] = useState({ xp: 0, level: 1, streak: 0, monthXP: 0 });
    const [friendMonthXP, setFriendMonthXP] = useState({});
    // Animations
    ...
    const fetchGlobalLeaderboard = useCallback(async () => {
        ...
        try {
            ...
            const entries = standings.map((row, idx) => ({
                rank: idx + 1,
                user_id: row.user_id,
    ...
    return (<View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        {pendingRequests.length > 0 && (<TouchableOpacity style={styles.requestsBadge} onPress={() => {
                pendingRequests.forEach(r => {
                    Alert.alert(`${r.fromUsername} wants to be friends`, 'Accept request?', [
      ...
      <View style={styles.tabRow}>
        {['friends', 'global'].map(tab => (<TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]} onPress={() => { triggerHaptic('light'); setActiveTab(tab); }}>
            <Feather name={tab === 'friends' ? 'users' : 'globe'} size={14} color={activeTab === tab ? '#FFF' : COLORS.textMuted}/>
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: fadeAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }}>
          ...
          {activeTab === 'friends' && (<>
              ...
              {/* Add Friend Toggle */}
              <TouchableOpacity style={styles.addFriendToggle} onPress={() => { triggerHaptic('light'); setShowAddInput(!showAddInput); }}>
                <Feather name="user-plus" size={16} color={COLORS.accent}/>
                <Text style={styles.addFriendToggleText}>Add friend</Text>
              </TouchableOpacity>

              {showAddInput && (<View style={styles.addRow}>
                  <TextInput style={styles.addInput} placeholder="Username or email..." placeholderTextColor={COLORS.textMuted} value={addUsername} onChangeText={setAddUsername} autoCapitalize="none" autoFocus returnKeyType="send" onSubmitEditing={handleAddFriend}/>
                  <TouchableOpacity style={[styles.sendBtn, !addUsername.trim() && { opacity: 0.4 }]} onPress={handleAddFriend} disabled={!addUsername.trim() || adding}>
                    {adding ? <ActivityIndicator color="#FFF" size="small"/> : <Feather name="send" size={16} color="#FFF"/>}
                  </TouchableOpacity>
                </View>)}

              {loading ? (<ActivityIndicator color={COLORS.accent} size="large" style={{ marginTop: 60 }}/>) : friendsError ? (<View style={styles.emptyState}>
                ...
                </View>) : friends.length === 0 ? (<View style={styles.emptyState}>
                  ...
                  {friendsBoard.map((row, idx) => (<BoardRow key={row.id} rank={idx + 1} name={row.name} initial={row.initial} xp={row.xp} meta={`Lv.${row.level}${row.streak > 0 ? ` · ${row.streak}d streak` : ''}${row.weekXP > 0 ? ` · ${row.weekXP} this month` : ''}`} isMe={row.isMe} online={row.friend ? isActive(row.friend) : false} onNudge={row.friend ? () => handleNudge(row.friend) : undefined} onMore={row.friend ? () => handleRemove(row.friend) : undefined}/>))}
                </View>)}
            </>)}
          ...
          {activeTab === 'global' && (<>
              ...
              {!globalLoading && globalEntries.length > 0 && (<View style={styles.heroCard}>
                  <Image source={SOCIAL_HERO} style={styles.heroLeo} resizeMode="cover"/>
                  <View style={{ flex: 1 }}>
              ...
              {/* Podium */}
              {!globalLoading && globalEntries.length >= 3 && (<View style={styles.podium}>
                  {[1, 0, 2].map((i) => {
                    const e = globalEntries[i];
                    const m = MEDALS[i];
                    const h = i === 0 ? 76 : i === 1 ? 60 : 50;
                    return (<View key={e.user_id} style={styles.podiumCol}>
              ...
              {globalLoading ? (<ActivityIndicator color={COLORS.accent} size="large" style={{ marginTop: 60 }}/>) : globalError ? (<View style={styles.emptyState}>
                  <Feather name="wifi-off" size={32} color={COLORS.textMuted}/>
                ...
                </View>) : globalEntries.length === 0 ? (<View style={styles.emptyState}>
                  <Image source={LEO_SASSY} style={styles.emptyLeo} resizeMode="contain"/>
                  ...
                  {globalEntries.map((entry) => (<BoardRow key={entry.user_id} rank={entry.rank} name={entry.isCurrentUser ? 'You' : entry.username} initial={entry.username.charAt(0).toUpperCase()} xp={entry.total_xp} meta={`Lv.${entry.current_level}${entry.current_streak > 0 ? ` · ${entry.current_streak}d streak` : ''}`} isMe={entry.isCurrentUser}/>))}
                </View>)}
            </>)}
...
// ── Board Row ───────────────────────────────────────
function BoardRow({ rank, name, initial, xp, meta, isMe, online, onNudge, onMore, }) {
    const medal = rank <= 3 ? MEDALS[rank - 1] : null;
    return (<View style={[styles.row, isMe && styles.rowSelf, medal && { borderColor: medal.ring + '45' }]}>
      ...
      {onNudge && (<TouchableOpacity style={styles.nudgeBtn} onPress={onNudge}>
          <Feather name="send" size={14} color={COLORS.accent}/>
        </TouchableOpacity>)}
      {onMore && (<TouchableOpacity style={styles.moreBtn} onPress={onMore} onLongPress={onMore}>
