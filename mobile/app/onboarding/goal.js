...
export default function GoalScreen() {
    ...
    const handleContinue = async () => {
        if (!selectedGoal || !selectedMarket)
            return;
        if (!user) {
        ...
        try {
            ...
            await supabase.from('user_progress').upsert({ user_id: user.id, market_id: selectedMarket, learning_goal: selectedGoal }, { onConflict: 'user_id,market_id' });
    ...
    return (<View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <Animated.ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
        ...
        {/* Leo reaction */}
        {selectedGoal && (<Animated.View style={[styles.reactionBubble, { opacity: reactionOpacity }]}>
        ...
        <View style={styles.cardsContainer}>
          {goalOptions.map((goal, index) => {
            ...
            return (<Animated.View key={goal.id} style={{
                    opacity: fadeAnim,
                    transform: [{
                ...
                <TouchableOpacity style={[styles.card, isSelected && styles.cardSelected]} onPress={() => handleSelect(goal.id)} activeOpacity={0.7}>
                  <View style={styles.cardRow}>
                    ...
                    <View style={styles.cardContent}>
                      ...
                      <View style={styles.chipRow}>
                        {goal.emphasis.map((item) => (<View key={item} style={styles.chip}>
        ...
        {selectedGoal && (<View style={styles.destinationBox}>
      ...
      <StickyBottomCTA title={isSubmitting ? 'Saving...' : 'Commit to my goal'} onPress={handleContinue} disabled={!selectedGoal || isSubmitting}/>
    </View>);
}
