...
import { View, Text, TouchableOpacity, StyleSheet, TextInput, ActivityIndicator, Animated, Dimensions, Image, } from 'react-native';
...
export default function OnboardingScreen() {
    ...
    const [demoMarket, setDemoMarket] = useState(null);
    ...
    const handleContinue = async () => {
        if (!selectedIndustry || isSubmitting)
            return;
    ...
    return (<View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <Animated.ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
        ...
        {/* Demo bridge banner */}
        {demoMarket && demoXP > 0 && (<View style={styles.demoBanner}>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search industries..." placeholderTextColor={COLORS.textMuted} value={searchQuery} onChangeText={setSearchQuery}/>
        </View>
        ...
        <View style={styles.gridContainer}>
          {filteredMarkets.map((market, index) => {
            ...
            return (<Animated.View key={market.id} style={{
                ...
                <TouchableOpacity style={[
                ...
                ]} onPress={() => handleSelect(market.id)} activeOpacity={0.7}>
                  {illustration ? (<Image source={illustration} style={styles.gridIllustration} resizeMode="contain"/>) : (<View style={[styles.gridIconFallback, { backgroundColor: market.color + '20' }]}>
                  ...
                  {selectedIndustry === market.id && (<View style={[styles.gridCheck, { backgroundColor: market.color }]}>
      ...
      <Animated.View style={[
        ...
        {selectedMarketData && (<View style={styles.selectedPreview}>
            <View style={styles.selectedPreviewRow}>
              {MARKET_ILLUSTRATIONS[selectedMarketData.id] && (<Image source={MARKET_ILLUSTRATIONS[selectedMarketData.id]} style={{ width: 24, height: 24, resizeMode: 'contain' }}/>)}
        ...
        ]} onPress={handleContinue} disabled={!selectedIndustry || isSubmitting} activeOpacity={0.85}>
          {isSubmitting ? (<ActivityIndicator color="#fff"/>) : (<Text style={styles.ctaButtonText}>
