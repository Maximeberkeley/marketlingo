...
function HomeScreen() {
  ...
  const {
    ...
    addXP,
    refetch: refetchXP
  } = useUserXP(selectedMarketLocal || void 0);
  ...
  const { evaluateRewards } = useCollectibles(selectedMarket || void 0);
  const { canFreeze, freezesUsedThisWeek, maxFreezes, useFreeze } = useStreakFreeze(
    selectedMarketLocal || void 0,
    isProUser
  );
  const league = useLeague(selectedMarketLocal || void 0);
  const recap = useWeeklyRecap(selectedMarketLocal || void 0);
  ...
  const { dueCount } = useSpacedRepetition(selectedMarketLocal || void 0);
  const deliverable = useDeliverable(selectedMarketLocal || void 0, learningGoal);
  const focusTopic = useFocusTopic(selectedMarketLocal || void 0, currentDay || 1);
  const intelHabit = useIntelHabit(selectedMarketLocal || void 0);
  const { syncLessons } = useOfflineCache(selectedMarketLocal || void 0);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: stack } = await supabase.from("stacks").select("id, title, stack_type, tags, duration_minutes, metadata, slides (id, slide_number, title, body, sources)").eq("id", openStackId).not("published_at", "is", null).single();
      if (stack && stack.slides?.length > 0) {
        const formatted = {
          ...stack,
          tags: stack.tags || [],
          slides: (stack.slides || []).sort((a, b) => a.slide_number - b.slide_number).map((s) => ({
            ...s,
            sources: Array.isArray(s.sources) ? s.sources.map((src) => typeof src === "string" ? { label: "Source", url: src } : src).filter(Boolean) : []
          }))
        };
  ...
  const leoPopups = useLeoPopups({ cooldownMs: 45e3, maxPerSession: 4, displayName });
  ...
  /> : session.showReader && session.activeStack ? <SlideReader
    ...
    onComplete={session.activeBiteIndex !== null ? session.handleBiteComplete : async (reviewMode, timeSpentSeconds, accuracy) => {
    ...
    metadata={session.activeStack.metadata}
  /> : session.showSessionComplete ? <SessionCompleteCard
  ...
  /> : selectedMarket ? <CourseJourney
    marketId={selectedMarket}
    currentDay={currentDay}
    learningGoal={learningGoal}
    completedStackIds={progress?.completed_stacks || []}
    streak={streak}
    totalXp={xpData?.total_xp || 0}
    level={xpData?.current_level || 1}
  ...
  />
      ...
      {league.lastWeek && <LeagueCeremonyModal
