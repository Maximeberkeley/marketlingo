...
function getRandomGreeting(key) {
...
export default function HomeScreen() {
    ...
    const { xpData, dailyCompletion, completeLessonForToday, getCurrentStage, getProgressToNextStage, isLessonCompletedToday, addXP, refetch: refetchXP, } = useUserXP(selectedMarketLocal || undefined);
    const lessonCompletedToday = isLessonCompletedToday();
    /** Day number a stack belongs to, from its own `day-N` tag. */
    const stackDayNumber = (stack) => {
        const tag = (stack?.tags || []).find((t) => t.startsWith('day-'));
    ...
    const { selectedMarket, isProUser, lessonStack, newsStack, newsItems, streakRiskHours, socialNudge, tomorrowLesson, loading, refreshing, currentDay, learningGoal, fetchData, onRefresh, } = homeData;
    const { evaluateRewards } = useCollectibles(selectedMarket || undefined);
    const { canFreeze, freezesUsedThisWeek, maxFreezes, useFreeze } = useStreakFreeze(selectedMarketLocal || undefined, isProUser);
    ...
    useEffect(() => {
        if (selectedMarket)
            setSelectedMarketLocal(selectedMarket);
    }, [selectedMarket]);
    ...
    useEffect(() => {
        if (currentDay && selectedMarket)
            syncLessons(currentDay);
    ...
    const openStackHandled = useRef(null);
    useEffect(() => {
        if (!openStackId || !selectedMarket || !user || session.showReader)
            return;
        if (openStackHandled.current === openStackId)
            return;
    ...
    const criticalTimerActive = (() => {
        if (!progress?.streak_expires_at || streak === 0 || lessonCompletedToday)
            return false;
        ...
        if (lessonCompletedToday) {
            ...
            if (!previousLessonComplete.current) {
                void claimLeoNudge('completion').then(claimed => {
                    if (claimed)
                        leoPopups.triggerCompletionNod(() => leoPopups.dismiss());
                });
            }
        }
    ...
    useEffect(() => {
        if (loading || authLoading || hasTriggeredWelcome.current)
            return;
        if (!selectedMarket || !user)
            return;
        if (!courseFocused || session.showGoals || session.showReader || session.showSessionComplete)
            return;
    ...
    useEffect(() => {
        if (session.showGoals || session.showReader || lessonCompletedToday)
            leoPopups.clear();
    ...
    const [greeting] = useState(() => {
        ...
        if (lessonCompletedToday)
            return getRandomGreeting('completed');
        if (hour < 12)
    ...
    return (<View style={styles.container}>
      ...
      <WelcomeBackModal visible={returnVisit.isReturningVisit && !session.showReader} level={xpData?.current_level || 1} xp={xpData?.total_xp || 0} xpToNextLevel={xpData?.xp_to_next_level} streak={streak} marketId={selectedMarket || 'aerospace'} marketName={getMarketName(selectedMarket || 'aerospace')} onClose={returnVisit.dismiss}/>
      {/* Leo voice chat — fullscreen immersive */}
      <LeoVoiceChatOverlay visible={showLeoChat} onClose={() => setShowLeoChat(false)} marketId={selectedMarket || undefined} lessonContext={`${getMarketName(selectedMarket || 'aerospace')} industry learning — Day ${currentDay}`}/>
      <CardRevealModal card={revealedCard} marketId={selectedMarket || undefined} onClose={() => setRevealedCard(null)}/>

      {session.showGoals && session.activeStack ? (<LessonGoalsScreen title={session.activeStack.title} slides={session.activeStack.slides} objectives={session.activeStack.metadata?.learning_objectives} marketId={selectedMarket || undefined} day={currentDay} isBite={session.activeBiteIndex !== null} onStart={session.beginLesson} onBack={session.closeReader}/>) : session.showReader && session.activeStack ? (<SlideReader stackTitle={session.activeStack.title} stackType={session.activeStack.stack_type} slides={session.activeStack.slides.map((s) => ({
                slideNumber: s.slide_number, title: s.title, body: s.body, sources: s.sources,
            }))} onClose={session.closeReader} onComplete={session.activeBiteIndex !== null
                ...
                : async (reviewMode, timeSpentSeconds, accuracy) => {
                    ...
                    if (!synced || reviewMode || !selectedMarket || !session.activeStack?.id)
                        return;
                    const rewards = await evaluateRewards('lesson', `lesson:${selectedMarket}:${session.activeStack.id}`, accuracy);
                    if (rewards?.collectibles?.[0])
                        setRevealedCard(rewards.collectibles[0]);
                }} onSaveInsight={session.handleSaveInsight} onAddNote={session.handleAddNote} marketId={selectedMarket || undefined} stackId={session.activeStack.id} learningGoal={learningGoal} 
        ...
        isReview={(() => {
            ...
            })()} isProUser={isProUser} streakDays={streak} dayNumber={stackDayNumber(session.activeStack) || currentDay} metadata={session.activeStack.metadata}/>) : session.showSessionComplete ? (<SessionCompleteCard dayNumber={currentDay} marketName={getMarketName(selectedMarket || 'aerospace')} marketEmoji="" xpEarned={session.sessionXPEarned} streak={streak} lessonTitle={session.activeStack?.title || lessonStack?.title || 'Lesson'} totalXP={xpData?.total_xp || 0} stageName={currentStage.name} onContinue={() => {
                session.dismissSessionComplete();
            }} onDismiss={() => {
                session.dismissSessionComplete();
            }}/>) : selectedMarket ? (<CourseJourney marketId={selectedMarket} currentDay={currentDay} learningGoal={learningGoal} completedStackIds={progress?.completed_stacks || []} streak={streak} totalXp={xpData?.total_xp || 0} level={xpData?.current_level || 1} lessonCompletedToday={lessonCompletedToday} arenaCompletedToday={(dailyCompletion?.drills_completed || 0) > 0} caseCompletedToday={(dailyCompletion?.games_completed || 0) > 0} intelReadToday={intelHabit.readToday} intelTarget={intelHabit.target} rescueAvailable={showStreakWarning || criticalTimerActive} safeTop={insets.top} onOpenLesson={(stackId) => router.setParams({ openStackId: stackId })} onAskLeo={() => setShowLeoChat(true)}/>) : (<HomeSkeleton />)}

      <MilestoneShareCard visible={milestone.visible} type={milestone.type} data={milestone.data} onDismiss={dismissMilestone}/>

      <AchievementPopup visible={!!achievementPopup} achievement={achievementPopup} onDismiss={() => setAchievementPopup(null)}/>

      {league.lastWeek && (<LeagueCeremonyModal visible={league.ceremonyPending && !session.showReader} tier={league.lastWeek.tier} result={league.lastWeek.result} finalRank={league.lastWeek.finalRank} onClose={league.dismissCeremony} onViewLeague={() => { league.dismissCeremony(); router.push('/league'); }}/>)}
    </View>);
}
