...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, Animated, ActivityIndicator, Dimensions, ImageBackground, } from 'react-native';
...
const ACTIVITY_CARDS = [
    {
        id: 'arena',
        title: 'Daily Arena',
        subtitle: 'Three Waves. One Clock.',
        description: 'Fast industry calls with combos, shields, and a sudden-death finish.',
        icon: 'zap',
        iconColor: '#FDE68A',
        gradientColors: ['#92400E', '#B45309', '#D97706'],
    ...
    },
    {
        id: 'deep-case',
        title: 'Deep Case',
        subtitle: 'Four Stages. One Verdict.',
        description: 'Study the evidence, read the numbers, and defend your final call.',
        icon: 'briefcase',
        iconColor: '#C7D2FE',
...
const LAB_CARDS = [
    {
        ...
        description: 'Mock interviews with focused coaching and realistic pressure.',
        icon: 'mic',
        iconColor: '#C4B5FD',
        gradientColors: ['#1E1B4B', '#4338CA', '#7C3AED'],
...
const COMPETE_CARDS = [
    {
        ...
        icon: 'award',
        iconColor: '#BFDBFE',
        gradientColors: ['#1E3A8A', '#1D4ED8', '#3B82F6'],
    ...
    },
    {
        ...
        icon: 'users',
        iconColor: '#FDE68A',
        gradientColors: ['#7C2D12', '#B45309', '#F59E0B'],
...
/* ─── Premium Carousel ─── */
function PremiumCarousel({ cards, title }) {
    ...
    return (<View style={styles.carouselWrap}>
      {/* Section header */}
      <View style={styles.sectionHeader}>
        ...
        <View style={styles.pageIndicator}>
          {cards.map((_, i) => (<TouchableOpacity key={i} onPress={() => scrollToIndex(i)} hitSlop={{ top: 10, bottom: 10, left: 4, right: 4 }}>
              <Animated.View style={[
                styles.indicatorDot,
      ...
      {/* Cards */}
      <ScrollView ref={scrollRef} horizontal pagingEnabled={false} showsHorizontalScrollIndicator={false} snapToInterval={SNAP_WIDTH} decelerationRate="fast" contentContainerStyle={styles.scrollContent} onMomentumScrollEnd={handleScroll}>
        {cards.map((card, index) => (<PremiumCard key={card.id} card={card} index={index}/>))}
      </ScrollView>
    </View>);
}
/* ─── Premium Card ─── */
function PremiumCard({ card, index }) {
    ...
    const cardContent = (<View style={styles.cardInner}>
      {/* Top section: hero image full-bleed */}
      {card.heroImage ? (<ImageBackground source={card.heroImage} style={styles.heroImageBg} imageStyle={styles.heroImageStyle} resizeMode="cover">
          {/* Dark gradient overlay for readability */}
          <View style={styles.heroOverlay}/>
          
          {/* Floating tag */}
          {card.tag && (<View style={styles.floatingTag}>
              <Text style={styles.floatingTagText}>{card.tag}</Text>
            </View>)}
        </ImageBackground>) : (<View style={[styles.heroPlaceholder, { backgroundColor: card.gradientColors[1] }]}>
      ...
      <View style={styles.contentPanel}>
        ...
        <View style={styles.ctaRow}>
          ...
          <View style={[styles.ctaArrow, { backgroundColor: `${card.gradientColors[1]}12` }]}>
            <Feather name="arrow-right" size={14} color={card.gradientColors[1]}/>
          </View>
        </View>
      </View>
    ...
    return (<Animated.View style={[styles.cardOuter, { transform: [{ scale: scaleAnim }] }]}>
      <TouchableOpacity activeOpacity={0.92} onPressIn={onPressIn} onPressOut={onPressOut} onPress={onPress} style={styles.cardTouch}>
        {cardContent}
      </TouchableOpacity>
    </Animated.View>);
}
...
export default function PracticeScreen() {
    ...
    useEffect(() => {
        ...
        supabase
            ...
            .single()
            .then(({ data }) => {
            if (data?.selected_market)
                setSelectedMarket(data.selected_market);
            setLoading(false);
        });
    }, [user]);
    useEffect(() => {
      ...
      <ScrollView contentContainerStyle={{
        ...
        <Animated.View style={[
          ...
          <View>
            ...
            {selectedMarket && (<View style={styles.marketBadge}>
        ...
        <View style={styles.groundedBand}>
          ...
          <View style={styles.groundedCopy}>
            ...
            {studied.lessons.length === 0 && !studied.isLoading && (<Text style={styles.groundedBody}>
