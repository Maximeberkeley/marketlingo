...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, TextInput, ActivityIndicator, Modal, Alert, KeyboardAvoidingView, Platform, Animated, Image, Dimensions, } from 'react-native';
...
/* ─── Helpers ─── */
function getLinkedType(label) {
    if (!label)
        return 'lesson';
    const lower = label.toLowerCase();
    if (lower.includes('interview'))
        return 'interview';
    if (lower.includes('news') || lower.includes('daily'))
        return 'news';
...
function NoteCard({ note, onDelete, onOpen }) {
    ...
    return (<Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
      <TouchableOpacity style={styles.noteCard} activeOpacity={0.9} onPressIn={onPressIn} onPressOut={onPressOut} onPress={() => onOpen(note)} onLongPress={() => {
...
/* ─── Streak Week View ─── */
function StreakWeekView({ streak, thisWeek, totalNotes }) {
    const dayLabels = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
    const today = new Date().getDay();
    return (<View style={styles.streakCard}>
      ...
      <View style={styles.weekRow}>
        {dayLabels.map((label, i) => {
            ...
            return (<View key={i} style={styles.weekDay}>
...
export default function NotebookScreen() {
    ...
    const handleAddNote = async () => {
        if (!user || !newNoteContent.trim() || !selectedMarket)
            return;
    ...
    // Category counts
    const categoryCounts = { all: notes.length };
    notes.forEach((n) => {
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[
        ...
        <Animated.View style={[
          ...
          <TouchableOpacity style={styles.searchToggle} onPress={() => setShowSearch((s) => !s)}>
        ...
        {showSearch && (<View style={styles.searchContainer}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search your notes" placeholderTextColor={COLORS.textMuted} value={searchQuery} onChangeText={setSearchQuery} autoFocus/>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryRow} contentContainerStyle={{ gap: 8, paddingRight: 16 }}>
          {CATEGORIES.map((cat) => {
            ...
            return (<TouchableOpacity key={cat.id} style={[styles.categoryChip, isActive && { backgroundColor: cat.color + '18', borderColor: cat.color + '40' }]} onPress={() => {
                    triggerHaptic('light');
                ...
                {count > 0 && (<View style={[styles.categoryCount, isActive && { backgroundColor: cat.color + '20' }]}>
        ...
        {/* ── Search Term Chips ── */}
        {searchTerms.length > 1 && (<View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
            {searchTerms.map((term, i) => {
                const chipColors = ['#EAB308', '#06B6D4', '#EC4899', '#22C55E', '#F97316'];
                const c = chipColors[i % chipColors.length];
                return (<View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: c + '20', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: c + '40' }}>
        ...
        {/* ── Notes List ── */}
        {Object.keys(groupedNotes).length > 0 ? (<View style={{ gap: 20 }}>
            {Object.entries(groupedNotes).map(([date, dateNotes]) => (<View key={date}>
                ...
                <View style={{ gap: 10 }}>
                  {dateNotes.map((note) => (<NoteCard key={note.id} note={note} onDelete={handleDeleteNote} onOpen={(n) => { setOpenNote(n); setEditingContent(n.content); }}/>))}
                </View>
              </View>))}
          </View>) : (<View style={styles.emptyState}>
            <Image source={require('../../assets/cards/notebook-hero.png')} style={styles.emptyImage} resizeMode="contain"/>
            ...
            {!searchQuery && (<TouchableOpacity style={styles.emptyCta} onPress={() => {
      ...
      {/* ── FAB ── */}
      {notes.length > 0 && (<TouchableOpacity style={[styles.fab, { bottom: insets.bottom + 80 }]} onPress={() => {
                triggerHaptic('medium');
                setShowAddNote(true);
            }} activeOpacity={0.85}>
      ...
      <Modal visible={!!openNote} transparent animationType="slide">
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setOpenNote(null)}>
            ...
            setOpenNote(null);
              ...
              <ScrollView style={styles.detailScroll} nestedScrollEnabled>
                <TextInput style={styles.detailInput} value={editingContent} onChangeText={setEditingContent} multiline textAlignVertical="top" placeholder="Your note..." placeholderTextColor={COLORS.textMuted}/>
              </ScrollView>

              {/* Save / Close */}
              <View style={styles.modalActions}>
                <TouchableOpacity style={styles.modalCancel} onPress={() => setOpenNote(null)}>
                  <Text style={styles.modalCancelText}>Close</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.modalSave, editingContent === openNote?.content && { opacity: 0.4 }]} disabled={editingContent === openNote?.content} onPress={async () => {
            if (!openNote || !user)
                return;
            const { error } = await supabase
                ...
                .eq('user_id', user.id);
            if (!error) {
                setNotes((prev) => prev.map((n) => n.id === openNote.id ? { ...n, content: editingContent.trim() } : n));
                triggerHaptic('success');
            }
            setOpenNote(null);
        }}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => {
            ...
            <TouchableOpacity activeOpacity={1} style={styles.modalCard}>
              ...
              <TextInput style={styles.modalInput} placeholder="Write your insight, takeaway, or idea..." placeholderTextColor={COLORS.textMuted} value={newNoteContent} onChangeText={setNewNoteContent} multiline autoFocus textAlignVertical="top"/>
            ...
            setNewNoteContent('');
                ...
                <TouchableOpacity style={[styles.modalSave, !newNoteContent.trim() && { opacity: 0.4 }]} onPress={handleAddNote} disabled={!newNoteContent.trim()}>
                  <Feather name="check" size={16} color="#fff"/>
