...
export default function InvestmentModuleScreen() {
    ...
    const { moduleId = 'valuation' } = useLocalSearchParams();
    ...
    useEffect(() => {
        if (!lab.loading && scenarios.length)
            setIndex(lab.getResumeIndex(scenarios));
    }, [lab.loading, scenarios.length]);
    const choose = (choice) => { setSelected(choice); triggerHaptic('light'); };
    const commit = async () => {
        if (!scenario || selected === null || saving)
            return;
        setSaving(true);
        try {
            ...
            if (correct)
                await lab.addLearnedConcept(scenario.tags[0] || config.title);
            triggerHaptic(correct ? 'success' : 'error');
            setPhase('verdict');
        }
        catch (error) {
            log.warn('[Investment] Could not save decision:', error);
        }
        finally {
            setSaving(false);
        }
    ...
    const finish = async () => {
        ...
        try {
            ...
            log.warn('[Investment] Rewards failed:', error);
        }
...
function Coach({ text }) { return <View style={styles.coach}><Image source={SOPHIA} style={styles.coachImage}/><Text style={styles.coachText}>{text}</Text></View>; }
...
function Debrief({ label, body, color }) { return <View style={[styles.debrief, { borderLeftColor: color }]}><Text style={[styles.debriefLabel, { color }]}>{label}</Text><Text style={styles.debriefText}>{body}</Text></View>; }
