...
export default function LegalScreen() {
    ...
    const [activeTab, setActiveTab] = useState(params.type === 'terms' ? 'terms' : 'privacy');
    ...
    const renderContent = (text) => {
        return text.split('\n').map((line, i) => {
            ...
            if (trimmed.startsWith('**') && trimmed.endsWith('**')) {
                return (<Text key={i} style={styles.heading}>
            {trimmed.replace(/\*\*/g, '')}
          </Text>);
            }
            if (trimmed.startsWith('•')) {
                return (<Text key={i} style={styles.bullet}>
            {trimmed}
          </Text>);
            }
            return (<Text key={i} style={styles.paragraph}>
          {trimmed}
        </Text>);
        });
    };
