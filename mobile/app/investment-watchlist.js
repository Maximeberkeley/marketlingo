...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, ActivityIndicator, Alert, Image, TextInput, Modal, } from 'react-native';
...
export default function InvestmentWatchlistScreen() {
    ...
    const browsableCompanies = useMemo(() => {
        ...
        return filtered.filter((c) => c.name.toLowerCase().includes(q) || (c.ticker && c.ticker.toLowerCase().includes(q)) || c.segment.toLowerCase().includes(q));
    }, [allCompanies, watchlistIds, browseSearch]);
    const handleRemove = (companyId, companyName) => {
    ...
    const findFullCompany = (id) => allCompanies.find((c) => c.id === id);
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        {/* Stats bar */}
        {watchlist.length > 0 && (<View style={styles.statsBar}>
            ...
            <View style={styles.statDivider}/>
            <View style={styles.statItem}>
              <Text style={styles.statNum}>{new Set(watchlist.map((c) => c.segment).filter(Boolean)).size || '—'}</Text>
              <Text style={styles.statLabel}>Segments</Text>
            </View>
          </View>)}

        {/* Tabs */}
        {watchlist.length > 0 && (<View style={styles.tabRow}>
            <TouchableOpacity style={[styles.tab, activeTab === 'companies' && styles.tabActive]} onPress={() => setActiveTab('companies')}>
        ...
        {activeTab === 'companies' && (<>
            ...
            {showBrowse && (<View style={styles.browsePanel}>
                <View style={styles.searchBar}>
                  <Feather name="search" size={14} color={COLORS.textMuted}/>
                  <TextInput style={styles.searchInput} placeholder="Search by name, ticker, or segment..." placeholderTextColor={COLORS.textMuted} value={browseSearch} onChangeText={setBrowseSearch}/>
                  {browseSearch.length > 0 && (<TouchableOpacity onPress={() => setBrowseSearch('')}>
                ...
                <ScrollView style={{ maxHeight: 280 }} showsVerticalScrollIndicator={false} nestedScrollEnabled>
                  {browsableCompanies.map((company) => {
                    ...
                    return (<TouchableOpacity key={company.id} style={styles.browseItem} onPress={() => handleAdd({ id: company.id, name: company.name, ticker: company.ticker, segment: company.segment })}>
                        <View style={styles.browseItemLogo}>
                          {company.logoUrl ? (<Image source={{ uri: company.logoUrl }} style={{ width: 24, height: 24 }} resizeMode="contain"/>) : (<Text style={{ fontSize: 12, fontWeight: '700', color: COLORS.textMuted }}>{company.name.charAt(0)}</Text>)}
                        </View>
                        <View style={{ flex: 1 }}>
                  ...
                  {browsableCompanies.length === 0 && (<Text style={styles.emptySearch}>
            ...
            {dueForReview.length > 0 && (<TouchableOpacity style={styles.reviewBanner} onPress={() => setShowReviewOverlay(true)}>
            ...
            {/* Tracked Companies */}
            {watchlist.length > 0 ? (<View style={{ gap: 8, marginTop: 16 }}>
                ...
                {watchlist.map((company) => {
                    ...
                    return (<View key={company.id}>
                      <TouchableOpacity style={styles.companyCard} onPress={() => fullCompany && setSelectedCompany(fullCompany)} activeOpacity={fullCompany ? 0.7 : 1}>
                        <View style={styles.companyLogo}>
                          {fullCompany?.logoUrl ? (<Image source={{ uri: fullCompany.logoUrl }} style={{ width: 28, height: 28 }} resizeMode="contain"/>) : (<Feather name="globe" size={16} color={COLORS.accent}/>)}
                        </View>
                        <View style={{ flex: 1 }}>
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 4 }}>
                            {segStyle && company.segment && (<View style={[styles.miniSegment, { backgroundColor: segStyle.bg }]}>
                                <Text style={[styles.miniSegmentText, { color: segStyle.text }]}>{company.segment}</Text>
                              </View>)}
                            {company.addedAt && (<Text style={styles.addedDate}>Added {formatDate(company.addedAt)}</Text>)}
                            {newsCount > 0 && (<View style={styles.newsCountBadge}>
                                <Feather name="zap" size={9} color="#F59E0B"/>
                                <Text style={styles.newsCountText}>{newsCount}</Text>
                              </View>)}
                          </View>
                        </View>
                        <TouchableOpacity style={styles.removeBtn} onPress={() => handleRemove(company.id, company.name)}>
                          <Feather name="x" size={14} color="#EF4444"/>
                        </TouchableOpacity>
                      </TouchableOpacity>

                      {/* Thesis row */}
                      {thesis ? (<View style={styles.thesisRow}>
                          <Feather name="file-text" size={12} color={COLORS.accent}/>
                          <Text style={styles.thesisText} numberOfLines={2}>{thesis.thesis}</Text>
                          {isDue && (<View style={{ flexDirection: 'row', gap: 4 }}>
                          ...
                          {!isDue && thesis.still_valid !== null && (<View style={[styles.validityBadge, { backgroundColor: thesis.still_valid ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)' }]}>
                              <Feather name={thesis.still_valid ? 'check' : 'x'} size={10} color={thesis.still_valid ? '#22C55E' : '#EF4444'}/>
                            </View>)}
                        </View>) : (<TouchableOpacity style={styles.addThesisBtn} onPress={() => { setThesisText(''); setThesisModal({ companyId: company.id, companyName: company.name, ticker: company.ticker }); }}>
        ...
        {/* News Tab */}
        {activeTab === 'news' && (<View style={{ gap: 8, marginTop: 8 }}>
            {newsLoading ? (<ActivityIndicator size="small" color={COLORS.accent} style={{ marginTop: 40 }}/>) : relevantNews.length > 0 ? (<>
                <Text style={styles.sectionTitle}>NEWS MENTIONING YOUR COMPANIES</Text>
                {relevantNews.map((item) => (<View key={item.id} style={styles.newsCard}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 6 }}>
                      ...
                      {item.category_tag && (<View style={styles.newsCategoryBadge}>
        ...
        {watchlist.length >= 3 && (<TouchableOpacity style={styles.portfolioBtn} onPress={() => router.push('/portfolio-builder')}>
      ...
      <Modal visible={!!thesisModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.thesisInput} placeholder="e.g. Strong market position in EV charging, expecting 40% revenue growth..." placeholderTextColor={COLORS.textMuted} value={thesisText} onChangeText={setThesisText} multiline numberOfLines={4} textAlignVertical="top"/>
      ...
      <CompanyDetailModal company={selectedCompany} onClose={() => setSelectedCompany(null)} isOnWatchlist={selectedCompany ? watchlistIds.has(selectedCompany.id) : false} onToggleWatchlist={handleToggle}/>

      {/* Thesis Review Overlay */}
      <ThesisReviewOverlay visible={showReviewOverlay} onClose={() => setShowReviewOverlay(false)} dueTheses={dueForReview} onReview={reviewThesis} onUpdateThesis={saveThesis}/>
    </View>);
}
