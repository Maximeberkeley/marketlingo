...
export default function AdminContentScreen() {
    ...
    const loadMarketStatuses = async () => {
        ...
        data?.forEach((stack) => {
            ...
            if (!statusMap.has(stack.market_id))
                statusMap.set(stack.market_id, { count: 0, maxDay: 0 });
    ...
    const getMarketStatus = (id) => marketStatuses.find((s) => s.market_id === id) || { total_lessons: 0, max_day: 0 };
    const fetchPlan = async (marketId, month) => {
        setLoading(true);
        try {
            const body = { marketId, month, dryRun: true };
            if (selectedGoal !== 'all')
    ...
    const generateContent = async () => {
        if (!selectedMonth)
            return;
        Alert.alert('Generate Content', `Generate ${MARKETS.find((m) => m.id === selectedMarket)?.name} Month ${selectedMonth}? This may take several minutes.`, [
            { text: 'Cancel', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    setLoading(true);
                    try {
                        const body = { marketId: selectedMarket, month: selectedMonth, batchSize: 3 };
                        if (selectedGoal !== 'all')
                            body.goal = selectedGoal;
                        const { data, error } = await supabase.functions.invoke('generate-curriculum', { body });
                        if (error)
    ...
    const getMonthProgress = (monthNum) => {
        if (!planInfo)
            return 0;
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.marketsGrid}>
          {MARKETS.map((market) => {
            ...
            return (<TouchableOpacity key={market.id} style={[styles.marketCard, isSelected && styles.marketCardSelected]} onPress={() => { setSelectedMarket(market.id); setSelectedMonth(null); fetchPlan(market.id); }} activeOpacity={0.75}>
                ...
                <View style={styles.marketProgressBg}>
                  <View style={[styles.marketProgressFill, { width: `${pct}%` }]}/>
                </View>
        ...
        <View style={styles.goalsRow}>
          {GOALS.map((g) => (<TouchableOpacity key={g.id} style={[styles.goalChip, selectedGoal === g.id && styles.goalChipSelected]} onPress={() => { setSelectedGoal(g.id); fetchPlan(selectedMarket, selectedMonth ?? undefined); }} activeOpacity={0.75}>
        ...
        {/* Per-goal stats */}
        {planInfo?.goalStats && (<View style={styles.goalStatsRow}>
            {Object.entries(planInfo.goalStats).map(([goalKey, stats]) => (<View key={goalKey} style={styles.goalStatItem}>
        ...
        {/* Overview */}
        {planInfo && (<View style={styles.overviewCard}>
            ...
            <View style={styles.progressBg}>
              <View style={[styles.progressFill, { width: `${((planInfo.existingDays?.length || 0) / 180) * 100}%` }]}/>
            </View>
        ...
        <View style={styles.monthsGrid}>
          {MONTHS.map((m) => {
            ...
            return (<TouchableOpacity key={m.month} style={[styles.monthCard, isSelected && { borderColor: m.color + '80' }]} onPress={() => { setSelectedMonth(m.month); fetchPlan(selectedMarket, m.month); }} activeOpacity={0.8}>
        ...
        {selectedMonth && (<View style={styles.actionsCard}>
            ...
            <View style={styles.actionsBtns}>
              ...
              <TouchableOpacity style={[styles.generateBtn, (!planInfo || planInfo.daysToGenerate?.length === 0) && styles.btnDisabled]} onPress={generateContent} disabled={loading || !planInfo || planInfo.daysToGenerate?.length === 0} activeOpacity={0.85}>
