...
export default function SeminarDetailScreen() {
    ...
    const { seminarId } = useLocalSearchParams();
    ...
    React.useEffect(() => {
        if (!registration || prepModules.length === 0)
            return;
    ...
    return (<View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {seminar.speaker_name && (<Text style={styles.headerSubtitle}>with {seminar.speaker_name}</Text>)}
        </View>
      </View>

      {/* Tab bar */}
      <View style={styles.tabBar}>
        {TABS.map(tab => (<TouchableOpacity key={tab.id} onPress={() => setActiveTab(tab.id)} style={[styles.tab, activeTab === tab.id && styles.activeTab]}>
      ...
      {!registration && (<TouchableOpacity onPress={handleRegister} style={styles.registerCta} activeOpacity={0.85}>
      ...
      {/* Tab content */}
      {activeTab === 'prep' && (<ScrollView style={styles.tabContent} contentContainerStyle={styles.tabContentInner} showsVerticalScrollIndicator={false}>
          <Text style={styles.sectionIntro}>
            Complete these modules before the seminar to get the most out of it.
          </Text>
          {prepModules.map((mod, idx) => (<PrepModule key={mod.id} module={mod} index={idx} isCompleted={completedModules.has(mod.id)} onComplete={() => handleModuleComplete(mod.id)}/>))}
          {prepModules.length === 0 && (<View style={styles.emptyTab}>
              <Feather name="book" size={28} color={COLORS.textMuted}/>
              <Text style={styles.emptyTabText}>No prep modules for this seminar.</Text>
            </View>)}
        </ScrollView>)}

      {activeTab === 'watch' && (<View style={styles.watchTab}>
      ...
      {activeTab === 'takeaways' && (<ScrollView style={styles.tabContent} contentContainerStyle={styles.tabContentInner} showsVerticalScrollIndicator={false}>
          {!isTakeawaysUnlocked ? (<View style={styles.lockedTab}>
              ...
              {takeaways.map((t, i) => (<View key={i} style={styles.takeawayCard}>
