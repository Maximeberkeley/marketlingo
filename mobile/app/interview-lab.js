...
import { INTERVIEW_PERSONAS, MECE_FRAMEWORKS, STORY_HERO_STEPS } from '../lib/interviewLabData';
...
export default function InterviewLabScreen() {
    ...
    const [pressureChoice, setPressureChoice] = useState(null);
    ...
    const pressure = useMemo(() => {
        if (path === 'consulting' && questions.mentalMathQuestions.length)
            return questions.mentalMathQuestions[0];
    ...
    const next = (target) => { triggerHaptic('light'); setStage(target); };
    const choosePressure = (index) => {
        if (!pressure || pressureChoice !== null)
            return;
    ...
    const startRecording = useCallback(async () => {
        try {
            ...
            log.warn('[Interview] Recording failed:', error);
        }
    }, []);
    const stopRecording = useCallback(async () => {
        ...
        try {
            ...
            if (uri)
                setResponse(await transcribeAudio(uri));
        }
        catch (error) {
            log.warn('[Interview] Transcription failed:', error);
        }
        finally {
            setSubmitting(false);
    ...
    const submit = async () => {
        if (!user || !mock || response.trim().length < 20)
            return;
        setSubmitting(true);
        try {
    ...
    const bankResult = async () => {
        ...
        try {
            ...
            log.warn('[Interview] Reward banking failed:', error);
        }
        router.back();
    };
    if (marketLoading || questions.loading)
    ...
    return (<View style={[styles.root, { paddingTop: insets.top + 8 }]}>
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 28 }]} showsVerticalScrollIndicator={false}>
        ...
        {stage === 'brief' && <>
          ...
          <View style={styles.personas}>{Object.entries(INTERVIEW_PERSONAS).map(([key, item]) => (<TouchableOpacity key={key} onPress={() => setPersona(key)} style={[styles.persona, persona === key && { borderColor: item.color, backgroundColor: `${item.color}10` }]}>
        ...
        {stage === 'pressure' && pressure && <>
          ...
          <View style={styles.options}>{pressure.options.map((option, index) => {
                const revealed = pressureChoice !== null;
                const correct = index === pressure.correctIndex;
                const picked = index === pressureChoice;
...
function PathCard({ icon, title, subtitle, color, onPress }) {
...
function Coach({ text, onSpeak, speaking }) { return <View style={styles.coach}><Image source={SOPHIA} style={styles.coachAvatar}/><Text style={styles.coachText}>{text}</Text>{onSpeak && <TouchableOpacity onPress={onSpeak}><Feather name={speaking ? 'volume-2' : 'volume-1'} size={18} color={COLORS.accent}/></TouchableOpacity>}</View>; }
function Action({ label, color, onPress, disabled }) { return <TouchableOpacity disabled={disabled} onPress={onPress} style={[styles.action, { backgroundColor: color }, disabled && styles.disabled]}><Text style={styles.actionText}>{label}</Text><Feather name="arrow-right" size={18} color={COLORS.bg0}/></TouchableOpacity>; }
function Debrief({ icon, title, body, color }) { return <View style={styles.debrief}><Feather name={icon} size={19} color={color}/><View style={{ flex: 1 }}><Text style={[styles.debriefTitle, { color }]}>{title}</Text><Text style={styles.debriefBody}>{body || 'No note this round.'}</Text></View></View>; }
