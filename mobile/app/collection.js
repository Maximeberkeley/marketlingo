...
function CollectionScreen() {
  ...
  return <View style={styles.root}>
    ...
    return <TouchableOpacity key={card.id} style={[styles.card, card.owned ? styles.ownedCard : styles.lockedCard]} onPress={() => setSelected(card)} activeOpacity={0.82}>
      ...
      <Modal visible={!!selected} transparent animationType="slide" onRequestClose={() => setSelected(null)}>
        <View style={styles.backdrop}><View style={styles.sheet}>
          {selected && <>
            ...
            {selected.owned && featuredId !== selected.id && <TouchableOpacity style={[styles.primary, { backgroundColor: getMarketWorld(selected.market_id).colors[0] }]} onPress={async () => {
    await featureCard(selected.id);
    triggerHaptic("success");
    setSelected(null);
  }}><Text style={styles.primaryText}>Feature on profile</Text></TouchableOpacity>}
...
function Filter({ active, label, onPress }) {
  return <TouchableOpacity style={[styles.filter, active && styles.filterActive]} onPress={onPress}><Text style={[styles.filterText, active && styles.filterTextActive]}>{label}</Text></TouchableOpacity>;
}
...
const styles = StyleSheet.create({
  ...
  iconButton: { width: 38, height: 38, marginLeft: -8, alignItems: "center", justifyContent: "center", borderRadius: 19 },
  headerCopy: { flex: 1, marginLeft: 4 },
  title: { fontSize: 24, lineHeight: 29, fontWeight: "900", color: COLORS.textPrimary },
  subtitle: { fontSize: 13, fontWeight: "600", color: COLORS.textMuted, marginTop: 2 },
  filters: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 16, gap: 8 },
  filter: { height: 36, paddingHorizontal: 15, borderRadius: 18, borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.bg1, alignItems: "center", justifyContent: "center" },
  filterActive: { backgroundColor: COLORS.textPrimary, borderColor: COLORS.textPrimary },
  filterText: { fontSize: 11, fontWeight: "800", color: COLORS.textMuted, textTransform: "uppercase" },
  filterTextActive: { color: COLORS.bg0 },
  grid: { paddingHorizontal: 20, paddingTop: 2, flexDirection: "row", flexWrap: "wrap", gap: 12 },
  card: { width: "48%", minHeight: 205, borderRadius: 22, borderWidth: 1, padding: 12, backgroundColor: COLORS.bg2 },
  ownedCard: { borderColor: COLORS.accentMedium, shadowColor: COLORS.accent, shadowOpacity: isDark ? 0.18 : 0.11, shadowRadius: 15, shadowOffset: { width: 0, height: 5 }, elevation: 3 },
  lockedCard: { borderColor: COLORS.border, backgroundColor: isDark ? COLORS.bg1 : COLORS.bg2, opacity: isDark ? 0.62 : 0.7 },
  cardArt: { height: 92, borderRadius: 14, alignItems: "center", justifyContent: "center", marginBottom: 13 },
  rarity: { fontSize: 9, fontWeight: "900", letterSpacing: 1.1 },
  cardName: { fontSize: 14, lineHeight: 18, fontWeight: "900", color: COLORS.textPrimary, marginTop: 4 },
  locked: { color: COLORS.textMuted },
  cardRole: { fontSize: 11, lineHeight: 15, color: COLORS.textMuted, marginTop: 4 },
  featured: { flexDirection: "row", gap: 4, alignItems: "center", marginTop: "auto", paddingTop: 12 },
  featuredText: { fontSize: 8, fontWeight: "900", letterSpacing: 0.6, color: COLORS.streak },
  backdrop: { flex: 1, backgroundColor: isDark ? "rgba(0,0,0,0.72)" : "rgba(26,31,54,0.3)", justifyContent: "flex-end" },
  sheet: { backgroundColor: COLORS.bg0, padding: 24, paddingBottom: 36, borderTopLeftRadius: 24, borderTopRightRadius: 24, alignItems: "center", borderTopWidth: 1, borderColor: COLORS.border },
  detailIcon: { width: 80, height: 80, borderRadius: 22, alignItems: "center", justifyContent: "center" },
  detailWorld: { fontSize: 10, fontWeight: "900", letterSpacing: 1.1, color: COLORS.textMuted, marginTop: 16 },
  detailName: { fontSize: 26, fontWeight: "900", color: COLORS.textPrimary, marginTop: 4 },
  detailInsight: { fontSize: 14, lineHeight: 21, textAlign: "center", color: COLORS.textSecondary, marginTop: 12 },
  stats: { flexDirection: "row", gap: 8, marginTop: 20, width: "100%" },
  stat: { flex: 1, padding: 12, borderWidth: 1, borderColor: COLORS.border, borderRadius: 12, alignItems: "center", backgroundColor: COLORS.bg1 },
  statValue: { fontSize: 20, fontWeight: "900", color: COLORS.textPrimary },
  statLabel: { fontSize: 8, fontWeight: "800", color: COLORS.textMuted },
  primary: { width: "100%", height: 52, borderRadius: 14, alignItems: "center", justifyContent: "center", marginTop: 20 },
  primaryText: { color: "#FFFFFF", fontSize: 15, fontWeight: "800" },
  close: { padding: 14 },
  closeText: { color: COLORS.textMuted, fontWeight: "700" }
});
export {
  CollectionScreen as default
};
