    ...
    return (<View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={[styles.grid, { paddingBottom: insets.bottom + 32 }]} showsVerticalScrollIndicator={false}>
        {filtered.map(card => {
            ...
            return (<TouchableOpacity key={card.id} style={[styles.card, { borderColor: card.owned ? world.colors[0] : COLORS.border }]} onPress={() => setSelected(card)} activeOpacity={0.8}>
...
function unlockCopy(card) {
    if (card.unlock_type === 'first_lesson')
        return 'Complete your first lesson';
    if (card.unlock_type === 'lesson_count')
        return `Complete ${card.unlock_threshold} lessons`;
    if (card.unlock_type === 'mastery')
        return `Master ${card.unlock_threshold} concepts`;
    return `Reach day ${card.unlock_threshold}`;
}
function Filter({ active, label, onPress }) { return <TouchableOpacity style={[styles.filter, active && styles.filterActive]} onPress={onPress}><Text style={[styles.filterText, active && styles.filterTextActive]}>{label}</Text></TouchableOpacity>; }
