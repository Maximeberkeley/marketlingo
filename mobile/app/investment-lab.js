...
const MODULES = [
    { id: 'valuation', title: 'Valuation', desc: 'Price it like an insider.', featherIcon: 'bar-chart-2', color: '#10B981', scoreKey: 'valuation_score' },
    { id: 'due_diligence', title: 'Due Diligence', desc: 'Find what the deck hides.', featherIcon: 'search', color: '#3B82F6', scoreKey: 'due_diligence_score' },
    { id: 'risk_assessment', title: 'Risk', desc: 'Name the way this breaks.', featherIcon: 'shield', color: '#F59E0B', scoreKey: 'risk_assessment_score' },
    { id: 'portfolio', title: 'Portfolio', desc: 'Spread the bets that pay.', featherIcon: 'layers', color: '#8B5CF6', scoreKey: 'portfolio_construction_score' },
];
const EXTRAS = [
    { path: '/portfolio-guide', icon: 'map', color: '#10B981', title: 'Portfolio Guide', desc: '5 steps to your first portfolio' },
    { path: '/investment-watchlist', icon: 'bookmark', color: '#0EA5E9', title: 'Watchlist', desc: 'Companies you track' },
    { path: '/portfolio-builder', icon: 'pie-chart', color: '#F59E0B', title: 'Portfolio Builder', desc: 'Allocate and balance positions' },
];
export default function InvestmentLabScreen() {
    ...
    const nextModule = MODULES.find((mod) => {
        const score = progress ? progress[mod.scoreKey] || 0 : 0;
        return score < CERTIFICATION_THRESHOLDS[mod.scoreKey.replace('_score', '')];
    }) || MODULES[0];
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: 0, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        {/* Hero */}
        <ImageBackground source={heroImage} style={[styles.heroBanner, { paddingTop: insets.top + 12 }]} imageStyle={{ borderBottomLeftRadius: 26, borderBottomRightRadius: 26 }}>
        ...
        <View style={{ paddingHorizontal: 16, paddingTop: 18 }}>
          {/* Continue */}
          <TouchableOpacity style={[styles.continueCard, { backgroundColor: nextModule.color }]} onPress={() => handleModulePress(nextModule.id)} activeOpacity={0.88}>
            <View style={{ flex: 1 }}>
          ...
          {/* Run progress */}
          {progress && (<View style={styles.progressCard}>
          ...
          <View style={{ gap: 10 }}>
            {MODULES.map((mod) => {
            const score = progress ? progress[mod.scoreKey] || 0 : 0;
            const passed = score >= CERTIFICATION_THRESHOLDS[mod.scoreKey.replace('_score', '')];
            return (<TouchableOpacity key={mod.id} style={[styles.scenarioCard, passed && { borderColor: 'rgba(34,197,94,0.35)' }]} onPress={() => handleModulePress(mod.id)} activeOpacity={0.9}>
                  ...
                  {passed ? (<Feather name="check-circle" size={20} color="#22C55E"/>) : (<Text style={styles.scorePill}>{score > 0 ? `${score}%` : 'Start'}</Text>)}
                </TouchableOpacity>);
        })}
          </View>

          {/* Certificate */}
          <TouchableOpacity style={[styles.certCard, progress?.investment_certified && { borderColor: 'rgba(34,197,94,0.35)', backgroundColor: 'rgba(34,197,94,0.06)' }]} onPress={() => router.push('/investment-certificate')} activeOpacity={0.9}>
          ...
          <View style={{ gap: 10 }}>
            {EXTRAS.map((item) => (<TouchableOpacity key={item.path} style={styles.toolCard} onPress={() => router.push(item.path)} activeOpacity={0.9}>
