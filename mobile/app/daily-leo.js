...
function presentation(data) {
    ...
    if (hoursLeft <= 3 || hour >= 21) {
        return { mood: 'urgent', eyebrow: 'FINAL HOURS', line: `${Math.ceil(hoursLeft)} hours left. I brought the rain. You bring five minutes for ${market}.` };
    }
    if (hour >= 18) {
        return { mood: 'thinking', eyebrow: 'EVENING CHECK', line: `Your ${market} lesson is still waiting. Bold strategy. Shall we save the streak?` };
    }
    if (hour >= 12) {
        return { mood: 'sassy', eyebrow: 'AFTERNOON CHECK', line: `You have time. Your excuses are simply getting more creative than your ${market} knowledge.` };
    }
    return { mood: 'reading', eyebrow: 'TODAY’S MISSION', line: `${market} is moving while you sleep. Conveniently, I took notes.` };
}
export default function DailyLeoScreen() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const { data: profile, error: profileError } = await withStartupTimeout(supabase.from('profiles').select('selected_market').eq('id', user.id).maybeSingle());
                if (profileError)
                    throw profileError;
    ...
    return (<View style={[styles.root, { paddingTop: insets.top + 28, paddingBottom: insets.bottom + 22 }]}>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity style={styles.continueButton} activeOpacity={0.88} onPress={() => { triggerHaptic('medium'); router.replace('/(tabs)/home'); }}>
