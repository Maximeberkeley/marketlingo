...
export default function StreakRescueScreen() {
    ...
    const finish = useCallback(async (finalCorrect) => {
        ...
        if (finalCorrect >= NEEDED_CORRECT) {
            ...
            if (!saved && user && marketId) {
                ...
                if (error)
                    log.warn('Rescue clock extension failed', error);
            }
    ...
    if (outcome) {
        ...
        return (<LinearGradient colors={saved ? ['#F59E0B', '#EA580C', '#C2410C'] : ['#475569', '#334155', '#1E293B']} style={styles.full}>
    ...
    if (!started) {
        ...
        <View style={[styles.fullInner, { paddingTop: insets.top + 40, paddingBottom: insets.bottom + 24 }]}>
          ...
          <Image source={require('../assets/leo-sticker.png')} style={styles.fullLeo} resizeMode="contain"/>
          <View style={{ flex: 1 }}/>
          <TouchableOpacity style={styles.whiteCta} onPress={() => { triggerHaptic('medium'); setStarted(true); }} activeOpacity={0.9}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 140 }}>
        ...
        <View style={styles.answerRow}>
          {[true, false].map((value) => {
            ...
            return (<TouchableOpacity key={String(value)} style={[
                    styles.answerBtn,
                ...
                ]} onPress={() => onAnswer(value)} activeOpacity={0.85} disabled={answered}>
