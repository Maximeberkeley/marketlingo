...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, ActivityIndicator, Alert, TextInput, Image, Modal, } from 'react-native';
...
const CATEGORY_CONFIG = {
    core: { label: 'Core', color: '#10B981', target: '60-70%', icon: 'shield' },
    growth: { label: 'Growth', color: '#3B82F6', target: '20-30%', icon: 'trending-up' },
    speculative: { label: 'Speculative', color: '#F59E0B', target: '5-10%', icon: 'zap' },
};
export default function PortfolioBuilderScreen() {
    ...
    const availableCompanies = useMemo(() => {
        ...
        watchlist.forEach((c) => {
            if (!positionIds.has(c.id))
                all.set(c.id, c);
        });
        // Add suggested companies
        suggestedCompanies.forEach((c) => {
            if (!positionIds.has(c.id))
                all.set(c.id, { id: c.id, name: c.name, ticker: c.ticker });
        });
    ...
    const removePosition = (companyId) => {
        setPositions((prev) => prev.filter((p) => p.company.id !== companyId));
    };
    const updateAllocation = (companyId, newAlloc) => {
        setPositions((prev) => prev.map((p) => p.company.id === companyId ? { ...p, allocation: Math.max(0, Math.min(100, newAlloc)) } : p));
    };
    const getFeedback = () => {
        const messages = [];
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[
        ...
        <View style={styles.overviewCard}>
          ...
          <View style={styles.legendRow}>
            {Object.entries(CATEGORY_CONFIG).map(([key, config]) => (<View key={key} style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: config.color }]}/>
                <Text style={styles.legendLabel}>
                  {config.label} {categoryBreakdown[key]}%
                </Text>
              </View>))}
          </View>
        </View>
        ...
        {/* Feedback */}
        {feedback.length > 0 && (<View style={styles.feedbackCard}>
            <Text style={styles.feedbackTitle}>
              <Feather name="cpu" size={14} color={COLORS.accent}/> AI Feedback
            </Text>
            {feedback.map((msg, i) => (<View key={i} style={styles.feedbackRow}>
                <Feather name={msg.type === 'success' ? 'check-circle' : msg.type === 'error' ? 'x-circle' : 'alert-circle'} size={14} color={msg.type === 'success' ? '#22C55E' : msg.type === 'error' ? '#EF4444' : '#F59E0B'}/>
                <Text style={styles.feedbackText}>{msg.text}</Text>
              </View>))}
          </View>)}

        {/* Positions */}
        {positions.length > 0 && (<View style={{ marginTop: 16 }}>
            <Text style={styles.sectionTitle}>YOUR POSITIONS</Text>
            {positions.map((pos) => {
                const catConfig = CATEGORY_CONFIG[pos.category];
                return (<View key={pos.company.id} style={styles.positionCard}>
                  <View style={styles.positionHeader}>
                    ...
                    <TouchableOpacity onPress={() => removePosition(pos.company.id)} style={styles.removeBtn}>
                  ...
                  <View style={styles.allocControl}>
                    <TouchableOpacity style={styles.allocBtn} onPress={() => updateAllocation(pos.company.id, pos.allocation - 5)}>
        ...
        {/* Add Position */}
        {!showAddModal ? (<TouchableOpacity style={styles.addBtn} onPress={() => setShowAddModal(true)}>
            <Feather name="plus" size={20} color={COLORS.accent}/>
            <Text style={styles.addBtnText}>Add Company</Text>
          </TouchableOpacity>) : (<View style={styles.addModal}>
            ...
            <View style={styles.catRow}>
              {Object.entries(CATEGORY_CONFIG).map(([key, config]) => (<TouchableOpacity key={key} style={[
                    styles.catOption,
            ...
            {availableCompanies.length > 0 ? (<View style={{ gap: 6 }}>
                {availableCompanies.slice(0, 10).map((company) => {
                    const isWatchlisted = watchlist.some((w) => w.id === company.id);
                    return (<TouchableOpacity key={company.id} style={styles.companyOption} onPress={() => addPosition(company)}>
                      <Feather name={isWatchlisted ? 'star' : 'globe'} size={14} color={isWatchlisted ? '#F59E0B' : COLORS.textMuted}/>
                      <Text style={styles.companyOptionName} numberOfLines={1}>
                        {company.name}
                      </Text>
                      {company.ticker && (<Text style={styles.companyOptionTicker}>
                          {company.ticker}
                        </Text>)}
                      <Feather name="plus" size={14} color={COLORS.accent}/>
                    </TouchableOpacity>);
                })}
              </View>) : (<Text style={styles.noCompanies}>
                All available companies have been added
              </Text>)}
          </View>)}

        {/* Save & History Actions */}
        {positions.length >= 2 && (<View style={{ flexDirection: 'row', gap: 8, marginTop: 16 }}>
            <TouchableOpacity style={[styles.actionBtn, { flex: 1, backgroundColor: COLORS.accent }]} onPress={() => { setSnapshotName(''); setStrategyNotes(''); setShowSaveModal(true); }}>
              <Feather name="save" size={16} color="#fff"/>
              <Text style={[styles.actionBtnText, { color: '#fff' }]}>Save Snapshot</Text>
            </TouchableOpacity>
            {snapshots.length > 0 && (<TouchableOpacity style={[styles.actionBtn, { backgroundColor: COLORS.bg2, borderWidth: 1, borderColor: COLORS.border }]} onPress={() => setShowHistory(!showHistory)}>
                <Feather name="clock" size={16} color={COLORS.accent}/>
                <Text style={styles.actionBtnText}>{snapshots.length}</Text>
              </TouchableOpacity>)}
            {snapshots.length >= 2 && (<TouchableOpacity style={[styles.actionBtn, { backgroundColor: 'rgba(59,130,246,0.08)', borderWidth: 1, borderColor: 'rgba(59,130,246,0.3)' }]} onPress={() => setShowCompare(true)}>
                <Feather name="git-pull-request" size={16} color="#3B82F6"/>
              </TouchableOpacity>)}
          </View>)}

        {/* Snapshot History */}
        {showHistory && snapshots.length > 0 && (<View style={{ marginTop: 12 }}>
            ...
            {snapshots.map((snap, i) => {
                ...
                return (<View key={snap.id} style={styles.snapshotCard}>
                  ...
                  {snap.strategy_notes && (<Text style={styles.snapshotNotes} numberOfLines={2}>{snap.strategy_notes}</Text>)}
                  {/* Delta vs previous */}
                  {prevSnap && (<View style={styles.deltaRow}>
                      <Feather name="git-branch" size={11} color={COLORS.textMuted}/>
                      <Text style={styles.deltaText}>
                        vs {prevSnap.snapshot_name}: Core {bd.core - (prevSnap.category_breakdown?.core || 0) >= 0 ? '+' : ''}{bd.core - (prevSnap.category_breakdown?.core || 0)}%
                      </Text>
                    </View>)}
                </View>);
            })}
      ...
      <Modal visible={showSaveModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.modalInput} placeholder="e.g. Conservative Q1 Strategy" placeholderTextColor={COLORS.textMuted} value={snapshotName} onChangeText={setSnapshotName}/>
            <Text style={styles.inputLabel}>Strategy Notes (optional)</Text>
            <TextInput style={[styles.modalInput, { minHeight: 80 }]} placeholder="Why did you choose this allocation?" placeholderTextColor={COLORS.textMuted} value={strategyNotes} onChangeText={setStrategyNotes} multiline textAlignVertical="top"/>
            <TouchableOpacity style={[styles.saveBtn, !snapshotName.trim() && { opacity: 0.5 }]} disabled={!snapshotName.trim()} onPress={async () => {
            await saveSnapshot(snapshotName.trim(), positions.map(p => ({ company: p.company, allocation: p.allocation, category: p.category })), totalAllocation, categoryBreakdown, strategyNotes.trim() || undefined);
      ...
      {/* Snapshot Comparison */}
      <SnapshotCompare visible={showCompare} onClose={() => setShowCompare(false)} snapshots={snapshots}/>
    </View>);
}
