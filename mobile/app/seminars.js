...
export default function SeminarsScreen() {
    ...
    return (<View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {loading ? (<View style={styles.loadingState}>
        ...
        </View>) : seminars.length === 0 ? (<View style={styles.emptyState}>
          ...
          {/* Live Now */}
          {liveSeminars.length > 0 && (<View style={styles.section}>
              <View style={styles.sectionHeader}>
                <View style={styles.liveDot}/>
                <Text style={styles.sectionTitle}>Live Now</Text>
              </View>
              {liveSeminars.map(s => (<SeminarCard key={s.id} seminar={s} onPress={() => handlePress(s)}/>))}
            </View>)}

          {/* Upcoming */}
          {upcomingSeminars.length > 0 && (<View style={styles.section}>
              <Text style={styles.sectionTitle}>Upcoming</Text>
              {upcomingSeminars.map(s => (<SeminarCard key={s.id} seminar={s} onPress={() => handlePress(s)}/>))}
            </View>)}

          {/* Past */}
          {pastSeminars.length > 0 && (<View style={styles.section}>
              <Text style={styles.sectionTitle}>Past Seminars</Text>
              {pastSeminars.map(s => (<SeminarCard key={s.id} seminar={s} onPress={() => handlePress(s)}/>))}
            </View>)}
        </ScrollView>)}
    </View>);
}
