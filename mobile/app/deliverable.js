...
function DeliverableScreen() {
  ...
  return <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : void 0}>
      <ScrollView
    style={styles.flex}
  ...
  >
    ...
    /* Hero: the document's own cover. */
        ...
        <View style={styles.heroWrap}>
          <LinearGradient
    colors={isDark ? ["#2A2340", "#1E2230", "#191C22"] : ["#F1EBFF", "#F6F4FF", "#FFFFFF"]}
    start={{ x: 0, y: 0 }}
    end={{ x: 1, y: 1 }}
    style={styles.hero}
  >
                  ...
                  <View style={styles.statusRow}>
                    {status === "empty" ? <View style={styles.statusEmpty}>
                        ...
                        <Text style={styles.statusDoneText}>
                          ...
                          {own.length > 1 ? ` \xB7 ${own.length} lines` : ""}
                        </Text>
                      </View>}
                  </View>
                </View>
              ...
              {own.map((entry) => <View key={entry.id} style={styles.entry}>
                  ...
                  <View style={styles.flex}>
                    ...
                    <View style={styles.entryFoot}>
                      {entry.dayNumber ? <Text style={styles.entryDay}>Written on day {entry.dayNumber}</Text> : <View />}
                      <TouchableOpacity
      onPress={() => {
        triggerHaptic("light");
