...
export default function DeliverableScreen() {
    ...
    const [openSection, setOpenSection] = useState(params.section || weeklyPrompt?.key || null);
    ...
    return (<KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.flex} contentContainerStyle={{ paddingTop: insets.top + 12, paddingBottom: insets.bottom + 48 }} keyboardShouldPersistTaps="handled">
        ...
        {template.sections.map(section => {
            ...
            return (<View key={section.key} style={styles.card}>
              <TouchableOpacity style={styles.cardHead} onPress={() => setOpenSection(open ? null : section.key)} activeOpacity={0.8}>
              ...
              {own.map(entry => (<View key={entry.id} style={styles.entry}>
              ...
              {open && (<View style={styles.composer}>
                  <TextInput style={styles.input} value={draft} onChangeText={setDraft} placeholder="Write it the way you would say it out loud…" placeholderTextColor={COLORS.textMuted} multiline/>
                  <TouchableOpacity style={[styles.saveBtn, draft.trim().length < 3 && styles.saveBtnOff]} disabled={draft.trim().length < 3 || saving} onPress={() => save(section.key)}>
