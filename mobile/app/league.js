...
import { LEAGUE_TIERS, TIER_META, useLeague } from '../hooks/useLeague';
...
function TierLadder({ current, selected, onSelect }) {
    const currentIndex = LEAGUE_TIERS.indexOf(current);
    return (<View style={styles.ladder}>
      <View style={styles.ladderLine}/>
      {LEAGUE_TIERS.map((tier, index) => {
            const tierMeta = TIER_META[tier];
            const locked = index > currentIndex;
            const active = tier === current;
            return (<TouchableOpacity key={tier} style={styles.tierStep} onPress={() => onSelect(tier)} accessibilityLabel={`View ${tierMeta.label} League`}>
...
function RankBadge({ rank }) {
    ...
    if (!medalColor) {
        return (<View style={styles.rankNumberWrap}>
        <Text style={styles.rankNumber}>{rank}</Text>
      </View>);
    }
    return (<View style={[styles.medal, { backgroundColor: medalColor }]}>
      <Feather name="award" size={14} color={COLORS.textOnAccent}/>
      <Text style={styles.medalText}>{rank}</Text>
    </View>);
}
function ZoneDivider({ type, label }) {
    const color = type === 'promotion' ? COLORS.success : COLORS.error;
    return (<View style={[styles.zoneDivider, { borderColor: color }]}>
    ...
    return (<View style={[styles.screen, { paddingTop: insets.top }]}>
      <View style={styles.navBar}>
        <TouchableOpacity style={styles.iconButton} onPress={() => {
            triggerHaptic('light');
            router.back();
        }} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }} accessibilityRole="button" accessibilityLabel="Go back">
      ...
      {league.loading ? (<View style={styles.loading}>
          <ActivityIndicator size="large" color={COLORS.accent}/>
          <Text style={styles.loadingText}>Building this month’s table…</Text>
        </View>) : (<ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 40 }]}>
          ...
          <View style={styles.mascotStage}>
            ...
            <View style={styles.statusPill}>
              <Feather name={league.myRank && league.myRank <= league.promotionCutoff ? 'trending-up' : 'target'} size={15} color={league.myRank && league.myRank <= league.promotionCutoff ? COLORS.success : COLORS.accent}/>
          ...
          <View style={styles.table}>
            {league.error ? (<View style={styles.emptyState}>
              ...
              </View>) : viewedRivals.length === 0 ? (<View style={styles.emptyState}>
              ...
              </View>) : (viewedRivals.map((r, index) => {
                const showPromotionDivider = index === 0;
                const showDemotionDivider = viewedDemotionCutoff !== null && r.rank === viewedDemotionCutoff + 1;
                return (<React.Fragment key={r.userId}>
                    {showPromotionDivider && (<ZoneDivider type="promotion" label={viewedTier === league.tier && promotionTier ? `Top ${viewedPromotionCutoff} advance to ${promotionTier} League` : `Top ${viewedPromotionCutoff} lead ${viewedMeta.label}`}/>)}
                    ...
                    <View style={[styles.row, r.isMe && styles.myRow]}>
                      ...
                      <View style={styles.personCopy}>
                        <View style={styles.nameRow}>
                          ...
                          {r.isMe && (<View style={styles.youBadge}>
                              <Text style={styles.youBadgeText}>{meta.label.toUpperCase()}</Text>
                            </View>)}
                        </View>
                        {r.rank <= viewedPromotionCutoff && (<View style={styles.advanceRow}>
...
const styles = StyleSheet.create({
    ...
    trophyLeo: { width: 190, height: 190, resizeMode: 'contain', backfaceVisibility: 'hidden', imageRendering: 'crisp-edges' },
