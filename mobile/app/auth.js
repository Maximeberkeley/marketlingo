        ...
        <View style={styles.form}>
          ...
          {mode === 'signup' && signupStep === 'name' ? (<>
              ...
              <View style={styles.inputGroup}>
                <TextInput style={styles.input} placeholder="Enter your name..." placeholderTextColor={COLORS.textMuted} value={displayName} onChangeText={(value) => setDisplayName(value.slice(0, 40))} autoFocus autoCapitalize="words" autoCorrect={false} maxLength={40} returnKeyType="next" onSubmitEditing={() => {
                if (displayName.trim()) {
              ...
              <TouchableOpacity style={[styles.submitButton, !displayName.trim() && styles.disabledButton]} disabled={!displayName.trim()} onPress={() => {
                void storage.setDisplayName(normalizeDisplayName(displayName));
                setSignupStep('credentials');
            }}>
          ...
          <TouchableOpacity style={[styles.submitButton, loading && { opacity: 0.7 }]} onPress={handleSubmit} disabled={loading}>
            {loading ? (<ActivityIndicator color="#FFFFFF"/>) : (<Text style={styles.submitButtonText}>
                {mode === 'login' ? 'Sign In' : 'Sign Up'}
              </Text>)}
          </TouchableOpacity>

          {mode === 'signup' && (<TouchableOpacity style={styles.backToName} onPress={() => setSignupStep('name')}>
          ...
          <TouchableOpacity style={styles.switchMode} onPress={() => {
            ...
            setMode(next);
            if (next === 'signup')
                setSignupStep(displayName ? 'credentials' : 'name');
        }}>
