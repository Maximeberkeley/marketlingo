...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, ActivityIndicator, Image, Animated as RNAnimated, } from 'react-native';
...
function ScoreMascot({ isGoodScore }) {
    ...
    return (<RNAnimated.View style={{ transform: [{ scale: scaleAnim }, { rotate }], marginBottom: 12, alignItems: 'center' }}>
...
export default function DrillsScreen() {
    ...
    // Timer
    useEffect(() => {
        if (!isTimerActive || showResult || loading || questions.length === 0)
            return;
    ...
    if (showIntro && questions.length > 0) {
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
          ...
          {/* Set selector */}
          {totalSets > 1 && (<View style={styles.setSelector}>
              ...
              <View style={styles.setGrid}>
                {Array.from({ length: Math.min(totalSets, 5) }, (_, i) => {
                    ...
                    return (<TouchableOpacity key={setNum} style={[
                            styles.setCard,
                      ...
                      {isCompleted ? (<Feather name="check-circle" size={20} color="#22C55E"/>) : (<Text style={[styles.setNumber, isCurrent && styles.setNumberActive]}>
                          {setNum}
                        </Text>)}
          ...
          <View style={styles.featuresCard}>
            ...
            {['15 seconds per question', 'True or False answers', 'Based on your lessons', '3 sets for deeper mastery'].map((f, i) => (<View key={i} style={styles.featureRow}>
                <View style={[styles.featureDot, { backgroundColor: '#F59E0B' }]}/>
                <Text style={styles.featureText}>{f}</Text>
              </View>))}
          </View>
          <TouchableOpacity style={styles.ctaButton} onPress={() => { triggerHaptic('medium'); setShowIntro(false); setIsTimerActive(true); }}>
    ...
    if (drillComplete) {
        ...
        <View style={styles.setsProgress}>
          {Array.from({ length: Math.min(totalSets, 5) }, (_, i) => (<View key={i} style={[
                    styles.setDot,
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginTop: 20, width: '100%' }}>
          ...
          {hasMoreSets ? (<TouchableOpacity style={[styles.ctaButton, { flex: 1 }]} onPress={handleNextSet}>
              <Text style={styles.ctaText}>Next Set →</Text>
            </TouchableOpacity>) : (<TouchableOpacity style={[styles.ctaButton, { flex: 1 }]} onPress={() => {
                    setCurrentSet(1);
                    setSetsCompleted(0);
                    setCurrentQuestion(0);
                    setScore(0);
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        {!showResult && (<View style={styles.answerRow}>
        ...
        {showResult && (<View style={[styles.feedbackCard,
                selectedAnswer === null ? styles.feedbackTimeout :
                    isCorrect ? styles.feedbackCorrect : styles.feedbackWrong]}>
            <View style={styles.feedbackHeader}>
              <Feather name={selectedAnswer === null ? 'clock' : isCorrect ? 'check-circle' : 'x-circle'} size={20} color={selectedAnswer === null ? '#F59E0B' : isCorrect ? '#22C55E' : '#EF4444'}/>
