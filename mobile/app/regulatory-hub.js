...
export default function RegulatoryHubScreen() {
    ...
    const [marketId, setMarketId] = useState('aerospace');
    useEffect(() => {
        const fetchMarket = async () => {
            ...
            if (profile?.selected_market)
                setMarketId(profile.selected_market);
            setLoading(false);
        };
        fetchMarket();
    }, [user]);
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ gap: 8, marginBottom: 24 }}>
          {data.bodies.map((body, idx) => (<View key={idx} style={styles.bodyCard}>
        ...
        <View style={{ gap: 8, marginBottom: 24 }}>
          {data.keyRegulations.map((reg, idx) => (<View key={idx} style={styles.regCard}>
        ...
        <View style={{ gap: 8 }}>
          {data.resources.map((resource, idx) => (<TouchableOpacity key={idx} style={styles.resourceCard} onPress={() => resource.url && Linking.openURL(resource.url)} activeOpacity={0.7}>
