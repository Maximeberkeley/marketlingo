...
export function LessonHeader({ progress, onExit, onBack, lives, label, accentColor, onAskLeo, showLeoHint, onDismissLeoHint, }) {
    const accent = accentColor || tokens.color.accent;
    return (<View style={styles.wrap}>
      <View style={styles.row}>
        {onBack ? (<TouchableOpacity onPress={onBack} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
        ...
        {typeof lives === 'number' ? (<View style={styles.lives}>
            <Feather name="heart" size={18} color={tokens.color.heart}/>
            <Text style={styles.livesText}>{lives}</Text>
          </View>) : (<View style={styles.spacer}/>)}

        {onAskLeo ? (<TouchableOpacity onPress={onAskLeo} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} accessibilityLabel="Ask Leo a question" style={[styles.askLeoPill, { borderColor: accent + '55' }]}>
            <Image source={LEO_IMAGE} style={styles.askLeoImage}/>
            <Text style={[styles.askLeoText, { color: accent }]}>Ask</Text>
          </TouchableOpacity>) : null}
      </View>

      {showLeoHint && onAskLeo ? (<TouchableOpacity style={[styles.hint, { borderColor: accent + '44', backgroundColor: accent + '12' }]} onPress={onDismissLeoHint} activeOpacity={0.9}>
