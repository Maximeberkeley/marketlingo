...
export function BriefingReader({ visible, title, eyebrow, text, keyTerms, sources, onClose }) {
    const insets = useSafeAreaInsets();
    const [progress, setProgress] = useState(0);
    const [openTerm, setOpenTerm] = useState(null);
    const sections = useMemo(() => paragraphs(text), [text]);
    const { stackId, learningGoal } = useDeepDiveTarget();
    const { deepDive, loading: deepLoading, error: deepError, load: loadDeepDive } = useDeepDive(stackId, learningGoal);
    ...
    return (<Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={close}>
      <View style={[styles.root, { paddingTop: insets.top }]}> 
        <View style={styles.header}>
          <TouchableOpacity accessibilityRole="button" accessibilityLabel="Close briefing" hitSlop={10} style={styles.close} onPress={close}>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} onScroll={handleScroll} scrollEventThrottle={32}>
          ...
          {!!deepDive && (<View style={styles.deep}>
              ...
              {!!deepDive.mechanism?.length && (<View style={styles.deepBlock}>
                  <Text style={styles.deepHeading}>How it works</Text>
                  {deepDive.mechanism.map((step, index) => (<View key={`${index}-${step.step}`} style={styles.stepRow}>
              ...
              {!!deepDive.case_study?.company && (<View style={styles.deepBlock}>
                  <Text style={styles.deepHeading}>In the real world</Text>
                  <Text style={styles.caseCompany}>{deepDive.case_study.company}</Text>
                  {!!deepDive.case_study.situation && (<Text style={styles.stepDetail}>{deepDive.case_study.situation}</Text>)}
                  {deepDive.case_study.figures?.map((figure, index) => (<View key={`${index}-fig`} style={styles.figureRow}>
                      <View style={styles.signalDot}/>
                      <Text style={styles.figureText}>{figure}</Text>
                    </View>))}
                  {!!deepDive.case_study.outcome && (<Text style={styles.stepDetail}>{deepDive.case_study.outcome}</Text>)}
                </View>)}

              {!!deepDive.key_terms?.length && (<View style={styles.deepBlock}>
                  <Text style={styles.deepHeading}>Words to own</Text>
                  {deepDive.key_terms.map(term => (<View key={term.term} style={styles.term}>
                      <Text style={styles.termWord}>{term.term}</Text>
                      <Text style={styles.termDefinition}>{term.definition}</Text>
                    </View>))}
                </View>)}

              {!!deepDive.sources?.length && (<View style={styles.deepBlock}>
                  <Text style={styles.deepHeading}>Where this comes from</Text>
                  {deepDive.sources.map((source, index) => (<TouchableOpacity key={`${source.url}-${index}`} accessibilityRole="link" style={styles.source} onPress={() => Linking.openURL(source.url).catch(() => { })}>
          ...
          {!!keyTerms?.length && (<View style={styles.terms}>
              <Text style={styles.sectionLabel}>KEY TERMS</Text>
              {keyTerms.map(term => {
                const expanded = openTerm === term.term;
                return (<TouchableOpacity key={term.term} accessibilityRole="button" accessibilityState={{ expanded }} accessibilityLabel={`${expanded ? 'Hide' : 'Show'} definition for ${term.term}`} activeOpacity={0.82} style={[styles.term, expanded && styles.termOpen]} onPress={() => {
                        Haptics.selectionAsync().catch(() => { });
          ...
          {!!sources?.length && (<View style={styles.sources}>
              ...
              {sources.map((source, index) => (<TouchableOpacity key={`${source.url}-${index}`} style={styles.source} accessibilityRole="link" onPress={() => Linking.openURL(source.url).catch(() => { })}>
                  <Text style={styles.sourceText}>{source.label}</Text>
                  <Feather name="external-link" size={14} color={tokens.color.textMuted}/>
                </TouchableOpacity>))}
            </View>)}
