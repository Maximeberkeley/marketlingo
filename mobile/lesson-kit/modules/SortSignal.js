 ...
 */
export function SortSignal({ exercise, phase, onChange }) {
    const order = useMemo(() => shuffle(exercise.items.map((_, i) => i)), [exercise.id]);
    const [cursor, setCursor] = useState(0);
    const [placed, setPlaced] = useState([]); // bucket index per item, in order sorted
    const [wrongs, setWrongs] = useState(0);
    const [why, setWhy] = useState(null);
    const bucketRects = useRef([]);
    const bucketRefs = useRef([]);
    const cardRect = useRef(null);
    const cardRef = useRef(null);
    const measure = (node, store) => {
    ...
    const responder = useRef(PanResponder.create({
        ...
        onMoveShouldSetPanResponder: (_event, gesture) => phaseRef.current === 'answering' && !doneRef.current && (Math.abs(gesture.dx) > 3 || Math.abs(gesture.dy) > 3),
    ...
    return (<View style={styles.wrap}>
      ...
      <View style={styles.stage}>
        {!done && item ? (<Animated.View {...responder.panHandlers} ref={node => {
                cardRef.current = node;
            }} onLayout={() => measure(cardRef.current, r => (cardRect.current = r))} style={[
                styles.card,
          ...
          </Animated.View>) : (<View style={styles.cardDone}>
      ...
      <View style={styles.buckets}>
        {exercise.buckets.map((b, i) => {
            ...
            return (<View key={b} ref={node => {
                    bucketRefs.current[i] = node;
                }} onLayout={() => measure(bucketRefs.current[i], r => (bucketRects.current[i] = r))} style={[styles.bucket, active && styles.bucketActive]}>
