...
export function SpotTheFake({ exercise, phase, onChange }) {
    ...
    return (<View style={styles.wrap}>
      ...
      {exercise.statements.map((s, i) => {
            ...
            return (<Animated.View key={i} style={{ opacity: enter, transform: [{ translateY }] }}>
            <TouchableOpacity activeOpacity={0.9} onPress={() => choose(i)} style={[
                    styles.row,
              ...
              <View style={styles.iconWrap}>
                {reveal ? (<Feather name={isFake ? 'x-circle' : 'check-circle'} size={18} color={isFake ? tokens.color.incorrectDark : tokens.color.correctDark}/>) : (<Text style={styles.rowNum}>{i + 1}</Text>)}
              </View>
