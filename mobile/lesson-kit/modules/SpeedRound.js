...
export function SpeedRound({ exercise, phase, onChange }) {
    ...
    const answer = (i) => {
        if (!round || phase === 'feedback' || finished)
            return;
    ...
    return (<View style={styles.wrap}>
      ...
      <View style={[
        ...
        {finished ? (<Text style={styles.score}>
            {hits} / {exercise.rounds.length} right
          </Text>) : (<ColorText text={round.question} style={styles.question} maxSentences={1} maxLength={90}/>)}
      </View>

      {!finished && (<View style={styles.options}>
          {round.options.map((o, i) => (<TouchableOpacity key={i} activeOpacity={0.85} style={styles.option} onPress={() => answer(i)}>
