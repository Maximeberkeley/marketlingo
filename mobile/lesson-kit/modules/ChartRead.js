...
function Spark({ values, color, height = 60, faded = false, }) {
    return (<View style={[styles.spark, { height }]}>
      {values.map((v, i) => (<View key={i} style={{
                flex: 1,
                marginHorizontal: 1,
                borderRadius: 2,
...
export function ChartRead({ exercise, phase, onChange }) {
    ...
    return (<View style={styles.wrap}>
      ...
      <View style={styles.options}>
        {exercise.options.map((o, i) => {
            ...
            return (<TouchableOpacity key={i} activeOpacity={0.9} onPress={() => choose(i)} style={[
                    styles.option,
              ...
              <Spark values={o} color={reveal && right ? tokens.color.correct : tokens.color.accent} height={44}/>
            </TouchableOpacity>);
        })}
      </View>
    </View>);
