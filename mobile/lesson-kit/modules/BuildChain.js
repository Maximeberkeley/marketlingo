 ...
 */
export function BuildChain({ exercise, phase, onChange }) {
    const bank = useMemo(() => shuffle(exercise.steps.map((label, i) => ({ label, i }))), [exercise.id]);
    ...
    const pick = (stepIndex) => {
        if (phase === 'feedback' || complete)
            return;
        if (used.includes(stepIndex))
            return;
    ...
    return (<View style={styles.wrap}>
      ...
      <Animated.View style={[styles.bank, { transform: [{ translateX }] }]}>
        {bank.map(({ label, i }) => {
            ...
            return (<TouchableOpacity key={i} disabled={spent || phase === 'feedback'} onPress={() => {
                    tap();
                    pick(i);
