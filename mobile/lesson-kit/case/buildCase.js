...
import { drillSpotFake, packChain, packMap, packNumber, statNumberSense, trainerCall, } from '../industry/build';
...
function caseLead(text, fallback) {
    const first = text.replace(/\s+/g, ' ').trim().split(/(?<=[.!?])\s+/)[0]?.trim();
    if (first && first.length <= 190)
        return first;
...
const studiedSlides = (studied) => (studied ?? []).flatMap(l => l.slides);
export function buildCase(input) {
    ...
    const call = checkClaim(lessonSlides, 'case-call-claim') || trainerCall(scenario, 'case-call');
    if (!call)
        return null;
    const stages = [];
    ...
    const briefHeadline = lead
        ? caseLead(lead.slides?.[0]?.body || lead.title, `You are back on ${lead.title}. Time to use it.`)
        : scenario
    ...
    const evidence = checkMechanism(lessonSlides, 'case-evidence-lesson') ||
    ...
    const numbers = checkFigure(lessonSlides, 'case-numbers-lesson') ||
...
export function gradeCase(correct, total, callCorrect, confidence) {
