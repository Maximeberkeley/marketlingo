...
import { View, Text, ScrollView, StyleSheet, Animated, Easing, TouchableOpacity, Modal, } from 'react-native';
...
import { rankForScore } from './buildArena';
 ...
 */
export function ArenaScreen({ waves, marketId, marketName, bestScore, onExit, onFinish }) {
    ...
    const popGain = useCallback((amount) => {
        setLastGain(amount);
        gainAnim.setValue(0);
    ...
    const grade = useCallback((isCorrect, secondsRemaining) => {
        clearTick();
        setGraded(g => g + 1);
        if (isCorrect) {
    ...
    useEffect(() => {
        if (!wave || !exercise || showWaveIntro || phase === 'feedback' || ended)
            return;
        if (passive)
            return;
    ...
    if (ended) {
        ...
        return (<View style={[styles.root, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 20 }]}>
        <ScrollView contentContainerStyle={styles.summaryScroll} showsVerticalScrollIndicator={false}>
          <LinearGradient colors={[rank.color, world.colors[1]]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.summaryHero}>
            ...
            {newBest && (<View style={styles.bestPill}>
    ...
    return (<View style={[styles.root, { paddingTop: insets.top + 8 }]}>
      ...
      <View style={styles.hud}>
        ...
        <View style={styles.hudRight}>
          <View style={styles.shieldRow}>
            {Array.from({ length: START_SHIELDS }).map((_, i) => (<Feather key={i} name="shield" size={15} color={i < shields ? tokens.color.accent : tokens.color.disabled}/>))}
          </View>
      ...
      {!passive && (<Text style={[styles.timerLabel, urgent && { color: tokens.color.incorrect }]}>
          {phase === 'feedback' ? 'Locked' : `${timeLeft}s · x${wave.multiplier} points`}
        </Text>)}

      {lastGain !== null && (<Animated.View style={[
                styles.gain,
                {
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 12 }]}>
        {phase === 'feedback' && !passive && (<FeedbackFooter isCorrect={state.isCorrect} explanation={exercise.explanation} correctAnswer={correctAnswerText}/>)}
      ...
      <Modal visible={showWaveIntro} transparent animationType="fade">
        <View style={styles.waveBackdrop}>
          <LinearGradient colors={[world.colors[0], world.colors[1]]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.waveCard}>
            ...
            <TouchableOpacity style={styles.waveBtn} activeOpacity={0.9} onPress={() => {
