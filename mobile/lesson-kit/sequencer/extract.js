...
export function sentences(text, min = 40, max = 200) {
...
function leadSentence(text, fallback) {
...
// ── Cold open ──────────────────────────────────────────────────────
export function makeColdOpen(slides, id, eyebrow) {
...
// ── Micro-insight ──────────────────────────────────────────────────
export function makeMicroInsight(slide, id, eyebrow) {
...
export function makeSpeedRound(terms, id) {
    ...
    if (rounds.some(r => r.options.length < 3))
        return null;
    return {
        kind: 'speedRound',
        id,
...
export function makeMapMarket(terms, id) {
    const clean = terms.filter(t => t.term && t.definition && t.definition.length > 12);
    if (clean.length < 3)
        return null;
...
export function makeBuildChain(items, id, prompt) {
    const steps = items
...
export function makeSortSignal(slides, id) {
    const usable = slides
    ...
    const items = picked.flatMap((p, bucket) => shuffle(p.lines)
        .slice(0, 2)
...
export function makeNumberSense(slides, id) {
    for (const slide of shuffle(slides)) {
        const text = (slide.body || '').replace(/\s+/g, ' ');
        const m = text.match(/(?:\$|US\$)?\s?(\d{1,4}(?:[.,]\d{1,2})?)\s?(thousand|million|billion|trillion|%|percent)/i);
        if (!m)
            continue;
...
function mutate(sentence) {
    ...
    if (num) {
        ...
        if (isFinite(raw) && raw > 0) {
            ...
            if (next !== raw)
                return sentence.replace(num[0], String(next));
        }
    }
    const lower = sentence.toLowerCase();
    for (const [from, to] of [[UP_WORDS, DOWN_WORDS], [DOWN_WORDS, UP_WORDS]]) {
...
export function makeChartRead(slides, id) {
    ...
    if (up === down)
        return null; // ambiguous or silent — skip
...
export function makeRecall(slide, others, id) {
