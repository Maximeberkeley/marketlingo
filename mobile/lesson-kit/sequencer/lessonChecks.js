import { norm, sentences, shuffle } from './extract';
...
const cite = (sentence) => `Straight from what you just read: ${clean(sentence)}`;
// ── 1. Definition check — this lesson's own key terms ──────────────
export function checkDefinition(terms, id) {
    const usable = terms.filter(t => t.term?.trim() && (t.definition || '').trim().length > 14);
    const unique = usable.filter((t, i) => usable.findIndex(o => norm(o.term) === norm(t.term)) === i);
    if (unique.length < 3)
        return null;
...
export function checkFigure(slides, id) {
    for (const slide of shuffle(slides)) {
        ...
        if (/[A-Za-z-]$/.test(body.slice(Math.max(0, at - 1), at)))
            continue;
...
export function checkMechanism(slides, id) {
    const ranked = slides
...
const isYear = (value) => Number.isInteger(value) && value >= 1900 && value <= 2099;
...
export function checkClaim(slides, id) {
    ...
    for (const truth of pool) {
        ...
        const lies = others.map(falsify).filter((s) => Boolean(s) && norm(s) !== norm(truth)).slice(0, 2);
        if (lies.length < 2)
            continue;
...
export function checkTamper(slides, id) {
 ...
 */
export function termsFromSlides(slides) {
    const found = [];
    for (const slide of slides) {
        for (const sentence of sentences(slide.body || '', 20, 320)) {
            ...
            if (words.length > 4)
                continue;
            if (words.some(w => NOT_A_TERM.has(norm(w))))
                continue;
            if (norm(term).length < 4)
                continue;
            if (found.some(f => norm(f.term) === norm(term)))
                continue;
 ...
 */
export function lessonCheckFactories(slides, terms, nextId) {
