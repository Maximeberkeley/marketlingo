...
export function WordBank({ exercise, phase, onChange }) {
    const bank = useMemo(() => shuffle([...exercise.answer, ...(exercise.distractors ?? [])].map((word, i) => ({
    ...
    return (<View style={styles.wrap}>
      ...
      <View style={styles.answerArea}>
        {placed.length === 0 ? (<Text style={styles.placeholder}>Tap the words to build your answer</Text>) : (<View style={styles.tileRow}>
            {placed.map(tile => (<TouchableOpacity key={tile.key} style={[styles.tile, styles.tilePlaced]} onPress={() => remove(tile)}>
      ...
      <View style={styles.tileRow}>
        {available.map(tile => (<TouchableOpacity key={tile.key} style={styles.tile} onPress={() => add(tile)}>
            <Text style={styles.tileText}>{tile.word}</Text>
          </TouchableOpacity>))}
      </View>
    </View>);
