...
export function packNumber(pack, id) {
...
export function packSpeedRound(pack, id) {
 ...
 */
export function trainerCall(row, id) {
    if (!row)
        return null;
...
export function drillTrueFalse(drills, id) {
...
export function statTrend(stats, id) {
    const s = pick(stats);
    if (!s)
        return null;
...
export function statFaceOff(stats, id) {
    const two = shuffle(stats).slice(0, 2);
    if (two.length < 2 || two[0].value === two[1].value)
        return null;
    const [a, b] = two;
...
export function statColdOpen(stats, id, eyebrow) {
