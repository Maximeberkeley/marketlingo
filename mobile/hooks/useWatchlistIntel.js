 ...
 */
export function useWatchlistIntel(marketId, watchlist) {
    const { user } = useAuth();
    ...
    const getNewsCountForCompany = (companyName) => relevantNews.filter((n) => n.matchedCompany === companyName).length;
    return {
        relevantNews,
        loading,
