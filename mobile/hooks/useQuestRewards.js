 ...
 */
export function useQuestRewards(quests, marketId, addXP) {
    const { user } = useAuth();
    ...
    const claim = useCallback(async (quest) => {
        ...
        if (claimed.has(key) || inFlight.current.has(key))
            return;
    ...
    useEffect(() => {
        if (!user || !marketId || loadedFor.current === null)
            return;
        const pending = quests.filter((q) => q.isCompleted && !claimed.has(`quest:${q.id}:${today}`));
        pending.forEach((q) => {
            void claim(q);
        });
    }, [quests, claimed, claim, user, marketId, today]);
    const isClaimed = useCallback((questId) => claimed.has(`quest:${questId}:${today}`), [claimed, today]);
