...
export function useSessionFlow({ user, selectedMarket, lessonStack, progress, xpData, lessonCompletedToday, currentDay, completeStack, updateStreak, completeLessonForToday, addXP, checkStreakMilestone, checkLevelMilestone, xpRewardLessonComplete, xpRewardStreakBonus, onDataRefresh, }) {
    ...
    const handleStackComplete = useCallback(async (isReviewMode, timeSpentSeconds) => {
        ...
        try {
            ...
            Alert.alert('Saved locally', 'We had trouble syncing your progress. It will retry the next time you are online.');
        }
    ...
    const handleBiteComplete = useCallback(async (isReviewMode, _timeSpentSeconds) => {
        setShowReader(false);
        if (isReviewMode || activeBiteIndex === null)
            return;
    ...
    const handleAddNote = useCallback(async (slideNum, customContent) => {
        if (!user || !activeStack || !selectedMarket)
            return;
        const slide = activeStack.slides.find((s) => s.slide_number === slideNum);
        if (!slide)
            return;
