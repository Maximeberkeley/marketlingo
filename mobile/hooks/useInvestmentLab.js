...
export function useInvestmentLab(marketId) {
    const { user } = useAuth();
    ...
    const updateCompleted = useCallback((updater) => {
            ...
            if (investmentData) {
                const parsed = {
                    ...
                    watchlist_companies: Array.isArray(investmentData.watchlist_companies)
                        ? investmentData.watchlist_companies
                        : [],
                };
    ...
    const ensureProgress = async () => {
        ...
        try {
            ...
            const { data, error } = await supabase
                .from('investment_lab_progress')
                .upsert({ user_id: user.id, market_id: marketId }, { onConflict: 'user_id,market_id' })
                .select()
                .single();
            if (error) {
                ...
                const { data: existing } = await supabase
                    ...
                    .single();
                if (existing) {
                    const parsed = { ...existing, watchlist_companies: Array.isArray(existing.watchlist_companies) ? existing.watchlist_companies : [] };
                    updateProgress(parsed);
                    return parsed;
                }
                return null;
            }
            const newProgress = { ...data, watchlist_companies: [] };
    ...
    const addInvestmentXP = async (amount) => {
        if (!user || !marketId)
            return;
    ...
    const recordAttempt = async (scenarioId, selectedOption, isCorrect, timeSpent) => {
        if (!user || !marketId)
            return;
    ...
    const updateModuleScore = async (module, score) => {
        if (!user || !marketId)
            return;
    ...
    const addToWatchlist = async (company) => {
        ...
        if (currentList.some((c) => c.id === company.id))
            return;
        const enriched = {
            id: company.id,
            name: company.name,
    ...
    const removeFromWatchlist = async (companyId) => {
        if (!user || !marketId || !progressRef.current)
            return;
    ...
    const checkCertificationEligibility = async () => {
        ...
        const meetsThresholds = valuation_score >= CERTIFICATION_THRESHOLDS.valuation &&
    ...
    const addLearnedConcept = async (concept) => {
        if (!user || !marketId)
            return;
        await ensureProgress();
        const current = progressRef.current?.learned_concepts || [];
    ...
    const getLearnedConcepts = () => {
        return progressRef.current?.learned_concepts || [];
    };
    return {
