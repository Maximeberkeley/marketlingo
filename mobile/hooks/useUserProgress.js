...
function useUserProgress(marketId) {
  const { user } = useAuth();
  ...
  const fetchProgress = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await supabase.from("user_progress").select("*").eq("user_id", user.id).eq("market_id", marketId).single();
      if (error && error.code === "PGRST116") {
        const today = localDateString();
        const { data: newProgress, error: createError } = await supabase.from("user_progress").insert({
          user_id: user.id,
          market_id: marketId,
          current_day: 1,
          current_streak: 0,
  ...
  const completeStack = async (stackId) => {
  ...
  const isStreakActive = () => {
    ...
    return new Date(progress.streak_expires_at) > /* @__PURE__ */ new Date();
  };
  return {
    progress,
    loading,
