...
export function useInvestmentLab(marketId?: string) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (attempts) {
        setCompletedScenarioIds(attempts.filter((a) => a.is_correct).map((a) => a.scenario_id));
      }
    } catch (error) {
  ...
  const initializeProgress = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) { console.error('Error initializing investment progress:', error); return null; }
  ...
  const addInvestmentXP = async (amount: number) => {
    ...
    if (!currentProgress) { currentProgress = await initializeProgress(); if (!currentProgress) return; }
  ...
  const recordAttempt = async (scenarioId: string, selectedOption: number, isCorrect: boolean, timeSpent?: number) => {
    ...
    if (isCorrect) setCompletedScenarioIds((prev) => [...prev, scenarioId]);
    await addInvestmentXP(isCorrect ? INVESTMENT_XP_REWARDS.SCENARIO_CORRECT : INVESTMENT_XP_REWARDS.SCENARIO_WRONG);
  };

  const updateModuleScore = async (module: 'valuation' | 'due_diligence' | 'risk_assessment' | 'portfolio_construction', score: number) => {
    if (!user || !marketId) return;
  ...
  const checkCertificationEligibility = async () => {
    ...
    if (meetsThresholds && hasEnoughScenarios && !progress.investment_certified) {
      await supabase.from('investment_lab_progress').update({ investment_certified: true, certified_at: new Date().toISOString() }).eq('user_id', user.id).eq('market_id', marketId);
      await addInvestmentXP(INVESTMENT_XP_REWARDS.CERTIFICATION);
      setProgress((prev) => prev ? { ...prev, investment_certified: true, certified_at: new Date().toISOString() } : prev);
      return true;
    }
    return false;
  };
  ...
  const getOverallProgress = () => {
    ...
    return Math.round((progress.valuation_score + progress.due_diligence_score + progress.risk_assessment_score + progress.portfolio_construction_score) / 4);
  };
  ...
  return {
    ...
    initializeProgress, recordAttempt, addInvestmentXP, updateModuleScore,
    addToWatchlist, removeFromWatchlist, checkCertificationEligibility,
    getOverallProgress, getScenariosByType, refetch: fetchData,
  };
}
