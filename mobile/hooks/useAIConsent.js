...
import { emptyConsent, loadConsent, grantAIConsent, declineAIConsent, grantVoiceConsent, revokeAIConsent, revokeVoiceConsent, } from '../lib/aiConsent';
...
export function useAIConsent() {
    ...
    const request = useCallback((kind) => new Promise((resolve) => {
    ...
    const setAI = useCallback(async (value) => {
        if (value)
            await grantAIConsent();
        else
    ...
    const setVoice = useCallback(async (value) => {
        if (value)
            await grantVoiceConsent();
        else
