...
export function useFriends(marketId) {
    const { user } = useAuth();
    ...
    const fetchFriends = useCallback(async () => {
        ...
        if (!friendships?.length) {
            ...
            const friendList = friendIds.map((fId) => {
                const friendship = friendships.find((f) => (f.user_id === fId || f.friend_id === fId));
    ...
    const sendRequest = useCallback(async (friendIdentifier) => {
        ...
        if (!targetProfile)
            return { success: false, error: 'User not found. Make sure they have an account.' };
