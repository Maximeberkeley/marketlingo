...
export function useWeeklyRecap(marketId) {
    const { user } = useAuth();
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const best = perDay.reduce((acc, cur) => (cur.xp > (acc?.xp ?? 0) ? cur : acc), null);
    ...
    const markSeen = useCallback(async () => {
        if (marketId)
            await AsyncStorage.setItem(`${RECAP_KEY}:${marketId}:${weekOf}`, '1');
