...
export function useDisplayName() {
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                if (active && local)
                    setDisplayNameState(normalizeDisplayName(local));
                if (!user)
                    return;
                ...
                if (active)
                    setDisplayNameState(resolved);
                if (data?.display_name)
                    await storage.setDisplayName(resolved);
            }
            catch (error) {
    ...
    const saveDisplayName = useCallback(async (value) => {
        const clean = normalizeDisplayName(value);
        if (clean === DISPLAY_NAME_FALLBACK && !value.trim())
            return false;
