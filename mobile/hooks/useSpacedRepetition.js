 ...
 */
function sm2(easeFactor, intervalDays, grade) {
...
export function useSpacedRepetition(marketId) {
    const { user } = useAuth();
     ...
     */
    const addToReview = useCallback(async (concept, stackId, slideNumber) => {
        if (!user || !marketId)
            return;
     ...
     */
    const addLessonConcepts = useCallback(async (stackId, slides) => {
        if (!user || !marketId)
            return;
     ...
     * grade: 0=forgot, 1=hard, 2=good, 3=easy
     */
    const gradeReview = useCallback(async (itemId, grade) => {
