...
export function useLeague(marketId) {
    const { user } = useAuth();
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (syncError)
                log.warn('League sync failed', syncError);
            const myRow = Array.isArray(mine) ? mine[0] : mine;
            const tier = (myRow?.tier || 'bronze');
            ...
            const lastWeek = lastRow
                ? {
                    tier: lastRow.tier || 'bronze',
                    result: lastRow.result || 'held',
                    finalRank: lastRow.final_rank ?? null,
                    weekOf: lastRow.week_of,
                }
                : null;
