...
async function sendMilestoneNotification(userId, milestoneType, milestoneData) {
    try {
...
export function useAchievements(progress) {
    const { user } = useAuth();
    ...
    const checkAndUnlockAchievements = useCallback(async (currentProgress) => {
        if (!user)
            return;
        const newlyUnlocked = [];
        for (const achievement of ACHIEVEMENTS) {
            ...
            if (met) {
                ...
                if (!error) {
                    ...
                    try {
                        // Get current XP data (includes market_id for FK constraint)
