...
import {
  ...
  focusKeywords,
  focusOptionsFor,
} from '../lib/focusTopics';

export function useFocusTopic(marketId?: string, availableDay = 1) {
  const [focusKey, setFocusKey] = useState<string | null>(null);
  const [label, setLabel] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  const options: FocusOption[] = useMemo(() => focusOptionsFor(marketId), [marketId]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setFocusKey((data?.focus_key as string) ?? null);
      setLabel((data?.focus_label as string) ?? null);
    } catch (err) {
