...
function getXPAmount(base, isPro) {
...
const STARTUP_STAGES = [
  ...
  { stage: 4, name: "Traction", description: "Finding product-market fit", xpRequired: 3e3 },
  { stage: 5, name: "Scaling", description: "Growing your startup", xpRequired: 5e3 },
  { stage: 6, name: "Established", description: "Market leader", xpRequired: 8e3 }
];
function useUserXP(marketId) {
  const { user } = useAuth();
  const [xpData, setXpData] = useState(null);
  const [dailyCompletion, setDailyCompletion] = useState(null);
  ...
  const ensureXPRecord = useCallback(async () => {
    ...
    try {
      const { data: existingXP, error: xpError } = await supabase.from("user_xp").select("*").eq("user_id", user.id).eq("market_id", marketId).maybeSingle();
      ...
      const { data: newXP, error: createError } = await supabase.from("user_xp").insert({
        user_id: user.id,
        market_id: marketId,
        total_xp: 0,
        current_level: 1,
      ...
      if (createError) {
        const { data: recoveredXP, error: recoverError } = await supabase.from("user_xp").select("*").eq("user_id", user.id).eq("market_id", marketId).maybeSingle();
  ...
  const fetchXPData = useCallback(async () => {
    ...
    try {
      ...
      const { data: todayCompletion, error: completionError } = await supabase.from("daily_completions").select("*").eq("user_id", user.id).eq("market_id", marketId).eq("completion_date", today).maybeSingle();
  ...
  const addXP = async (amount, sourceType, sourceId, description) => {
    if (!user || !marketId) return null;
    const ensuredXP = await ensureXPRecord();
    ...
    const { data: updatedXP, error } = await supabase.rpc("increment_user_xp", {
      p_user_id: user.id,
    ...
    const today = localDateString();
    const { data: existingCompletion, error: completionFetchError } = await supabase.from("daily_completions").select("*").eq("user_id", user.id).eq("market_id", marketId).eq("completion_date", today).maybeSingle();
    ...
    const { data: updatedCompletion, error: completionUpsertError } = await supabase.from("daily_completions").upsert(
      {
        user_id: user.id,
        market_id: marketId,
  ...
  const completeLessonForToday = async (stackId) => {
    if (!user || !marketId) throw new Error("Sign in and choose a market before saving a lesson.");
    const today = localDateString();
    const { data: existing, error: lookupError } = await supabase.from("daily_completions").select("lesson_completed, completed_stack_id").eq("user_id", user.id).eq("market_id", marketId).eq("completion_date", today).maybeSingle();
    if (lookupError) throw lookupError;
    const alreadyDone = Boolean(existing?.lesson_completed);
    const { data, error } = await supabase.from("daily_completions").upsert(
      {
        user_id: user.id,
        market_id: marketId,
        completion_date: today,
