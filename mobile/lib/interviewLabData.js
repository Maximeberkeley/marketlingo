...
export const BIG_BOSS_QUESTIONS = {
    aerospace: [
...
export function dbToMockPrompt(row) {
    return {
        id: row.id,
...
export function dbToMentalMath(row) {
    const opts = typeof row.options === 'string' ? JSON.parse(row.options) : (row.options || []);
    return {
        question: row.question,
        options: opts,
...
export function dbToBigBoss(row) {
    return {
        question: row.question,
        tip: row.explanation || '',
    };
