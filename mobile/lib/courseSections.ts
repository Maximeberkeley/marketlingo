 ...
 */
export function resolveCourseSections(globalDay: number, completedDays: Set<number>): SectionAccess[] {
...
export function latestAccessibleSection(sections: SectionAccess[]): number {
  let latest = 0;
  sections.forEach(section => {
    if (section.unlocked) latest = section.index;
  });
  return latest;
}