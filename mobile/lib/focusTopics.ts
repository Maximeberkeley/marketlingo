...
export interface FocusOption {
  key: string;
  label: string;
  /** What leaning into this corner gets them, in plain words. */
  payoff: string;
}
...
const PAYOFF_HINTS: Array<{ match: RegExp; payoff: string }> = [
...
function payoffFor(label: string) {
 ...
 */
export function focusOptionsFor(marketId?: string): FocusOption[] {
  const market = markets.find(m => m.id === marketId);
  const themes = (market?.themes ?? []).slice(1);
  return themes.map(label => ({ key: slug(label), label, payoff: payoffFor(label) }));
}

export function focusLabel(marketId: string | undefined, key: string | null): string | null {
  if (!key) return null;
...
export function focusKeywords(label: string): string[] {
  return label
    .toLowerCase()
    .replace(/&/g, ' ')
    .split(/[^a-z0-9]+/)
...
export function canChooseFocus(availableDay: number, current: string | null): boolean {
