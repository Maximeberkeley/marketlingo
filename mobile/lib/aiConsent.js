...
export const AI_DISCLOSURE_TEXT = 'Leo uses an external AI service to answer questions and personalize explanations. Your question and relevant lesson context may be sent for processing. Voice recordings are processed when you use voice mode.';
export const VOICE_DISCLOSURE_TEXT = 'Voice mode records your microphone and sends the audio to an external speech service so Leo can hear and reply. Recordings are used only to produce your answer and are not used to identify you.';
const CACHE_KEY = 'ai_consent_state_v1';
export const emptyConsent = { ai: false, voice: false, declined: false, loaded: false };
/** Reads consent from the local cache first (instant), then the backend. */
export async function loadConsent() {
    let cached = { ...emptyConsent };
    try {
        const raw = await AsyncStorage.getItem(CACHE_KEY);
        if (raw)
            cached = { ...cached, ...JSON.parse(raw), loaded: true };
    }
    catch {
        // ignore cache errors
    }
...
async function persist(patch, next) {
    try {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
...
export async function grantAIConsent() {
    await persist({ ai_consent_at: new Date().toISOString(), ai_consent_declined_at: null }, { ai: true, declined: false });
}
export async function declineAIConsent() {
    await persist({ ai_consent_declined_at: new Date().toISOString(), ai_consent_at: null, voice_consent_at: null }, { ai: false, voice: false, declined: true });
}
...
export async function grantVoiceConsent() {
    await persist({ voice_consent_at: new Date().toISOString(), ai_consent_at: new Date().toISOString() }, { voice: true, ai: true, declined: false });
}
