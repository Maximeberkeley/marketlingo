...
export function currentLeoNudgeWindow(date = new Date()) {
    const minutes = date.getHours() * 60 + date.getMinutes();
    if (minutes >= 12 * 60 && minutes <= 14 * 60)
        return 'midday';
    if (minutes >= 17 * 60 && minutes <= 18 * 60 + 30)
        return 'afternoon';
    if (minutes >= 20 * 60 && minutes < 22 * 60)
        return 'evening';
    if (minutes >= 22 * 60 && minutes <= 22 * 60 + 30)
        return 'urgent';
    return null;
}
...
export async function claimLeoNudge(key) {
    const state = await readState();
    if (state.shown.includes(key))
        return false;
    state.shown.push(key);
    await writeState(state);
    return true;
}
export async function cancelRollingLeoNudges() {
    try {
        const state = await readState();
...
export async function scheduleRollingLeoNudges(lessonCompletedToday) {
