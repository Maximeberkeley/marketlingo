...
function generateSineWav(steps, volume) {
...
async function getSoundFilePath(type) {
...
export async function playSound(type) {
    ...
    try {
        ...
        const { sound } = await Audio.Sound.createAsync({ uri: filePath }, { shouldPlay: true, volume: 1.0 });
 ...
 */
export function playSoundAndHaptic(soundType, hapticFn) {
    playSound(soundType);
    hapticFn?.();
}
