...
export type DayOpenMode = 'today' | 'resume' | 'review' | 'locked';

export interface DayState {
  /** Day the curriculum has unlocked for this learner (1-180). */
  availableDay: number;
  /** Local calendar date used for every "today" decision. */
  today: string;
  /** True when today's lesson has already been completed. */
  lessonDoneToday: boolean;
  /** Stack completed for today, when there is one. */
  completedStackId: string | null;
  /** Extra lessons finished today beyond the first. */
  extraLessonsToday: number;
}

export const emptyDayState = (availableDay = 1): DayState => ({
  availableDay,
...
export async function resolveDayState(
  ...
  if (!userId || !marketId) return base;
  ...
  try {
    ...
    return {
      ...
      completedStackId: (data?.completed_stack_id as string | null) ?? null,
      extraLessonsToday: 0,
    };
  } catch (err) {
 ...
 */
export function resolveOpenMode(day: number, state: DayState): DayOpenMode {
  if (day > state.availableDay) return 'locked';
  if (day < state.availableDay) return 'review';
  return state.lessonDoneToday ? 'resume' : 'today';
}

/** Rewards (lesson XP, streak, day credit) are paid on the first run only. */
export function earnsRewards(mode: DayOpenMode): boolean {
  return mode === 'today';
}
